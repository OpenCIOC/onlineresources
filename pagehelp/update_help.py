...
def perform_update(to_database=True):
	...
	allfiles = set(fn for fn in os.listdir('.') if os.path.isfile(fn) and fn.endswith('.htm'))

	dstrs = Recordset()
	dstrs.Open('GBL_PageInfo', conn, ado.adOpenKeyset, ado.adLockOptimistic, ado.adCmdTable)
...
def main(argv):
	...
	parser = OptionParser()
	parser.add_option("-g", "--get", dest="to_database", action="store_false",
