...
Dim cnnProfileDists, cmdProfileDists, rsProfileDists
Call makeNewAdminConnection(cnnProfileDists)
Set cmdProfileDists = Server.CreateObject("ADODB.Command")
With cmdProfileDists
	.ActiveConnection = cnnProfileDists
	.CommandText = "dbo.sp_CIC_ExportProfile_Dist_l"
...
If rsProfileDists.EOF Then
	Call handleError(TXT_NO_RECORD_EXISTS_ID & Server.HTMLEncode(intProfileID) & "." & _
		vbCrLf & "<br>" & TXT_CHOOSE_PROFILE, _
		"export_profile.asp", vbNullString)
Else
	strProfileName = rsProfileDists("ProfileName")
End If

Set rsProfileDists = rsProfileDists.NextRecordset

Call makePageHeader(TXT_MANAGE_DISTS_TITLE & TXT_COLON & strProfileName, TXT_MANAGE_DISTS_TITLE & TXT_COLON & strProfileName, False, False, True, False)
%>
<p align="center">[ <a href="javascript:parent.close()"><%=TXT_CLOSE_WINDOW%></a><%= makePageHelpLink() %> ]</p>
<h2><%=TXT_MANAGE_DISTS_TITLE & " (" & strProfileName & ")"%></h2>
<form action="export_profile_edit_dists2.asp" method="post">
<%=g_strCacheFormVals%>
...
<%
			End If
%>
<label for="UseDist_<%=.Fields("DST_ID")%>"><input type="checkbox" name="UseDist" id="UseDist_<%=.Fields("DST_ID")%>" value="<%=.Fields("DST_ID")%>"<%If .Fields("IS_SELECTED") Then%> checked<%End If%>>&nbsp;<strong><%=.Fields("DistCode") & StringIf(Not Nl(.Fields("DistName"))," - " & .Fields("DistName"))%></strong></label><br>
<%
			bLastState = .Fields("IS_SELECTED")
			.MoveNext
		Wend
