...
h=/[\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,A,G,F={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},C;"function"!==typeof JSON.stringify&&(JSON.stringify=function(t,B,E){var x;G=A="";if("number"===typeof E)for(x=0;x<E;x+=1)G+=" ";else"string"===typeof E&&(G=E);if((C=B)&&"function"!==typeof B&&("object"!==typeof B||"number"!==typeof B.length))throw Error("JSON.stringify");return v("",{"":t})});
...
[];onbeforerestorevalues_fns=[];var v=function(){var q={form_values:get_form_values(g)};l.each(onbeforeunload_fns,function(h,A){A(q)});document.getElementById("cache_form_values").value=JSON.stringify(q)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&v()});document.addEventListener("freeze",v);window.addEventListener("pagehide",v);window.cache_register_onbeforeunload=function(q){onbeforeunload_fns.push(q)};window.cache_register_onbeforerestorevalues=function(q){onbeforerestorevalues_fns.push(q)};
window.restore_cached_state=function(){l(window).on("pageshow",function(q){if(!q.originalEvent.persisted&&(q=document.getElementById("cache_form_values"))&&q.value){var h=JSON.parse(q.value);l.each(onbeforerestorevalues_fns,function(A,G){G(h)});restore_form_values(g,h.form_values)}})}}})();
(function(){var g=jQuery,l={"en-CA":"d M yy",de:"dd.mm.yy",fr:"d M yy",it:"d M yy",nl:"d M yy",no:"d M yy",pt:"d-mm-yy",sv:"d M yy",hu:"M d. yy",ro:"d M yy",hr:"d M yy",sl:"d M yy",bg:"d MM yy",ru:"d M yy",tr:"d M yy",lv:"d M yy",lt:"d M yy",ko:"yy/mm/dd","zh-CN":"yy/mm/dd",th:"d M yy"},v={"en-CA":!0,"":!0},q={},h={"fr-CA":"fr","es-MX":"es"},A=function(){var F=g(this),C=F.data("culture"),t=l[C]||g.datepicker.regional[C].dateFormat,B={};F.hasClass("NoYear")&&(t=t.replace(/[\- .\/]*yy[\/]?/,""),B={beforeShow:function(E,
x){x.dpDiv.addClass("NoYearDatePicker")},onClose:function(E,x){x.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});B.dateFormat=t;F.datepicker(g.extend({},g.datepicker.regional[C],B)).prop("autocomplete","off")},G=function(F){g.getScript("//ajax.googleapis.com/ajax/libs/jqueryui/"+g.ui.version+"/i18n/jquery.ui.datepicker-"+F+".min.js",function(){v[F]=!0;g.each(q[F],function(){A.call(this)});delete q[F]})};g.datepicker.regional["en-CA"]=g.extend({},g.datepicker.regional[""]);g.fn.extend({autodatepicker:function(){return this.each(function(){var F=
g(this);var C=F.parents("[lang]").first().prop("lang")||"en-CA";C=h[C]||C;F.data("culture",C);v[C]?A.call(this):(F=q[C],F||(F=q[C]=[],G(C)),F.push(this))})}});jQuery(function(){g("input.DatePicker").autodatepicker()})})();
...
(function(g){var l=null,v=200,q,h=null,A=null,G=null,F=null,C=function(f,u){f=f.id.substr(12);f=document.getElementById("status_"+f);f.innerHTML=u;f.className=""},t=function(f){A.value=f.rowIndex;for(f=f.nextSibling;f&&(!/\bRowToCode\b/.test(f.className)||"tr"!==f.nodeName.toLowerCase());)f=f.nextSibling;return f},B=function(f,u,D){var P=f.id.substr(12);C(f,g.updating);var R=q===G.current?f.getAttribute("geocode_type"):q,X=null===h?f.getAttribute("map_pin"):h;u={NUM:P,GEOCODE_TYPE:R,MAP_PIN:X,LATITUDE:u?
Globalize.format(u.lat(),"n6"):"",LONGITUDE:u?Globalize.format(u.lng(),"n6"):""};$.ajax(F,{data:u,type:"post",dataType:"json",success:function(Y){C(f,Y.msg);D&&D()},error:function(){C(f,g.error_server);D&&D()}})},E=function(f,u){return function(D,P,R){if(P!==google.maps.GeocoderStatus.OK){if(P===google.maps.GeocoderStatus.OVER_QUERY_LIMIT){v+=100;x(f);return}if(P!==google.maps.GeocoderStatus.ZERO_RESULTS||u){switch(P){case google.maps.GeocoderStatus.ZERO_RESULTS:D=g.error_unknown_address;break;case google.maps.GeocoderStatus.REQUEST_DENIED:D=
g.error_map_key_fail;break;case google.maps.GeocoderStatus.OVER_QUERY_LIMIT:D=g.error_too_many_queries;break;default:D=g.error_unknown_error+P}C(f,D)}else{x(f,!0);return}}else u&&(document.getElementById("status_no_pc_"+f.id.substr(12)).className="Alert"),B(f,D[0].geometry.location);f=t(f);x(f)}},x=function(f,u){if(f){var D=f.getAttribute("can_update"),P=f.getAttribute("geocode_type");if("1"!==D||null===h&&(q===G.dont_change||q===G.current&&(P===G.blank.toString()||P===G.manual.toString())))C(f,g.nochange),
f=t(f),x(f);else if(q===G.dont_change)B(f,null,function(){f=t(f);x(f)});else if(q===G.blank)P!==G.blank&&(B(f,null),f=t(f),x(f));else{var R;(R=u?f.getAttribute("geocode_query_no_pc"):f.getAttribute("geocode_query"))&&"Canada"!==R?setTimeout(function(){C(f,g.coding);l.geocode({address:R},E(f,u))},v):(u?C(f,g.unknown):C(f,g.nochange),f=t(f),x(f))}}};window.maps_loaded=function(){l=new google.maps.Geocoder;var f=document.getElementById("table_header");"0"!==A.value&&(f=document.getElementById("geocode_table").rows[A.value]);
f=t(f);x(f)};var p=function(f,u){$.getScript("//maps.googleapis.com/maps/api/js?v=3&"+u+"&sensor=false&callback=maps_loaded&language="+f)};window.initialize=function(f){h=f.map_pin_target;G=f.types;F=f.url;q=f.geocode_type_target;Globalize.culture(f.culture);g=f.status_messages;A=document.getElementById("stateField");if("0"!==A.value){var u=document.getElementById("restart_geocode"),D=document.getElementById("continue_geocode");u.className="";u.onclick=function(){A.value="0";u.className="NotVisible";
