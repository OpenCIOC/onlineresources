...
x){x.dpDiv.addClass("NoYearDatePicker")},onClose:function(E,x){x.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});B.dateFormat=t;G.datepicker(g.extend({},g.datepicker.regional[C],B)).prop("autocomplete","off")},F=function(G){g.getScript("https://ajax.googleapis.com/ajax/libs/jqueryui/"+g.ui.version+"/i18n/jquery.ui.datepicker-"+G+".min.js",function(){v[G]=!0;g.each(q[G],function(){A.call(this)});delete q[G]})};g.datepicker.regional["en-CA"]=g.extend({},g.datepicker.regional[""]);g.fn.extend({autodatepicker:function(){return this.each(function(){var G=
...
f=t(f);x(f)};var p=function(f,u){$.getScript("https://maps.googleapis.com/maps/api/js?v=3&"+u+"&sensor=false&callback=maps_loaded&region=CA&language="+f)};window.initialize=function(f){h=f.map_pin_target;F=f.types;G=f.url;q=f.geocode_type_target;Globalize.culture(f.culture);g=f.status_messages;A=document.getElementById("stateField");if("0"!==A.value){var u=document.getElementById("restart_geocode"),D=document.getElementById("continue_geocode");u.className="";u.onclick=function(){A.value="0";u.className=
