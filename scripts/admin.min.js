...
g.data("maxAdd");k&&(k=parseInt(k,10));var n=[];l.value&&(n=a.map(l.value.split(","),function(a){return 0===a.indexOf("NEW")?null:a}));var p={},s=function(){g.find(".EntryFormItemBox").each(function(b){a(this).find(".EntryFormItemCount").text(b+1)})},r=function(a){if(k){var b=g.find(".EntryFormItemContent:hidden").parent().find("button.EntryFormItemDelete").prop("disabled",!a);q.prop("disabled",!a);a?(b.removeClass("ui-state-disabled"),q.removeClass("ui-state-disabled")):(b.addClass("ui-state-disabled"),
...
"2"==a?(b.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-month-label").show(),b.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-week-label").hide()):(b.find(".repeat-every-ui, .repeat-week-of-month-ui, .repeats-on-ui, .recurs-month-label").show(),b.find(".repeat-day-of-month-ui, .recurs-week-label").hide())};c("#ScheduleEditArea").on("change",".recur-type-selector",d).find(".recur-type-selector").each(d)}})();
