...
[];onbeforerestorevalues_fns=[];var l=function(){var g=document.getElementById("cache_form_values");if(g){var a={form_values:get_form_values(c)};n.each(onbeforeunload_fns,function(b,d){d(a)});g.value=JSON.stringify(a)}};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&l()});document.addEventListener("freeze",l);window.addEventListener("pagehide",l);window.cache_register_onbeforeunload=function(g){onbeforeunload_fns.push(g)};window.cache_register_onbeforerestorevalues=
function(g){onbeforerestorevalues_fns.push(g)};window.restore_cached_state=function(){n(window).on("pageshow",function(g){if(!g.originalEvent.persisted&&(g=document.getElementById("cache_form_values"))&&g.value){var a=JSON.parse(g.value);n.each(onbeforerestorevalues_fns,function(b,d){d(a)});restore_form_values(c,a.form_values)}})}}})();
...
q){q.dpDiv.addClass("NoYearDatePicker")},onClose:function(p,q){q.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});k.dateFormat=f;e.datepicker(c.extend({},c.datepicker.regional[h],k)).prop("autocomplete","off")},d=function(e){c.getScript("https://ajax.googleapis.com/ajax/libs/jqueryui/"+c.ui.version+"/i18n/jquery.ui.datepicker-"+e+".min.js",function(){l[e]=!0;c.each(g[e],function(){b.call(this)});delete g[e]})};c.datepicker.regional["en-CA"]=c.extend({},c.datepicker.regional[""]);c.fn.extend({autodatepicker:function(){return this.each(function(){var e=
