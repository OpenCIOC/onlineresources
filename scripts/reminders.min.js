...
[];onbeforerestorevalues_fns=[];var l=function(){var g={form_values:get_form_values(c)};n.each(onbeforeunload_fns,function(a,b){b(g)});document.getElementById("cache_form_values").value=JSON.stringify(g)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&l()});document.addEventListener("freeze",l);window.addEventListener("pagehide",l);window.cache_register_onbeforeunload=function(g){onbeforeunload_fns.push(g)};window.cache_register_onbeforerestorevalues=function(g){onbeforerestorevalues_fns.push(g)};
window.restore_cached_state=function(){n(window).on("pageshow",function(g){if(!g.originalEvent.persisted&&(g=document.getElementById("cache_form_values"))&&g.value){var a=JSON.parse(g.value);n.each(onbeforerestorevalues_fns,function(b,d){d(a)});restore_form_values(c,a.form_values)}})}}})();
