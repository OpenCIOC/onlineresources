...
[];onbeforerestorevalues_fns=[];var g=function(){var b={form_values:get_form_values(a)};c.each(onbeforeunload_fns,function(d,k){k(b)});document.getElementById("cache_form_values").value=JSON.stringify(b)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&g()});document.addEventListener("freeze",g);window.addEventListener("pagehide",g);window.cache_register_onbeforeunload=function(b){onbeforeunload_fns.push(b)};window.cache_register_onbeforerestorevalues=function(b){onbeforerestorevalues_fns.push(b)};
window.restore_cached_state=function(){c(window).on("pageshow",function(b){if(!b.originalEvent.persisted&&(b=document.getElementById("cache_form_values"))&&b.value){var d=JSON.parse(b.value);c.each(onbeforerestorevalues_fns,function(k,t){t(d)});restore_form_values(a,d.form_values)}})}}})();
...
m){m.dpDiv.addClass("NoYearDatePicker")},onClose:function(n,m){m.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});h.dateFormat=e;l.datepicker(a.extend({},a.datepicker.regional[p],h)).prop("autocomplete","off")},t=function(l){a.getScript("//ajax.googleapis.com/ajax/libs/jqueryui/"+a.ui.version+"/i18n/jquery.ui.datepicker-"+l+".min.js",function(){g[l]=!0;a.each(b[l],function(){k.call(this)});delete b[l]})};a.datepicker.regional["en-CA"]=a.extend({},a.datepicker.regional[""]);a.fn.extend({autodatepicker:function(){return this.each(function(){var l=
