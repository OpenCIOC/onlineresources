...
"u");return b=b.replace(RegExp("[\u00fd\u00ff]","g"),"y")};window.string_ci_ai=a;var c=function(b,f,k,t,l){k=k||"value";t=t||"";l=l||"";return function(p,e){if(f.term&&(new RegExp(t+b.ui.autocomplete.escapeRegex(f.term)+l,"i")).test(p.term)&&f.content&&13>f.content.length){var h=new RegExp(t+b.ui.autocomplete.escapeRegex(p.term)+l,"i");e(b.grep(f.content,function(n){return h.test(a(n[k]))}));return!0}return!1}};window.default_cache_search_fn=c;var d=function(b,f,k,t,l){k=k||{};l=l||c(b,k,t||"value");
return function(p,e,h){p.term=a(p.term);k.term===p.term&&k.content?e(k.content):l(p,e)||b.ajax({url:h||f,dataType:"json",data:p,cache:!1,success:function(n){k.term=p.term;k.content=n;e(n)}})}};window.create_caching_source_fn=d;window.init_community_autocomplete=function(b,f,k,t,l){var p={};l&&=b(l);l&&!l.length&&(l=null);var e=b("#"+f).autocomplete({focus:function(h,n){return!1},select:function(h,n){e.data({chkid:n.item.chkid,display:n.item.value})},source:d(b,k,p),minLength:t}).keypress(function(h){"13"==
h.keyCode&&(h.preventDefault(),e.autocomplete("close"))});l&&(e.parents("form").on("submit",function(){var h=e.val();if(e.data("display")===h)l.val(e.data("chkid"));else{if(p.content){var n=a(h);h=b.grep(p.content,function(m){return a(m.value)===n});if(1===h.length){l.val(h[0].chkid);e.data({chkid:h[0].chkid,display:h[0].value}).val(h[0].value);return}}l.val("")}}),cache_register_onbeforerestorevalues(function(h){e.length&&(h=h["autocomplete_data_"+e[0].id])&&e.data(h)}),cache_register_onbeforeunload(function(h){if(e.length){var n=
e.data();n.display&&n.chkid&&(h["autocomplete_data_"+e[0].id]={display:n.display,chkid:n.chkid})}}))}})();
