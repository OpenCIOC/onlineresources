...
"u");return d=d.replace(RegExp("[\u00fd\u00ff]","g"),"y")};window.string_ci_ai=c;window.slugify=function(d){d=c(d);d=d.replace(/-/g," ").replace(/[^a-z0-9\s]/g,"");d=d.trim();return d.replace(/\s+/g,"-")};var a=function(d,e,h,r,u){h=h||"value";r=r||"";u=u||"";return function(t,l){if(e.term&&(new RegExp(r+d.ui.autocomplete.escapeRegex(e.term)+u,"i")).test(t.term)&&e.content&&13>e.content.length){var y=new RegExp(r+d.ui.autocomplete.escapeRegex(t.term)+u,"i");l(d.grep(e.content,function(A){return y.test(c(A[h]))}));
return!0}return!1}};window.default_cache_search_fn=a;var b=function(d,e,h,r,u){h=h||{};u=u||a(d,h,r||"value");return function(t,l,y){t.term=c(t.term);h.term===t.term&&h.content?l(h.content):u(t,l)||d.ajax({url:y||e,dataType:"json",data:t,cache:!1,success:function(A){h.term=t.term;h.content=A;l(A)}})}};window.create_caching_source_fn=b;window.init_community_autocomplete=function(d,e,h,r,u){var t={};u&&=d(u);u&&!u.length&&(u=null);var l=d("#"+e).autocomplete({focus:function(y,A){return!1},select:function(y,
A){l.data({chkid:A.item.chkid,display:A.item.value})},source:b(d,h,t),minLength:r}).keypress(function(y){"13"==y.keyCode&&(y.preventDefault(),l.autocomplete("close"))});u&&(l.parents("form").on("submit",function(){var y=l.val();if(l.data("display")===y)u.val(l.data("chkid"));else{if(t.content){var A=c(y);y=d.grep(t.content,function(v){return c(v.value)===A});if(1===y.length){u.val(y[0].chkid);l.data({chkid:y[0].chkid,display:y[0].value}).val(y[0].value);return}}u.val("")}}),cache_register_onbeforerestorevalues(function(y){l.length&&
(y=y["autocomplete_data_"+l[0].id])&&l.data(y)}),cache_register_onbeforeunload(function(y){if(l.length){var A=l.data();A.display&&A.chkid&&(y["autocomplete_data_"+l[0].id]={display:A.display,chkid:A.chkid})}}))}})();
...
c.validator.addMethod("require-group",function(w,G){w=c(G).parents("td[data-field-display-name]");G=w.parent().children("td:first").prop("id");G=G.slice(-(G.length-6));var K=w.find(":input").not("[type=hidden]").not("button");if(window.tinymce){var N=w.find(".WYSIWYG").prop("id");N&&(N=window.tinymce.get(N))&&N.save()}switch(G){case "ACCESSIBILITY":case "AREAS_SERVED":case "LANGUAGES":case "FUNDING":case "SCHOOL_ESCORT":case "SCHOOLS_IN_AREA":case "TYPE_OF_CARE":K=K.not("input:text");break;case "ALT_ORG":case "LEGAL_ORG":K=
K.not("input:checkbox");break;case "FORMER_ORG":K=K.not("input:checkbox").not("[name^=FORMER_ORG_DATE]");break;case "FEES":K=K.not("input:text").add("#FEE_ASSISTANCE_FOR, #FEE_ASSISTANCE_FROM");break;case "VACANCY_INFO":K=K.filter("[name!=VUT_ID]")}(G=l(K))&&w.children("div.required-notice").hide();return G},"Field Required");var H=c('<div id="a_test"></div>').dialog({autoOpen:!1,title:r,closeOnEscape:!1,position:["right","bottom"],resizable:!1,dialogClass:"ValidationErrors"}),x=c("#validation_error_box");
c(document).on("click",".FieldErrorJump",function(w){w=c(w.currentTarget).prop("fieldname");(w=document.getElementById(w))&&w.scrollIntoView(!0)});c(".ValidationErrors.ui-dialog").css({position:"fixed"}).position({my:"right bottom",at:"right bottom",of:window});a(c(h).find("td[data-field-required]"));h=c(h).validate({ignore:"input[type=hidden]",ignoreTitle:!0,errorPlacement:function(w,G){var K=[],N=G[0];if(G.hasClass("require-group")){var R=G.parents("td[data-field-display-name]");K=R.children("div.required-notice");
var O=!1;/radio|checkbox/i.test(N.type)?N.checked&&N.value||(O=!0):c.trim(N.value)||(O=!0);if(O){K.length?(K.empty(),K.show(),K.append(w)):(w.removeAttr("for"),R.prepend(c('<div class="required-notice"></div>').append(w)));return}}K.length&&K.hide();w.insertAfter(G)},showErrors:function(w,G){this.defaultShowErrors();if(0===G.length)H.dialog("close"),x.hide("slow");else{var K={},N=c("<ul>");jQuery.each(G,function(R,O){O=O.element;R=c(O).parents("td[data-field-display-name]");O=R.parent().children("td:first").prop("id");
K[O]||(K[O]=!0,N.append(c("<li>").append(c("<span>").prop({fieldname:O,className:"FieldErrorJump SimulateLink"}).text(R.data("fieldDisplayName")))))});H.empty().append(N);H.dialog("open");x.show("slow");c("#SUBMIT_BUTTON").prop("disabled",!1)}},focusInvalid:!1,onfocusout:!1,onkeyup:!1,onclick:!1});"fr-CA"===t?jQuery.extend(jQuery.validator.messages,{required:"Ce champ est requis.",remote:"Veuillez remplir ce champ pour continuer.",email:"Veuillez entrer une adresse email valide.",url:"Veuillez entrer une URL valide.",
...
"require-group":"Ce champ est requis.",unique:"Une valeur unique est requise."}):jQuery.extend(jQuery.validator.messages,{protourl:"Please enter a valid URL.",posint:"Please enter a positive number.",posdbl:"Please enter a positive number.","record-num":"Please enter a valid record number.","require-group":"Field Required.",unique:"A unique value is required."});return h};window.init_record_type_form=function(h,r,u){var t=null,l=null,y={},A="ORG_LEVEL_1 ORG_LEVEL_2 ORG_LEVEL_3 ORG_LEVEL_4 ORG_LEVEL_5 LOCATION_NAME SERVICE_NAME_LEVEL_1 SERVICE_NAME_LEVEL_2".split(" "),
v=function(){c("#RecordTypeName").html(l);c.each(A,function(z){var E=u[z],H=c("#"+z);H.length&&(H[0].value=E)&&(c("#"+z+"_DISPLAY").text(E),c("#HIDDEN_"+H).prop("value",E))});c("#RECORD_TYPE").prop("value",t[0].value);e()},q=function(){var z=t[0].value;t.data("hasform")||(z=null);t=c(this).find("option:selected");var E=t[0].value;t.data("hasform")||(E=null);E!==z?(y[z]=c("#copyFieldsForm").contents().detach(),y[E]?(c("#copyFieldsForm").append(y[E]),c("#RECORD_TYPE").prop("value",t[0].value)):c("#copyFieldsForm").load(r+
" #copyFieldsInner",{NUM:h,RT:E||t[0].value},v)):e()};jQuery(function(){l=c("#RecordTypeName").html();var z=c("#RECORD_TYPE");c(document).on("change","#RECORD_TYPE",q);t=z.find("option:selected");e()})};window.init_validate_duplicate_org_names=function(h){var r=c.extend({selector:"#EntryForm",fields:"#ORG_LEVEL_1,#ORG_LEVEL_2,#ORG_LEVEL_3,#ORG_LEVEL_4,#ORG_LEVEL_5,#LOCATION_NAME,#SERVICE_NAME_LEVEL_1,#SERVICE_NAME_LEVEL_2,#OLS_SELECT input,#ORG_NUM",only_warn:!0,num:null,org_levels:[],url:null},h),
u=[],t=null!==r.num,l=[],y=c(r.selector).submit(function(A){var v,q=!1,z=c(r.fields).serializeArray();r.num&&z.push({name:"NUM",value:r.num});if(z.length!==u.length)q=!0;else for(v=0;v<z.length;v++){var E=z[v];if(E.name!==u[v].name||E.value!==u[v].value){q=!0;break}}if(!q&&t)return!0;var H=function(){r.only_warn&&confirm(r.confirm_string)?(t=!0,c("#duplicate_name_error_box").hide(),y.submit()):(c("#SUBMIT_BUTTON").prop("disabled",!1),c("#duplicate_name_error_box").show())};A.stopPropagation();A.preventDefault();
if(!q)return H(),!1;c.ajax({url:r.url,dataType:"json",method:"get",data:z,success:function(x){t=!0;l=[];for(var w=0;w<x.length;w++)t&=0===x[w].count,0!==x[w].count&&l.push(x[w].type);u=z;t?(c("#duplicate_name_error_box").hide(),y.submit()):H()}});return!1})}})();
