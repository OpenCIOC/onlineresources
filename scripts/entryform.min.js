...
[];onbeforerestorevalues_fns=[];var m=function(){var e={form_values:get_form_values(b)};g.each(onbeforeunload_fns,function(a,c){c(e)});document.getElementById("cache_form_values").value=JSON.stringify(e)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&m()});document.addEventListener("freeze",m);window.addEventListener("pagehide",m);window.cache_register_onbeforeunload=function(e){onbeforeunload_fns.push(e)};window.cache_register_onbeforerestorevalues=function(e){onbeforerestorevalues_fns.push(e)};
window.restore_cached_state=function(){g(window).on("pageshow",function(e){if(!e.originalEvent.persisted&&(e=document.getElementById("cache_form_values"))&&e.value){var a=JSON.parse(e.value);g.each(onbeforerestorevalues_fns,function(c,d){d(a)});restore_form_values(b,a.form_values)}})}}})();
