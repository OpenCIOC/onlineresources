...
*/
window.CIOC={};
(function(b,f){f.do_drop_down_navigation=function(){for(var m=b.getElementById("ActionList").options,g=0;g<m.length;g++)if(m[g].selected){var a=m[g].getAttribute("href");a&&("undefined"!=typeof m[g].attributes.newwindow?f.open(a):f.location.href=a);break}};jQuery(function(m){m("[data-toggle=dialog]").each(function(){var g=m(this),a=m(g.data("target")),c={autoOpen:!1,title:a.data("title"),modal:a.data("modal")};a.dialog(c);g.click(function(){a.dialog("isOpen")?a.dialog("close"):a.dialog("open")})})})})(document,window);
function CheckAll(b,f){b||="IDList";var m=f;f||(m=document.RecordList);f=m.elements.length;for(var g=0;g<f;g++){var a=m.elements[g];a.name===b&&(a.checked=!0)}}function ClearAll(b,f){b||="IDList";var m=f;f||(m=document.RecordList);f=m.elements.length;for(var g=0;g<f;g++){var a=m.elements[g];a.name===b&&(a.checked=!1)}}function add_class(b,f){b&&!(new RegExp("\\b"+f+"\\b")).test(b.className)&&(b.className&&(f=" "+f),b.className+=f)}
function remove_class(b,f){if(b){for(var m=b.className.split(" "),g=[],a=0;a<m.length;a++){var c=m[a];c!==f&&g.push(c)}b.className=g.join(" ")}}function hide(b){add_class(b,"NotVisible")}function show(b){remove_class(b,"NotVisible")}function openWin(b,f){window.open(b,f,"toolbar=no,width=490,height=485,location=no,scrollBars=yes,resizable=no,titlebar=yes").focus()}
function openWinL(b,f){window.open(b,f,"toolbar=no,width=650,height=520,location=no,scrollBars=yes,resizable=no,titlebar=yes").focus()}function openWinXL(b,f){window.open(b,f,"toolbar=no,width=755,height=550,location=no,scrollBars=yes,resizable=no,titlebar=yes").focus()}jQuery(function(b){var f={sm:openWin,lg:openWinL,xl:openWinXL};b(document).on("click","a.poplink",function(){var m=b(this).data("popargs");(f[m.size||"sm"]||openWin)(this.href,m.name||"popwin");return!1})});
(function(){var b=jQuery,f={},m=function(r){null!==r&&"undefined"!==typeof r?b("#myListCount").text(r):(r=b("#myListCount").text())&&(r=parseInt(r,10));r?f.myListLink.show():f.myListLink.hide()},g=function(r){b.each(r,function(z,A){b("#added_to_list_"+A).show();b("#add_to_list_"+A).hide()})},a=function(r){return function(z){z.fail?alert(z.errinfo):(z.ids?m(b.grep(z.ids,h[f.domain]).length):z.count?m(z.count):m(0),g([r]))}},c=function(r){r=b(r.currentTarget).data("id");b.ajax({success:a(r),dataType:"json",
error:function(){alert("Error")},data:{id:r},type:"POST",url:f.current_list_url})},d=function(r){return function(z){if(z.fail)alert(z.errinfo);else{z=!0;if(r){var A=b("#results_table #remove_from_list_"+r);A.length?A.parent().parent().remove():(z=!1,A=b("#results_container #remove_from_list_"+r),A.parents(".dlist-result").remove())}(z?1!==b("#results_table tr").length:0!==b("#results_container .dlist-result").length)&&r||(b("#records_ui").hide(),b("#no_records_message").show())}}},l=function(r){r=
b(r.currentTarget).data("id");b.ajax({success:d(r),dataType:"json",error:function(){alert("Error")},data:{ID:r,RemoveItem:f.domain},type:"POST",url:f.current_list_url})},p=function(r){b.ajax({success:d(),dataType:"json",error:function(){alert("Error")},data:{ID:"all",RemoveItem:f.domain},type:"POST",url:f.current_list_url})},h={CIC:function(r){return/^[A-Za-z]{3}[0-9]{4,5}$/.test(r)},VOL:function(r){return/^[1-9][0-9]*$/.test(r)}},q=function(r){if(r.fail)y();else if(r.inrequest){var z=b.grep(r.ids,
h[f.domain]);g(z);m(z.length);(r=r.previous_ids)&&b.each(r,function(A,E){b("#ct_added_to_previous_request_"+E).show()});b(".ListUI img").prop("src",function(A,E){return E.replace("list","referral")});b("#list_header_text").hide();b("#ct_header_text").show();f.current_list_url=f.ct_update_url;v()}else y()},y=function(){var r=function(){f.list_view_mode||g(f.already_added||[]);f.current_list_url=f.list_view_mode&&"ct"===f.list_view_mode?f.ct_update_url:f.list_update_url;m((f.already_added||[]).length);
v()};f.list_view_mode||f.has_session?r():b.ajax({dataType:"json",error:null,url:f.list_update_url,data:{SessionTest:"on"},type:"POST",success:function(z){z.has_session&&r()}})},v=function(){b(".HideListUI").delegate(".add_to_list","click",c).delegate(".remove_from_list","click",l).removeClass("HideListUI");b("#remove_all_from_list").click(p);b.browser.msie&&"8.0">b.browser.msie&&b(".ListUI").removeClass("FixIE")};window.init_list_adder=function(r){f=r;f.myListLink=b("#myListLink").parent(":hidden").add("#myListLink");
r.in_request?b.ajax({success:q,dataType:"json",error:null,type:"POST",url:r.in_request}):y()}})();this.JSON||(this.JSON={});
(function(){function b(h){return 10>h?"0"+h:h}function f(h){a.lastIndex=0;return a.test(h)?'"'+h.replace(a,function(q){var y=l[q];return"string"===typeof y?y:"\\u"+("0000"+q.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+h+'"'}function m(h,q){var y=c,v=q[h];v&&"object"===typeof v&&"function"===typeof v.toJSON&&(v=v.toJSON(h));"function"===typeof p&&(v=p.call(q,h,v));switch(typeof v){case "string":return f(v);case "number":return isFinite(v)?String(v):"null";case "boolean":case "null":return String(v);
case "object":if(!v)return"null";c+=d;var r=[];if("[object Array]"===Object.prototype.toString.apply(v)){var z=v.length;for(h=0;h<z;h+=1)r[h]=m(h,v)||"null";q=0===r.length?"[]":c?"[\n"+c+r.join(",\n"+c)+"\n"+y+"]":"["+r.join(",")+"]";c=y;return q}if(p&&"object"===typeof p)for(z=p.length,h=0;h<z;h+=1){var A=p[h];"string"===typeof A&&(q=m(A,v))&&r.push(f(A)+(c?": ":":")+q)}else for(A in v)Object.hasOwnProperty.call(v,A)&&(q=m(A,v))&&r.push(f(A)+(c?": ":":")+q);q=0===r.length?"{}":c?"{\n"+c+r.join(",\n"+
...
"function"!==typeof JSON.parse&&(JSON.parse=function(h,q){function y(v,r){var z,A=v[r];if(A&&"object"===typeof A)for(z in A)if(Object.hasOwnProperty.call(A,z)){var E=y(A,z);void 0!==E?A[z]=E:delete A[z]}return q.call(v,r,A)}h=String(h);g.lastIndex=0;g.test(h)&&(h=h.replace(g,function(v){return"\\u"+("0000"+v.charCodeAt(0).toString(16)).slice(-4)}));if(/^[\],:{}\s]*$/.test(h.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,"")))return h=eval("("+h+")"),"function"===typeof q?y({"":h},""):h;throw new SyntaxError("JSON.parse");})})();
(function(){window.get_form_values=function(b){var f={};jQuery(b).find("input,select,textarea").each(function(m){if(this.name)if(f[this.name]||(f[this.name]=[]),"select"===this.nodeName.toLowerCase()){var g=f[this.name];$(this).find("option").each(function(a){this.selected&&g.push(this.value)})}else("input"!==this.nodeName.toLowerCase()||"checkbox"!==this.type&&"radio"!==this.type||this.checked)&&f[this.name].push(this.value||"")});return f};window.restore_form_values=function(b,f){var m=jQuery;m(b).find("input,select,textarea").each(function(g){if(this.name){var a=
f[this.name];"undefined"!=typeof a&&null!==a&&("input"!==this.nodeName.toLowerCase()||"checkbox"!==this.type&&"radio"!==this.type?"select"===this.nodeName.toLowerCase()?m(this).find("option").each(function(c){this.selected=1<a.length?0<=m.inArray(this.value,a):0===a.length?!1:this.value===a[0]}):this.value=a.length?a[0]:"":this.checked=1<a.length?0<=m.inArray(this.value,a):0===a.length?!1:this.value===a[0])}})};window.init_cached_state=function(b){var f=jQuery;b=b||"#EntryForm";onbeforeunload_fns=
[];onbeforerestorevalues_fns=[];f(window).on("beforeunload",function(){var m={form_values:get_form_values(b)};f.each(onbeforeunload_fns,function(g,a){a(m)});document.getElementById("cache_form_values").value=JSON.stringify(m)});window.cache_register_onbeforeunload=function(m){onbeforeunload_fns.push(m)};window.cache_register_onbeforerestorevalues=function(m){onbeforerestorevalues_fns.push(m)};window.restore_cached_state=function(){var m=document.getElementById("cache_form_values");if(m&&m.value){var g=
JSON.parse(m.value);f.each(onbeforerestorevalues_fns,function(a,c){c(g)});restore_form_values(b,g.form_values)}}}})();
(function(){var b=jQuery,f={"en-CA":"d M yy",de:"dd.mm.yy",fr:"d M yy",it:"d M yy",nl:"d M yy",no:"d M yy",pt:"d-mm-yy",sv:"d M yy",hu:"M d. yy",ro:"d M yy",hr:"d M yy",sl:"d M yy",bg:"d MM yy",ru:"d M yy",tr:"d M yy",lv:"d M yy",lt:"d M yy",ko:"yy/mm/dd","zh-CN":"yy/mm/dd",th:"d M yy"},m={"en-CA":!0,"":!0},g={},a={"fr-CA":"fr","es-MX":"es"},c=function(){var l=b(this),p=l.data("culture"),h=f[p]||b.datepicker.regional[p].dateFormat,q={};l.hasClass("NoYear")&&(h=h.replace(/[\- .\/]*yy[\/]?/,""),q={beforeShow:function(y,
v){v.dpDiv.addClass("NoYearDatePicker")},onClose:function(y,v){v.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});q.dateFormat=h;l.datepicker(b.extend({},b.datepicker.regional[p],q)).prop("autocomplete","off")},d=function(l){b.getScript("//ajax.googleapis.com/ajax/libs/jqueryui/"+b.ui.version+"/i18n/jquery.ui.datepicker-"+l+".min.js",function(){m[l]=!0;b.each(g[l],function(){c.call(this)});delete g[l]})};b.datepicker.regional["en-CA"]=b.extend({},b.datepicker.regional[""]);b.fn.extend({autodatepicker:function(){return this.each(function(){var l=
b(this);var p=l.parents("[lang]").first().prop("lang")||"en-CA";p=a[p]||p;l.data("culture",p);m[p]?c.call(this):(l=g[p],l||(l=g[p]=[],d(p)),l.push(this))})}});jQuery(function(){b("input.DatePicker").autodatepicker()})})();
(function(b){b.widget("ui.combobox",{options:{source:null},_create:function(){var f=this.element;this.options.source&&(b(f).autocomplete({focus:function(m,g){return!1},source:this.options.source,delay:0,minLength:0}),b("<span>").insertAfter(f).addClass("SmallButton DownButton").css("margin-left","1px").click(function(m){f.autocomplete("widget").is(":visible")?f.autocomplete("close"):(f.autocomplete("search",""),f.focus())}))}})})(jQuery);
...
return q}}a=b.validator.normalizeRules(b.extend({},b.validator.classRules(d),b.validator.attributeRules(d),b.validator.dataRules(d),b.validator.staticRules(d)),d);a.required&&(c=a.required,delete a.required,a=b.extend({required:c},a));a.remote&&(c=a.remote,delete a.remote,a=b.extend(a,{remote:c}));return a}}});var f=function(a){return a.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,"")};b.extend(b.expr.pseudos||b.expr[":"],{blank:function(a){return!f(""+b(a).val())},filled:function(a){a=b(a).val();
...
a.cultures["default"].calendar=a.cultures["default"].calendars.standard;a.cultures.en=a.cultures["default"];a.cultureSelector="en";var c=/^0x[a-f0-9]+$/i;var d=/^[+\-]?infinity$/i;var l=/^[+\-]?\d*\.?\d*(e[+\-]?\d+)?$/;var p=/^\s+|\s+$/g;var h=function(e,n){if(e.indexOf)return e.indexOf(n);for(var w=0,k=e.length;w<k;w++)if(e[w]===n)return w;return-1};var q=function(e,n){return e.substr(e.length-n.length)===n};var y=function(){var e,n,w,k=arguments[0]||{},u=1,C=arguments.length,B=!1;"boolean"===typeof k&&
(B=k,k=arguments[1]||{},u=2);for("object"===typeof k||r(k)||(k={});u<C;u++)if(null!=(e=arguments[u]))for(n in e){var t=k[n];var G=e[n];k!==G&&(B&&G&&(z(G)||(w=v(G)))?(w?(w=!1,t=t&&v(t)?t:[]):t=t&&z(t)?t:{},k[n]=y(B,t,G)):G!==f&&(k[n]=G))}return k};var v=Array.isArray||function(e){return"[object Array]"===Object.prototype.toString.call(e)};var r=function(e){return"[object Function]"===Object.prototype.toString.call(e)};var z=function(e){return"[object Object]"===Object.prototype.toString.call(e)};
var A=function(e,n){return 0===e.indexOf(n)};var E=function(e){return(e+"").replace(p,"")};var x=function(e){return isNaN(e)?NaN:Math[0>e?"ceil":"floor"](e)};var D=function(e,n,w){var k;for(k=e.length;k<n;k+=1)e=w?"0"+e:e+"0";return e};var J=function(e,n){for(var w=0,k=!1,u=0,C=e.length;u<C;u++){var B=e.charAt(u);switch(B){case "'":k?n.push("'"):w++;k=!1;break;case "\\":k&&n.push("\\");k=!k;break;default:n.push(B),k=!1}}return w};var W=function(e,n){n=n||"F";e=e.patterns;var w=n.length;if(1===w){e=
e[n];if(!e)throw"Invalid date format string '"+n+"'.";n=e}else 2===w&&"%"===n.charAt(0)&&(n=n.charAt(1));return n};var N=function(e,n,w){function k(T,P){T+="";return 1<P&&T.length<P?(T=K[P-2]+T,T.substr(T.length-P,P)):T}function u(T,P){if(V)return V[P];switch(P){case 0:return T.getFullYear();case 1:return T.getMonth();case 2:return T.getDate();default:throw"Invalid part value "+P;}}var C=w.calendar,B=C.convert;if(!n||!n.length||"i"===n){if(w&&w.name.length)if(B)w=N(e,C.patterns.F,w);else{n=new Date(e.getTime());
var t=Z(e,C.eras);n.setFullYear(F(e,C,t));w=n.toLocaleString()}else w=e.toString();return w}var G=C.eras,H="s"===n;n=W(C,n);w=[];var K=["0","00","000"],Q=/([^d]|^)(d|dd)([^d]|$)/g,O=0,R=X(),V;for(!H&&B&&(V=B.fromGregorian(e));;){var L=R.lastIndex;B=R.exec(n);L=n.slice(L,B?B.index:n.length);O+=J(L,w);if(!B)break;if(O%2)w.push(B[0]);else switch(L=B[0],B=L.length,L){case "ddd":case "dddd":w.push((3===B?C.days.namesAbbr:C.days.names)[e.getDay()]);break;case "d":case "dd":t=!0;w.push(k(u(e,2),B));break;
case "MMM":case "MMMM":L=u(e,1);var M=w,ba=M.push,ca;if(ca=C.monthsGenitive){if(!t&&!Y){t=Q.test(n);var Y=!0}ca=t}ba.call(M,ca?C.monthsGenitive[3===B?"namesAbbr":"names"][L]:C.months[3===B?"namesAbbr":"names"][L]);break;case "M":case "MM":w.push(k(u(e,1)+1,B));break;case "y":case "yy":case "yyyy":L=V?V[0]:F(e,C,Z(e,G),H);4>B&&(L%=100);w.push(k(L,B));break;case "h":case "hh":L=e.getHours()%12;0===L&&(L=12);w.push(k(L,B));break;case "H":case "HH":w.push(k(e.getHours(),B));break;case "m":case "mm":w.push(k(e.getMinutes(),
B));break;case "s":case "ss":w.push(k(e.getSeconds(),B));break;case "t":case "tt":L=12>e.getHours()?C.AM?C.AM[0]:" ":C.PM?C.PM[0]:" ";w.push(1===B?L.charAt(0):L);break;case "f":case "ff":case "fff":w.push(k(e.getMilliseconds(),3).substr(0,B));break;case "z":case "zz":L=e.getTimezoneOffset()/60;w.push((0>=L?"+":"-")+k(Math.floor(Math.abs(L)),B));break;case "zzz":L=e.getTimezoneOffset()/60;w.push((0>=L?"+":"-")+k(Math.floor(Math.abs(L)),2)+":"+k(Math.abs(e.getTimezoneOffset()%60),2));break;case "g":case "gg":C.eras&&
w.push(C.eras[Z(e,G)].name);break;case "/":w.push(C["/"]);break;default:throw"Invalid date format pattern '"+L+"'.";}}return w.join("")};(function(){var e=function(n,w,k){var u=k.groupSizes,C=u[0],B=1,t=Math.pow(10,w);t=Math.round(n*t)/t;isFinite(t)||(t=n);n=t;t=(n+"").split(/e/i);var G=1<t.length?parseInt(t[1],10):0;n=t[0];t=n.split(".");n=t[0];t=1<t.length?t[1]:"";0<G?(t=D(t,G,!1),n+=t.slice(0,G),t=t.substr(G)):0>G&&(G=-G,n=D(n,G+1,!0),t=n.slice(-G,n.length)+t,n=n.slice(0,-G));t=0<w?k["."]+(t.length>
...
10));var B=w.charAt(0).toUpperCase();switch(B){case "D":w="n";u=x(u);-1!==C&&(u=D(""+u,C,!0));0>n&&(u="-"+u);break;case "N":var t=k;case "C":t=t||k.currency;case "P":t=t||k.percent;w=0>n?t.pattern[0]:t.pattern[1]||"n";-1===C&&(C=t.decimals);u=e(u*("P"===B?100:1),C,t);break;default:throw"Bad number format specifier: "+B;}n=/n|\$|-|%/g;for(t="";;){C=n.lastIndex;B=n.exec(w);t+=w.slice(C,B?B.index:w.length);if(!B)break;switch(B[0]){case "n":t+=u;break;case "$":t+=k.currency.symbol;break;case "-":/[1-9]/.test(u)&&
(t+=k["-"]);break;case "%":t+=k.percent.symbol}}return t}})();var X=function(){return/\/|dddd|ddd|dd|d|MMMM|MMM|MM|M|yyyy|yy|y|hh|h|HH|H|mm|m|ss|s|tt|t|fff|ff|f|zzz|zz|z|gg|g/g};var Z=function(e,n){if(!n)return 0;for(var w=e.getTime(),k=0,u=n.length;k<u;k++)if(e=n[k].start,null===e||w>=e)return k;return 0};var F=function(e,n,w,k){e=e.getFullYear();!k&&n.eras&&(e-=n.eras[w].offset);return e};(function(){var e=function(t,G){if(100>G){var H=new Date,K=Z(H);H=F(H,t,K);t=t.twoDigitYearMax;t="string"===
...
var C=function(t){return t.split("\u00a0").join(" ").toUpperCase()};var B=function(t){for(var G=[],H=0,K=t.length;H<K;H++)G[H]=C(t[H]);return G};g=function(t,G,H){t=E(t);H=H.calendar;G=k(H,G);var K=(new RegExp(G.regExp)).exec(t);if(null===K)return null;var Q=G.groups,O=G=t=null,R=null,V=null,L=0,M=0,ba=0,ca=0;var Y=null;for(var T=!1,P=0,ea=Q.length;P<ea;P++){var S=K[P+1];if(S){var U=Q[P],da=U.length,aa=parseInt(S,10);switch(U){case "dd":case "d":R=aa;if(u(R,1,31))return null;break;case "MMM":case "MMMM":O=
w(H,S,3===da);if(u(O,0,11))return null;break;case "M":case "MM":O=aa-1;if(u(O,0,11))return null;break;case "y":case "yy":case "yyyy":G=4>da?e(H,aa):aa;if(u(G,0,9999))return null;break;case "h":case "hh":L=aa;12===L&&(L=0);if(u(L,0,11))return null;break;case "H":case "HH":L=aa;if(u(L,0,23))return null;break;case "m":case "mm":M=aa;if(u(M,0,59))return null;break;case "s":case "ss":ba=aa;if(u(ba,0,59))return null;break;case "tt":case "t":T=H.PM&&(S===H.PM[0]||S===H.PM[1]||S===H.PM[2]);if(!T&&(!H.AM||
...
parseInt(H/60,10),H%60));return K}})();var I=function(e,n,w){var k=n["-"];n=n["+"];var u;switch(w){case "n -":k=" "+k,n=" "+n;case "n-":q(e,k)?u=["-",e.substr(0,e.length-k.length)]:q(e,n)&&(u=["+",e.substr(0,e.length-n.length)]);break;case "- n":k+=" ",n+=" ";case "-n":A(e,k)?u=["-",e.substr(k.length)]:A(e,n)&&(u=["+",e.substr(n.length)]);break;case "(n)":A(e,"(")&&q(e,")")&&(u=["-",e.substr(1,e.length-2)])}return u||["",e]};a.prototype.findClosestCulture=function(e){return a.findClosestCulture.call(this,
e)};a.prototype.format=function(e,n,w){return a.format.call(this,e,n,w)};a.prototype.localize=function(e,n){return a.localize.call(this,e,n)};a.prototype.parseInt=function(e,n,w){return a.parseInt.call(this,e,n,w)};a.prototype.parseFloat=function(e,n,w){return a.parseFloat.call(this,e,n,w)};a.prototype.culture=function(e){return a.culture.call(this,e)};a.addCultureInfo=function(e,n,w){var k=!1;"string"!==typeof e?(w=e,e=this.culture().name,n=this.cultures[e]):"string"!==typeof n?(w=n,k=null==this.cultures[e],
n=this.cultures[e]||this.cultures["default"]):(k=!0,n=this.cultures[n]);this.cultures[e]=y(!0,{},n,w);k&&(this.cultures[e].calendar=this.cultures[e].calendars.standard)};a.findClosestCulture=function(e){var n;if(!e)return this.findClosestCulture(this.cultureSelector)||this.cultures["default"];"string"===typeof e&&(e=e.split(","));if(v(e)){var w=this.cultures,k=e,u,C=k.length,B=[];for(u=0;u<C;u++){e=E(k[u]);e=e.split(";");var t=E(e[0]);1===e.length?e=1:(e=E(e[1]),0===e.indexOf("q=")?(e=e.substr(2),
e=parseFloat(e),e=isNaN(e)?0:e):e=1);B.push({lang:t,pri:e})}B.sort(function(H,K){return H.pri<K.pri?1:H.pri>K.pri?-1:0});for(u=0;u<C;u++)if(t=B[u].lang,n=w[t])return n;for(u=0;u<C;u++){t=B[u].lang;do{k=t.lastIndexOf("-");if(-1===k)break;t=t.substr(0,k);if(n=w[t])return n}while(1)}for(u=0;u<C;u++){t=B[u].lang;for(var G in w)if(k=w[G],k.language==t)return k}}else if("object"===typeof e)return e;return n||null};a.format=function(e,n,w){w=this.findClosestCulture(w);e instanceof Date?e=N(e,n,w):"number"===
typeof e&&(e=m(e,n,w));return e};a.localize=function(e,n){return this.findClosestCulture(n).messages[e]||this.cultures["default"].messages[e]};a.parseDate=function(e,n,w){w=this.findClosestCulture(w);var k;if(n){if("string"===typeof n&&(n=[n]),n.length){var u=0;for(var C=n.length;u<C;u++){var B=n[u];if(B&&(k=g(e,B,w)))break}}}else for(u in n=w.calendar.patterns,n)if(k=g(e,n[u],w))break;return k||null};a.parseInt=function(e,n,w){return x(a.parseFloat(e,n,w))};a.parseFloat=function(e,n,w){"number"!==
typeof n&&(w=n,n=10);var k=this.findClosestCulture(w);w=NaN;var u=k.numberFormat;-1<e.indexOf(k.numberFormat.currency.symbol)&&(e=e.replace(k.numberFormat.currency.symbol,""),e=e.replace(k.numberFormat.currency["."],k.numberFormat["."]));e=E(e);if(d.test(e))w=parseFloat(e);else if(!n&&c.test(e))w=parseInt(e,16);else{k=I(e,u,u.pattern[0]);n=k[0];k=k[1];""===n&&"(n)"!==u.pattern[0]&&(k=I(e,u,"(n)"),n=k[0],k=k[1]);""===n&&"-n"!==u.pattern[0]&&(k=I(e,u,"-n"),n=k[0],k=k[1]);n=n||"+";e=k.indexOf("e");0>
e&&(e=k.indexOf("E"));if(0>e){var C=k;e=null}else C=k.substr(0,e),e=k.substr(e+1);var B=u["."],t=C.indexOf(B);0>t?(k=C,C=null):(k=C.substr(0,t),C=C.substr(t+B.length));B=u[","];k=k.split(B).join("");t=B.replace(/\u00A0/g," ");B!==t&&(k=k.split(t).join(""));n+=k;null!==C&&(n+="."+C);null!==e&&(u=I(e,u,"-n"),n+="e"+(u[0]||"+")+u[1]);l.test(n)&&(w=parseFloat(n))}return w};a.culture=function(e){"undefined"!==typeof e&&(this.cultureSelector=e);return this.findClosestCulture(e)||this.cultures["default"]}})(this);
(function(){draggable=!1;var b=null,f=null,m=[59.888937,-101.601562],g=null,a=null,c=!0,d=null,l=null,p=function(){f.setCenter(new google.maps.LatLng(m[0],m[1]),2)},h=function(){a&&a.setMap(null);a=null;l&&(google.maps.event.removeListener(l),l=null)},q=function(k,u){document.getElementById("LATITUDE").value=Globalize.format(k,"n6");document.getElementById("LONGITUDE").value=Globalize.format(u,"n6")},y=function(k,u){k=new google.maps.LatLng(k,u);c&&f.setZoom(14);f.setCenter(k);c=!1;k=new google.maps.Marker({position:k,
map:f,clickable:!1,draggable});h();a=k;draggable&&(l=google.maps.event.addListener(k,"dragend",v))},v=function(){$("#GEOCODE_TYPE_SITE_REFRESH, #GEOCODE_TYPE_INTERSECTION_REFRESH").addClass("NotVisible");$("#GEOCODE_TYPE_MANUAL").prop("checked",!0);var k=this.getPosition();q(k.lat(),k.lng())},r=function(k){k&&(d=X,q(k.lat(),k.lng()),y(k.lat(),k.lng()))},z=function(k){switch(k){case google.maps.GeocoderStatus.ZERO_RESULTS:k=pageconstants.txt_geocode_unknown_address;break;case google.maps.GeocoderStatus.REQUEST_DENIED:k=
pageconstants.txt_geocode_map_key_fail;break;case google.maps.GeocoderStatus.OVER_QUERY_LIMIT:k=pageconstants.txt_geocode_too_many_queries;break;default:k=pageconstants.txt_geocode_unknown_error+k}return k};window.get_response_message=z;var A=function(k){return function(u,C){C!==google.maps.GeocoderStatus.OK?k(null,C):k(u[0].geometry.location,C)}};window.handle_geocode=A;var E=function(){var k={zoom:13,mapTypeId:google.maps.MapTypeId.ROADMAP};b=document.getElementById("map_canvas");f=new google.maps.Map(b,
k);p();g||=new google.maps.Geocoder;k=$("#LATITUDE")[0].value;var u=$("#LONGITUDE")[0].value;k&&u&&y(Globalize.parseFloat(k),Globalize.parseFloat(u))},x=[],D=!1;window.maps_loaded=function(){D=!0;$.each(x,function(k,u){u()});x=[]};window.add_maps_loaded_callback=function(k){D?k():x.push(k)};window.initialize_maps=function(k,u,C,B){if(!window.cioc_map_script_added){window.cioc_map_script_added=!0;Globalize.culture(k);var t="";B&&(t="&libraries=places");$.getScript("//maps.googleapis.com/maps/api/js?v=3&"+
...
u.push(t):u.push("Canada");return u.join(", ")},X=null,Z=function(k,u,C){X=k;C=J(u,C);(X=k)?g.geocode({address:k},C):(h(),q("",""));k?$("#geocode_no_postal_code").hide():u(null)},F=function(k,u){var C=W(u);Z(C,k,u)},I=function(k){var u=N();Z(u,k)};window.handle_geocode_entryform_feedback=function(k,u){"undefined"!=typeof k&&null!==k&&"undefined"!=typeof u&&null!==u&&(d=X,r(google.maps.LatLng(Globalize.parseFloat(k),Globalize.parseFloat(u))))};var e=function(k){r(k)},n=function(k){w(!1);$("#GEOCODE_TYPE_SITE_REFRESH,#GEOCODE_TYPE_INTERSECTION_REFRESH").hide();
a||k||F(function(u){u?r(u):(c=!1,u=f.getCenter(),u=u.toUrlValue().split(","),q.apply(null,u),y.apply(null,u),X=d=null)})};window.do_geocode_type_manual=n;var w=function(k){var u=document.getElementById("map_refresh_ui_area");u&&(document.getElementById("LONGITUDE").readOnly=k,(document.getElementById("LATITUDE").readOnly=k)?$(u).hide():$(u).show())};window.entryform_maps_loaded=function(){draggable=!0;E();var k=$("#GEOCODE_TYPE_INTERSECTION_REFRESH"),u=$("#GEOCODE_TYPE_SITE_REFRESH");$("#GEOCODE_TYPE_SITE").click(function(){k.hide();
...
(function(){var b=function(g){g=g.toLowerCase();g=g.replace(RegExp("[\u00e0\u00e1\u00e2\u00e3\u00e4\u00e5]","g"),"a");g=g.replace(RegExp("\u00e6","g"),"ae");g=g.replace(RegExp("\u00e7","g"),"c");g=g.replace(RegExp("[\u00e8\u00e9\u00ea\u00eb]","g"),"e");g=g.replace(RegExp("[\u00ec\u00ed\u00ee\u00ef]","g"),"i");g=g.replace(RegExp("\u00f1","g"),"n");g=g.replace(RegExp("[\u00f2\u00f3\u00f4\u00f5\u00f6]","g"),"o");g=g.replace(RegExp("\u009c","g"),"oe");g=g.replace(RegExp("[\u00f9\u00fa\u00fb\u00fc]","g"),
"u");return g=g.replace(RegExp("[\u00fd\u00ff]","g"),"y")};window.string_ci_ai=b;window.slugify=function(g){g=b(g);g=g.replace(/-/g," ").replace(/[^a-z0-9\s]/g,"");g=g.trim();return g.replace(/\s+/g,"-")};var f=function(g,a,c,d,l){c=c||"value";d=d||"";l=l||"";return function(p,h){if(a.term&&(new RegExp(d+g.ui.autocomplete.escapeRegex(a.term)+l,"i")).test(p.term)&&a.content&&13>a.content.length){var q=new RegExp(d+g.ui.autocomplete.escapeRegex(p.term)+l,"i");h(g.grep(a.content,function(y){return q.test(b(y[c]))}));
return!0}return!1}};window.default_cache_search_fn=f;var m=function(g,a,c,d,l){c=c||{};l=l||f(g,c,d||"value");return function(p,h,q){p.term=b(p.term);c.term===p.term&&c.content?h(c.content):l(p,h)||g.ajax({url:q||a,dataType:"json",data:p,cache:!1,success:function(y){c.term=p.term;c.content=y;h(y)}})}};window.create_caching_source_fn=m;window.init_community_autocomplete=function(g,a,c,d,l){var p={};l&&=g(l);l&&!l.length&&(l=null);var h=g("#"+a).autocomplete({focus:function(q,y){return!1},select:function(q,
y){h.data({chkid:y.item.chkid,display:y.item.value})},source:m(g,c,p),minLength:d}).keypress(function(q){"13"==q.keyCode&&(q.preventDefault(),h.autocomplete("close"))});l&&(h.parents("form").on("submit",function(){var q=h.val();if(h.data("display")===q)l.val(h.data("chkid"));else{if(p.content){var y=b(q);q=g.grep(p.content,function(v){return b(v.value)===y});if(1===q.length){l.val(q[0].chkid);h.data({chkid:q[0].chkid,display:q[0].value}).val(q[0].value);return}}l.val("")}}),cache_register_onbeforerestorevalues(function(q){h.length&&
...
(function(){window.entryform=window.entryform||{};window.create_checklist_onbefore_fns=function(m,g,a,c){cache_register_onbeforeunload(function(d){d[g+"_added"]=a});cache_register_onbeforerestorevalues(function(d){(d=d[g+"_added"])&&m.each(d,function(l,p){c(p.chkid,p.display,"")})})};window.basic_chk_add_html=function(m,g){return function(a,c){a=m("<tr>").append(m("<td>").append(m("<input>").prop({id:g+"_ID_"+a,type:"checkbox",checked:!0,defaultChecked:!0,name:g+"_ID",value:a})).append(document.createTextNode(" "+
c))).append(m("<td>").append(m("<input>").prop({id:g+"_NOTES_"+a,name:g+"_NOTES_"+a,size:entryform.chk_notes_size,maxlength:entryform.chk_notes_maxlen})));m("#"+g+"_existing_add_table").append(a);return a.find('input[type="checkbox"]')[0]}};var b={},f=function(m){$("#NEW_"+m).autocomplete(b[m])};window.init_autocomplete_checklist=function(m,g){var a=g.field;if(!b[a]){var c=g.source;g.minLength=g.minLength||1;g.delay=g.delay||300;g.add_new_html=g.add_new_html||basic_chk_add_html(m,a);g.match_prop=
g.match_prop||"value";g.txt_not_found=g.txt_not_found||"Not Found";var d=m(g.delegate_root||document),l=g.added_values||[],p=function(r,z,A){var E=document.getElementById(a+"_ID_"+r);E?(E.checked=!0,g.after_add_new_value&&g.after_add_new_value.call(E)):(l.push({chkid:r,display:z,label:A}),r=g.add_new_html(r,z,A),g.after_add_new_value&&g.after_add_new_value.call(r))};create_checklist_onbefore_fns(m,a,l,p);var h={},q=function(r){m("#NEW_"+a).data({chkid:"",display:"",label:""}).prop("value","").focus();
m("#"+a+"_error").hide("slow",function(){m(this).remove()})},y=null,v=null;(function(r){m.isArray(r)?(v=function(z,A){var E=new RegExp(m.ui.autocomplete.escapeRegex(string_ci_ai(z.term)));A(m.grep(r,function(x){return E.test(string_ci_ai(x[g.match_prop]||x))}))},y=function(z,A){var E=string_ci_ai(z);z=m.grep(r,function(x){return string_ci_ai(string_ci_ai(x[g.match_prop]||x))===E});if(1===z.length)return A(z[0]),!0;A()}):"string"===typeof r?(v=create_caching_source_fn(m,r,h,g.match_prop),y=function(z,
A,E){var x=string_ci_ai(z);if(h.content){var D=m.grep(h.content,function(J){return string_ci_ai(J[g.match_prop])===x});if(1===D.length){A(D[0]);return}}E||string_ci_ai(h.term||"")===x?A():v({term:x},function(J){y(z,A,!0)})}):(v=r,y=g.look_for_fn)})(c);d.on("click","#add_"+a,function(r){r=m("#NEW_"+a);var z=r.data("chkid"),A=r.data("display"),E=r.data("label"),x=r[0].value;if(z&&A&&A==x)return p(z,A,E),q(),!1;y(r[0].value,function(D){D?(p(D.chkid,D.value,D.label),q()):0===m("#"+a+"_error").length&&
(m("#"+a+"_new_input_table").before(m("<p>").hide().addClass("Alert").prop("id",a+"_error").append(document.createTextNode(g.txt_not_found))),m("#"+a+"_error").show("slow"))});return!1});b[a]={focus:function(r,z){return!1},source:v,minLength:g.minLength,delay:g.delay,select:function(r,z){m("#NEW_"+a).data({chkid:z.item.chkid,display:z.item.value,label:z.item.label})}};d.on("keypress","#NEW_"+a,function(r){"13"==r.keyCode&&(r.preventDefault(),m("#NEW_"+a).autocomplete("close"),m("#add_"+a).trigger("click"))})}f(a)};
window.init_languages=function(m,g){var a=null,c=null,d=m("#LN_existing_add_table"),l=function(){var p=m(this).parents(".language-entry").find(".language-details-notes");this.checked?p.slideDown():p.slideUp()};init_autocomplete_checklist(m,{field:"LN",source:entryform.languages_source,txt_not_found:g,after_add_new_value:l,add_new_html:function(p,h){null===a&&(a=m("#add_LN"),c=a.data("newTemplate"));p=c.replace(/LANGNAMELANGNAME/g,h).replace(/IDIDID/g,p);d.append(p)}});d.on("change",".language-primary",
...
(function(){window.only_items_chk_add_html=function(b,f,m){return function(g,a){m&&m();b("#"+f+"_existing_add_container").append(b("<label>").append(b("<input>").prop({id:f+"_ID_"+g,type:"checkbox",checked:!0,defaultChecked:!0,name:f+"_ID",value:g})).append("&nbsp;").append(document.createTextNode(a))).append(" ; ")}};window.init_check_for_autochecklist=function(b){var f=jQuery,m=function(){return f("<li>").append(f("<span>").data("jump_location",this.id).addClass("UnaddedChecklistJump SimulateLink").append(f(this).parentsUntil("td[data-field-display-name]").parent().data("fieldDisplayName")))},
g=f("input[id^=NEW_]");g.length&&(f("#EntryForm").submit(function(a){var c=g.map(function(){return this.value?this:null});if(c.length&&!a.isDefaultPrevented()){if(!confirm(b)){a.preventDefault();f("#SUBMIT_BUTTON").prop("disabled",!1);a=f("#unadded_checklist_error_box");var d=f("#unadded_checklist_error_list");var l=a.is(":visible");l&&d.children("ul:first").hide("slow",function(){f(this).remove()});var p=f("<ul>");l&&p.hide();d.append(p);f.each(c.map(m),function(){p.append(this)});l?p.show("slow"):
a.show("slow")}}else a.isDefaultPrevented()&&(a=f("#unadded_checklist_error_box"),d=f("#unadded_checklist_error_list"),a.hide("slow",function(){d.children().remove()}))}),f(document).on("click",".UnaddedChecklistJump",function(){var a=f(this).data("jump_location");(a=document.getElementById(a))&&a.scrollIntoView(!0)}))};window.init_entryform_items=function(b,f,m){var g=jQuery;b.each(function(){var a=null,c=this,d=g(c),l=1,p=d.find(".EntryFormItemContainerIds")[0];if(p){var h=d.data("maxAdd");h&&=
parseInt(h,10);var q=[];p.value&&(q=g.map(p.value.split(","),function(x){return 0===x.indexOf("NEW")&&0!==x.indexOf("NEWFB")?null:x}));var y={},v=function(){d.find(".EntryFormItemBox").each(function(x){g(this).find(".EntryFormItemCount").text(x+1)})},r=function(x){if(h){var D=d.find(".EntryFormItemContent:hidden").parent().find("button.EntryFormItemDelete").prop("disabled",!x);E.prop("disabled",!x);x?(D.removeClass("ui-state-disabled"),E.removeClass("ui-state-disabled")):(D.addClass("ui-state-disabled"),
E.addClass("ui-state-disabled"))}},z=function(){var x=0,D;for(D in y)x++;return q.length-x},A=function(x){var D=0;if(h&&!x&&(D=z(),D>=h)){alert("over");return}var J=d.data("addTmpl");q.push("NEW"+l++);p.value=q.join(",");J=g(J.replace(/\[COUNT\]/g,q.length).replace(/\[ID\]/g,q[q.length-1]));J.find(".DatePicker").autodatepicker();J.find(".Province").combobox({source:window.cioc_province_source});entryform.service_titles&&J.find(".ServiceTitleField").combobox({source:entryform.service_titles});d.append(J);
a&&(a.sortable("refresh"),v());h&&!x&&D+1===h&&r(!1)},E=d.parent().find(".EntryFormItemAdd").click(function(){A(!1)});d.on("click","button.EntryFormItemDelete",function(){var x=this,D=g(this).parents(".EntryFormItemBox").find(".EntryFormItemContent"),J=this.id.split("_");J=J[J.length-2];if(D.is(":visible"))D.hide("slow",function(){y[J]=get_form_values(D);g(x).text(m);D.find("input,select,textarea").each(function(){"input"!==this.nodeName.toLowerCase()||"checkbox"!==this.type&&"radio"!==this.type?
"select"===this.nodeName.toLowerCase()?g(this).find("option").each(function(){this.selected=!1}):this.value="":this.checked=!1});r(!0)});else{var W=0;if(h&&(W=z(),W>=h)){alert("over");return}var N=y[J];N&&(delete y[J],restore_form_values(D,N),D.show("slow",function(){g(x).text(f)}),h&&W+1===h&&r(!1))}});d.hasClass("sortable")&&(a=d.sortable({items:".EntryFormItemBox"}),a.bind("sortstart",function(x,D){D.helper.bgiframe();d.addClass("sorting")}),a.bind("sortstop",function(){var x=a.sortable("toArray");
q=g.map(x,function(D){D=D.split("_");return D[D.length-2]});p.value=q.join(",");setTimeout(function(){d.removeClass("sorting");v()},1)}));cache_register_onbeforeunload(function(x){x[c.id+"_ids"]=q;x[c.id+"_deletes"]=y;a&&(x[c.id+"_order"]=a.sortable("toArray"))});cache_register_onbeforerestorevalues(function(x){x[c.id+"_ids"]&&g.each(x[c.id+"_ids"],function(D,J){0===J.indexOf("NEW")&&A(!0)});x[c.id+"_deletes"]&&(y=x[c.id+"_deletes"],d.find(".EntryFormItemContent").each(function(){var D=this.id.split("_");
D=D[D.length-2];y[D]&&g(this).hide().parent().find("button.EntryFormItemDelete").text(m)}));h&&z()>=h&&r(!1);a&&(x=x[c.id+"_order"])&&(x.reverse(),g.each(x,function(D,J){g("#"+J).detach().prependTo(d)}),a.sortable("refresh"),v())})}})};window.init_schedule=function(b){var f=function(){var m=b(this).parents(".EntryFormItemBox"),g=this.value;"0"===g?m.find(".recurs-ui").hide():"1"==g?(m.find(".repeat-every-ui, .repeats-on-ui, .recurs-week-label").show(),m.find(".repeat-week-of-month-ui, .repeat-day-of-month-ui, .recurs-month-label").hide()):
"2"==g?(m.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-month-label").show(),m.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-week-label").hide()):(m.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-month-label").show(),m.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-week-label").hide())};b("#ScheduleEditArea").on("change",".recur-type-selector",f).on("click",".schedule-ui-accept-feedback",function(m){var g=b(this);m=g.parents(".EntryFormItemBox");g=g.data("schedule");
var a=m.find(".recur-type-selector");restore_form_values(m,g);f.call(a[0]);return!1}).find(".recur-type-selector").each(f)}})();
(function(){window.changeAutoAssign=function(d,l,p){d.checked?(l.value="",l.disabled=!0,p.disabled=!0):(l.disabled=!1,p.disabled=!1)};var b=jQuery,f=function(d){d.find(":input").not('input[type="hidden"]').filter("[name]").addClass("require-group")},m=function(d){d.find(":input").not('input[type="hidden"]').filter("[name]").removeClass("require-group")};window.make_required_group=f;window.remove_required_group=m;var g=function(d,l){var p=d.parent().find("td[data-field-display-name]"),h=p.data("fieldDisplayName");
l?(d.text(h).append(' <span class="Alert">*</span>'),f(p)):(d.text(h),m(p))},a=function(){var d=b("#FIELD_ORG_LEVEL_2");if(d.length){var l=b("#RECORD_TYPE").find(":selected").data("progorbranch");g(d,l)}};window.update_form_make_required_org_level=function(){for(var d,l=0;6>l;l++)if(d=b("#FIELD_ORG_LEVEL_"+l),d.length){g(d,!0);break}};var c=function(d){d=b(d);var l=d.prop("lang")||d.parents("[lang]").first().prop("lang")||"en-CA",p=function(E){var x=!1;try{E.each(function(){if(this.name)if(/radio|checkbox/i.test(this.type)){if(this.checked&&
...
(function(){var b=jQuery;window.configure_entry_form_button=function(){b("#SUBMIT_BUTTON").click(function(f){f=b("#SUBMIT_BUTTON");f.prop("disabled")||(f.prop("disabled",!0),setTimeout(function(){b("#EntryForm").submit()},1))});b(document).bind("keydown",function(f){if(f.ctrlKey&&"s"===String.fromCharCode(f.which).toLowerCase())return setTimeout(function(){b("#SUBMIT_BUTTON").click()},1),f.preventDefault(),f.stopPropagation(),!1});b("#SUBMIT_BUTTON").prop("disabled",!1)};window.configure_feedback_submit_button=
function(){var f=b("#SUBMIT_BUTTON").prop("value");b("#SUBMIT_BUTTON").replaceWith(b('<input type="button" class="btn btn-default" id="SUBMIT_BUTTON">').prop("value",f));b("#EntryForm").submit(function(){var m=validateForm();!1===m&&b("#SUBMIT_BUTTON").prop("disabled",!1);return m})}})();
(function(){window.init_entryform_notes=function(b,f,m){var g=jQuery;b.each(function(){var a=this,c=g(a),d=c.parent(),l=1,p=c.find(".EntryFormNotesUpdateIds")[0];if(p){var h=c.find(".EntryFormNotesDeleteIds")[0],q=c.find(".EntryFormNotesCancelIds")[0],y=c.find(".EntryFormNotesRestoreIds")[0],v=[],r=[],z=[],A={},E=[],x=function(F,I){v.push(F.id);p.value=v.join(",");var e=c.find(".EntryFormItemBox").length+1;template=c.data("addTmpl");new_item=g(template.replace(/\[COUNT\]/g,e).replace(/\[ID\]/g,F.id).replace(/\[MODIFIED_DATE\]/g,
...
c.append(new_item)};d.find(".EntryFormItemAdd").click(function(){var F="NEW"+l++;x({id:F},!0)});var D=function(F){var I=F.id.split("_");F=g(F);var e=F.parents(".EntryFormNotesItem");e.find(".EntryFormItemContent").css("text-decoration","line-through");r.push(I[I.length-2]);h.value=r.join(",");e.data("actionToRestore","delete");F.parent().hide().prev().show()},J=function(F,I){var e=F.id.split("_"),n=g(F),w=n.parents(".EntryFormNotesItem"),k=w.find(".EntryFormItemContent"),u=F.id.lastIndexOf("_");F=
F.id.substring(0,u)+"_CANCEL_REASON";e=e[e.length-2];k.prepend(I.data("cancelTemplate"));z.push(e);q.value=z.join(",");A[e]=I.data("cancelType");c.append(g("<input>").prop({type:"hidden",id:F,name:F,value:I.data("cancelType")}));w.data("actionToRestore","cancel");n.parent().hide().prev().show()},W={update:function(F){F=g(F).parents(".EntryFormNotesItem");var I=F.find(".EntryFormItemContent");x(I.data("formValues"));F.hide()},"delete":D,cancel:J},N=d.find(".EntryFormItemActionMenu").menu({select:function(F,
I){g(this).hide();W[I.item.data("action")](N.data("activeButton"),I.item)}}).hide().css({position:"absolute",zIndex:1});d.on("click",".EntryFormItemAction",function(F){N.data("activeButton",this);if(N.is(":visible"))return N.hide(),!1;N.menu("blur").show();N.position({my:"right top",at:"right bottom",of:this});g(document).one("click",function(){N.hide()});return!1});d.find(".EntryFormItemViewHidden").click(function(F){F=g(this);var I=F.next();I.is(":visible")?(F.text(f),I.hide()):(F.text(m),I.show())});
var X={"delete":function(F,I,e,n){n.css("text-decoration","none");r=g.grep(r,function(w){return w!==F});h.value=r.join(",");I.parent().hide().next().show()},cancel:function(F,I,e,n){e=I[0];var w=e.id.lastIndexOf("_");e=e.id.substring(0,w)+"_CANCEL_REASON";n.find(".EntryFormItemCancelledDetails").remove();z=g.grep(z,function(k){return k!==F});q.value=z.join(",");delete A[F];g("#"+e).remove();I.parent().hide().next().show()}};d.on("click",".EntryFormItemRestoreAction",function(F){F=this.id.split("_");
var I=g(this),e=I.parents(".EntryFormNotesItem"),n=e.find(".EntryFormItemContent");X[e.data("actionToRestore")](F[F.length-2],I,e,n)});var Z=function(F){F=this.id.split("_");var I=g(this);I.parents(".EntryFormNotesItem").find(".EntryFormItemCancelledDetails").css("text-decoration","line-through");I.parent().hide().prev().show();E.push(F[F.length-2]);y.value=E.join(",")};d.on("click",".EntryFormItemRestoreCancel",Z);d.on("click",".EntryFormItemDontRestoreCancel",function(F){var I=this.id.split("_");
F=g(this);F.parents(".EntryFormNotesItem").find(".EntryFormItemCancelledDetails").css("text-decoration","none");F.parent().hide().next().show();I=I[I.length-2];E=g.grep(E,function(e){return e!==I});y.value=E.join(",")});cache_register_onbeforeunload(function(F){F[a.id+"_updates"]=v;F[a.id+"_deletes"]=r;F[a.id+"_restores"]=E;F[a.id+"_cancels"]=z;F[a.id+"_cancel_reason"]=A});cache_register_onbeforerestorevalues(function(F){var I=a.id.substring(0,a.id.lastIndexOf("_"))+"_";F[a.id+"_updates"]&&g.each(F[a.id+
"_updates"],function(n,w){0===w.indexOf("NEW")?(l++,x({id:w},!0)):(n=g("#"+I+w+"_DISPLAY"),x(n.data("formValues")),g("#"+I+w+"_CONTAINER").hide())});F[a.id+"_deletes"]&&g.each(F[a.id+"_deletes"],function(n,w){D(document.getElementById(I+w+"_ACTION"))});F[a.id+"_restores"]&&(g.each(F[a.id+"_restores"],function(n,w){Z.call(document.getElementById(I+w+"_RESTORECANCEL"))}),d.find(".EntryFormItemViewHidden").click());if(F[a.id+"_cancels"]){var e=F[a.id+"_cancel_reason"];g.each(F[a.id+"_cancels"],function(n,
w){J(document.getElementById(I+w+"_ACTION"),N.find("li[data-cancel-type='"+e[w]+"']"))})}})}})}})();
(function(){function b(f,m,g){var a=m.pop(),c=g.shift(),d=create_caching_source_fn(f,entryform.org_level_complete_url,c,null,default_cache_search_fn(f,c,null,"^"));f(a).autocomplete({focus:function(){return!1},source:function(l,p){var h;for(h=0;h<m.length;h++){if(!m[h].value||c[m[h].name]&&c[m[h].name].value&&m[h].value!==c[m[h].name]){delete c.term;delete c.content;for(var q=0;q<m.length;q++)delete c[m[h].name];if(!m[h].value){p([]);return}}l[m[h].name]=m[h].value}d(l,function(y){for(h=0;h<m.length;h++)c[m[h].name]=
m[h].value;p(y)})},minLength:1===a.id.slice(-1)?5:3}).keypress(function(l){13===l.keyCode&&(l.preventDefault(),f(a).autocomplete("close"))})}window.init_orglevels=function(f){var m=f.makeArray(f("#ORG_LEVEL_1, #ORG_LEVEL_2, #ORG_LEVEL_3, #ORG_LEVEL_4")),g=[],a;for(a=0;a<m.length;a++)g.push({});for(a=0;a<m.length;a++){if(m[a].name!=="ORG_LEVEL_"+(a+1))return;b(f,f.makeArray(m).slice(0,a+1),f.makeArray(g).slice(a))}m=f.makeArray(f("#ORG_LEVEL_1, #ORG_LEVEL_2, #ORG_LEVEL_3, #ORG_LEVEL_4, #ORG_LEVEL_5, #ORG_NUM, #NUM, #SERVICE_NAME_LEVEL_1"));
g=[{}];m.length&&"SERVICE_NAME_LEVEL_1"===m[m.length-1].name&&b(f,m,g)};window.init_areas_served=function(f,m){var g=basic_chk_add_html(f,"CM");init_autocomplete_checklist(f,{field:"CM",source:entryform.community_complete_url,minLength:3,txt_not_found:m,add_new_html:function(a,c,d){return g(a,d)}})};window.init_inarea_schools=function(f,m){init_autocomplete_checklist(f,{field:"INAREA_SCH",source:entryform.sch_source,txt_not_found:m})};window.init_escort_schools=function(f,m){init_autocomplete_checklist(f,
{field:"ESCORT_SCH",source:entryform.sch_source,txt_not_found:m})};window.init_distribution=function(f,m){init_autocomplete_checklist(f,{field:"DST",source:entryform.dst_complete_url,add_new_html:only_items_chk_add_html(f,"DST"),match_prop:"label",txt_not_found:m})};window.init_busroutes=function(f){var m=[],g=function(c,d){var l=f("#BR_ID_"+c);l.length?l[0].checked=!0:(m.push({chkid:c,display:d}),f("#BR_existing_add_table").append(f("<tr>").append(f("<td>").append(f("<input>").prop({id:"BR_ID_"+
c,type:"checkbox",checked:!0,defaultChecked:!0,name:"BR_ID",value:c})).append(document.createTextNode(" "+d)))))},a=f("#NEW_BR");f("#add_BR").click(function(){var c=a[0].value,d=a.children("[value="+c+"]").text();g(c,d)});create_checklist_onbefore_fns(f,"BR",m,g)};window.init_subjects=function(f,m){init_autocomplete_checklist(f,{field:"Subj",source:entryform.subj_complete_url,add_new_html:only_items_chk_add_html(f,"Subj",function(){f("#Subj_existing_add_title").removeClass("NotVisible")}),txt_not_found:m})};
window.init_fees=function(f){var m=f("#FEE_ASSISTANCE_FOR, #FEE_ASSISTANCE_FROM"),g=function(){m.prop("disabled",!f(this).prop("checked"))};f("#FEE_ASSISTANCE_AVAILABLE").click(g).each(g)};window.init_org_num=function(f){f("#FIELD_ORG_NUM").next().on("click",".suggested-num",function(){f("#ORG_NUM").val(this.value).trigger("change");return!1})};window.init_locations_services=function(f,m){var g=function(a,c,d){var l='<li><label><input type="checkbox" name="[FIELD]" value="[VALUE]" id="[FIELD]_[VALUE]" checked> [VALUE]</label></li>'.replace(/\[FIELD\]/g,
c),p=a.siblings(".locations-services-new-container"),h=p.find(".locations-services-new"),q=p.find(".locations-services-add"),y=a.siblings(".locations-services-list-suggestions"),v=f(d);d=function(x){x[c+"_STATE"]=a.html()};var r=function(x){(x=x[c+"_STATE"])&&a.html(x)},z=function(x){var D=f("#"+c+"_"+x);D.length?D[0].checked=!0:(x=l.replace(/\[VALUE\]/g,x),a.append(f(x)))};a.length&&(cache_register_onbeforeunload(d),cache_register_onbeforerestorevalues(r));p.on("click",".locations-services-add",
function(){var x=f.trim(h[0].value);if(x){if(!/^[A-Z]{3}[0-9]{4,5}$/.test(x))return a.siblings(".locations-services-error").length||f("<p>").hide().addClass("Alert").addClass("locations-services-error").append(document.createTextNode(m)).insertBefore(p).show("slow"),!1;z(x);a.siblings(".locations-services-error").hide("slow",function(){f(this).remove()});h[0].value=""}return!1}).on("keypress",".locations-services-new",function(x){13===x.keyCode&&(x.preventDefault(),q.trigger("click"))});y.on("click",
".locations-services-suggestion",function(x){x.preventDefault();z(this.value);return!1});if(v.length){var A=a.parents("tr").first(),E=function(){var x=f(this);if(this.checked){if(A.show(),x.data("maybeRequired"))if(x.is(".ols-type-TOPIC,.ols-type-SERVICE")){var D=x.is(".ols-type-TOPIC")?f(".ols-type-SERVICE"):f(".ols-type-TOPIC");D.prop("checked")?(D.prop("disabled")&&(D.prop("disabled",!1),f("#OLS_ID_"+D.prop("value")+"_forceon").remove()),this.disabled&&(this.disabled=!1,f("#OLS_ID_"+this.value+
"_forceon").remove())):this.disabled||(this.disabled=!0,x.parent().after(f('<input type="hidden" name="OLS_ID" id="OLS_ID_'+this.value+'_forceon" value="'+this.value+'">')))}else this.disabled||(this.disabled=!0,x.parent().after(f('<input type="hidden" name="OLS_ID" id="OLS_ID_'+this.value+'_forceon" value="'+this.value+'">')))}else x.is(".ols-type-SITE")?A.hide():f(x.is(".ols-type-SERVICE")?".ols-type-TOPIC":".ols-type-SERVICE").prop("checked")||A.hide(),x.data("maybeRequired")&&x.is(".ols-type-TOPIC,.ols-type-SERVICE")&&
(this.disabled=!1,D=x.is(".ols-type-TOPIC")?f(".ols-type-SERVICE"):f(".ols-type-TOPIC"),D.prop("checked")?(x=D.prop("value"),D.prop("disabled")||(D.prop("disabled","true"),D.parent().after(f('<input type="hidden" name="OLS_ID" id="OLS_ID_'+x+'_forceon" value="'+x+'">')))):f("#ols-type-SERVICE-warning").show())};v.click(E);setTimeout(function(){v.each(function(x,D){E.call(D)})},1)}};f(".locations-services-list").each(function(){var a=f(this),c=a.data("fieldName"),d=a.data("needsOlsType");g(a,c,d)})};
Array.prototype.indexOf||(Array.prototype.indexOf=function(f,m){if(void 0===this||null===this)throw new TypeError('"this" is null or not defined');var g=this.length>>>0;m=+m||0;Infinity===Math.abs(m)&&(m=0);0>m&&(m+=g,0>m&&(m=0));for(;m<g;m++)if(this[m]===f)return m;return-1});window.init_name_editable_toggle=function(f){var m=$("#ORG_NUM"),g=$("#DISPLAY_ORG_NAME"),a=function(){if(m.length&&!m.val()||!m.length&&!f.org_num)return g.prop("disabled",!0),!1;g.prop("disabled",!1);return g.length&&g.prop("checked")||
!g.length&&f.display_org_name},c=function(p){return function(h){h=h.not("#DISPLAY_ORG_NAME");var q=h.filter(":checked");h.length||(q=$.map(p,function(y){return f[y]?y:null}));if(!q.length)return!0;if(h.length){if(q.is(":not(.ols-type-TOPIC)"))return!1}else if($.map(q,function(y){return"TOPIC"===y?null:y}).length)return!1;return!a()}},d=function(){var p=!0;return function(h,q){p&&(q.each(function(){var y=$(this);y.data("was_required",y.hasClass("require-group"))}),p=!1);q.each(function(){var y=$(this);
...
c(["SERVICE","TOPIC"]),d())}})();jQuery(function(b){var f="AB BC MB NB NL NS NT NU ON PE QC SK YT".split(" ");b(".Province").combobox({source:f});window.cioc_province_source=f});
