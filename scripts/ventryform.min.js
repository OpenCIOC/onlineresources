...
a(document).on("click",".FieldErrorJump",function(p){p=a(p.currentTarget).prop("fieldname");(p=document.getElementById(p))&&p.scrollIntoView(!0)});a(".ValidationErrors.ui-dialog").css({position:"fixed"}).position({my:"right bottom",at:"right bottom",of:window});e(a(h).find("td[data-field-required]"));h=a(h).validate({ignore:"input[type=hidden]",ignoreTitle:!0,errorPlacement:function(p,x){var A=[],B=x[0];if(x.hasClass("require-group")){var G=x.parents("td[data-field-display-name]");A=G.children("div.required-notice");
...
"require-group":"Ce champ est requis.",unique:"Une valeur unique est requise."}):jQuery.extend(jQuery.validator.messages,{protourl:"Please enter a valid URL.",posint:"Please enter a positive number.",posdbl:"Please enter a positive number.","record-num":"Please enter a valid record number.","require-group":"Field Required.",unique:"A unique value is required."});return h};window.init_record_type_form=function(h,n,r){var k=null,g=null,q={},w="ORG_LEVEL_1 ORG_LEVEL_2 ORG_LEVEL_3 ORG_LEVEL_4 ORG_LEVEL_5 LOCATION_NAME SERVICE_NAME_LEVEL_1 SERVICE_NAME_LEVEL_2".split(" "),
l=function(){a("#RecordTypeName").html(g);a.each(w,function(m){var u=r[m],y=a("#"+m);y.length&&(y[0].value=u)&&(a("#"+m+"_DISPLAY").text(u),a("#HIDDEN_"+y).prop("value",u))});a("#RECORD_TYPE").prop("value",k[0].value);d()},f=function(){var m=k[0].value;k.data("hasform")||(m=null);k=a(this).find("option:selected");var u=k[0].value;k.data("hasform")||(u=null);u!==m?(q[m]=a("#copyFieldsForm").contents().detach(),q[u]?(a("#copyFieldsForm").append(q[u]),a("#RECORD_TYPE").prop("value",k[0].value)):a("#copyFieldsForm").load(n+
" #copyFieldsInner",{NUM:h,RT:u||k[0].value},l)):d()};jQuery(function(){g=a("#RecordTypeName").html();var m=a("#RECORD_TYPE");a(document).on("change","#RECORD_TYPE",f);k=m.find("option:selected");d()})};window.init_validate_duplicate_org_names=function(h){var n=a.extend({selector:"#EntryForm",fields:"#ORG_LEVEL_1,#ORG_LEVEL_2,#ORG_LEVEL_3,#ORG_LEVEL_4,#ORG_LEVEL_5,#LOCATION_NAME,#SERVICE_NAME_LEVEL_1,#SERVICE_NAME_LEVEL_2,#OLS_SELECT input,#ORG_NUM",only_warn:!0,num:null,org_levels:[],url:null},h),
r=[],k=null!==n.num,g=[],q=a(n.selector).submit(function(w){var l,f=!1,m=a(n.fields).serializeArray();n.num&&m.push({name:"NUM",value:n.num});if(m.length!==r.length)f=!0;else for(l=0;l<m.length;l++){var u=m[l];if(u.name!==r[l].name||u.value!==r[l].value){f=!0;break}}if(!f&&k)return!0;var y=function(){n.only_warn&&confirm(n.confirm_string)?(k=!0,a("#duplicate_name_error_box").hide(),q.submit()):(a("#SUBMIT_BUTTON").prop("disabled",!1),a("#duplicate_name_error_box").show())};w.stopPropagation();w.preventDefault();
if(!f)return y(),!1;a.ajax({url:n.url,dataType:"json",method:"get",data:m,success:function(v){k=!0;g=[];for(var p=0;p<v.length;p++)k&=0===v[p].count,0!==v[p].count&&g.push(v[p].type);r=m;k?(a("#duplicate_name_error_box").hide(),q.submit()):y()}});return!1})}})();
