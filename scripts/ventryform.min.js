...
"2"==c?(b.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-month-label").show(),b.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-week-label").hide()):(b.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-month-label").show(),b.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-week-label").hide())};a("#ScheduleEditArea").on("change",".recur-type-selector",b).on("click",".schedule-ui-accept-feedback",function(b){var c=a(this);b=c.parents(".EntryFormItemBox");c=c.data("schedule");
restore_form_values(b,c);return!1}).find(".recur-type-selector").each(b)}})();
