...
"u");return d=d.replace(RegExp("[\u00fd\u00ff]","g"),"y")};window.string_ci_ai=b;window.slugify=function(d){d=b(d);d=d.replace(/-/g," ").replace(/[^a-z0-9\s]/g,"");d=d.trim();return d.replace(/\s+/g,"-")};var a=function(d,e,h,q,v){h=h||"value";q=q||"";v=v||"";return function(r,m){if(e.term&&(new RegExp(q+d.ui.autocomplete.escapeRegex(e.term)+v,"i")).test(r.term)&&e.content&&13>e.content.length){var y=new RegExp(q+d.ui.autocomplete.escapeRegex(r.term)+v,"i");m(d.grep(e.content,function(E){return y.test(b(E[h]))}));
return!0}return!1}};window.default_cache_search_fn=a;var c=function(d,e,h,q,v){h=h||{};v=v||a(d,h,q||"value");return function(r,m,y){r.term=b(r.term);h.term===r.term&&h.content?m(h.content):v(r,m)||d.ajax({url:y||e,dataType:"json",data:r,cache:!1,success:function(E){h.term=r.term;h.content=E;m(E)}})}};window.create_caching_source_fn=c;window.init_community_autocomplete=function(d,e,h,q,v){var r={};v&&=d(v);v&&!v.length&&(v=null);var m=d("#"+e).autocomplete({focus:function(y,E){return!1},select:function(y,
E){m.data({chkid:E.item.chkid,display:E.item.value})},source:c(d,h,r),minLength:q}).keypress(function(y){"13"==y.keyCode&&(y.preventDefault(),m.autocomplete("close"))});v&&(m.parents("form").on("submit",function(){var y=m.val();if(m.data("display")===y)v.val(m.data("chkid"));else{if(r.content){var E=b(y);y=d.grep(r.content,function(t){return b(t.value)===E});if(1===y.length){v.val(y[0].chkid);m.data({chkid:y[0].chkid,display:y[0].value}).val(y[0].value);return}}v.val("")}}),cache_register_onbeforerestorevalues(function(y){m.length&&
(y=y["autocomplete_data_"+m[0].id])&&m.data(y)}),cache_register_onbeforeunload(function(y){if(m.length){var E=m.data();E.display&&E.chkid&&(y["autocomplete_data_"+m[0].id]={display:E.display,chkid:E.chkid})}}))}})();
...
c,name:"CM_NUM_NEEDED_"+c,size:3,maxlength:3})).append(b("<span>").addClass("input-group-addon").text(h))))))};window.init_num_needed=function(c){init_autocomplete_checklist(b,{field:"CM",source:entryform.community_complete_url,add_new_html:a,minLength:3,txt_not_found:c})}})();
(function(){var b=jQuery;window.init_interests=function(a,c){var d=[],e=only_items_chk_add_html(b,"AI");init_autocomplete_checklist(b,{field:"AI",source:c,add_new_html:e,added_values:d,txt_not_found:a});var h=function(v){var r=b("#AreaOfInterestList");r.length&&r.prop("id","AreaOfInterestListOld");b("<ul>").hide().insertAfter(q).prop("id","AreaOfInterestList").append(b(b.map(v,function(m,y){return b("<li>").append(b("<label>").append(b("<input>").prop({type:"checkbox",value:m.chkid}).data("cioc_chk_display",
m.value)).append(document.createTextNode(" "+m.value)))[0]}))).show("slow");r.length&&r.hide("slow",function(){r.remove()})};var q=b("#InterestGroup").change(function(){b.getJSON(c,{IGID:q.prop("value")},h)});b("#FIELD_INTERESTS").next().on("click","#AreaOfInterestList input:checkbox",{added_values:d,add_item_fn:e},function(v){var r=b(this),m=document.getElementById("AI_ID_"+this.value);m?m.checked=!0:(m=r.data("cioc_chk_display"),v.data.added_values.push({chkid:this.value,display:m}),v.data.add_item_fn(this.value,
m));r.parent().parent().hide("slow",function(){r.remove()})})}})();
