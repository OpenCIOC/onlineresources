...
d))).append(a("<td>").append(a("<input>").prop({id:b+"_NOTES_"+c,name:b+"_NOTES_"+c,size:entryform.chk_notes_size,maxlength:entryform.chk_notes_maxlen})));a("#"+b+"_existing_add_table").append(k);return k.find('input[type="checkbox"]')[0]}};var a={},d=function(e){$("#NEW_"+e).autocomplete(a[e])};window.init_autocomplete_checklist=function(e,b){var c=b.field;if(!a[c]){var f=b.source;b.minLength=b.minLength||1;b.delay=b.delay||300;b.add_new_html=b.add_new_html||basic_chk_add_html(e,c);b.match_prop=
b.match_prop||"value";b.txt_not_found=b.txt_not_found||"Not Found";var k=e(b.delegate_root||document),h=b.added_values||[],g=function(a,e,d){var f=document.getElementById(c+"_ID_"+a);f?(f.checked=!0,b.after_add_new_value&&b.after_add_new_value.call(f)):(h.push({chkid:a,display:e,label:d}),a=b.add_new_html(a,e,d),b.after_add_new_value&&b.after_add_new_value.call(a))};create_checklist_onbefore_fns(e,c,h,g);var l={},m=function(a){e("#NEW_"+c).data({chkid:"",display:"",label:""}).prop("value","").focus();
...
"2"==b?(d.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-month-label").show(),d.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-week-label").hide()):(d.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-month-label").show(),d.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-week-label").hide())};a("#ScheduleEditArea").on("change",".recur-type-selector",d).on("click",".schedule-ui-accept-feedback",function(d){var b=a(this);d=b.parents(".EntryFormItemBox");b=b.data("schedule");
restore_form_values(d,b);return!1}).find(".recur-type-selector").each(d)}})();
