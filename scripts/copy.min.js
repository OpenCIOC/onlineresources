...
posdbl:"Veuillez entrer un nombre positif.","record-num":"Please enter a valid record number.","require-group":"Ce champ est requis.",unique:"Une valeur unique est requise."}):jQuery.extend(jQuery.validator.messages,{protourl:"Please enter a valid URL.",posint:"Please enter a positive number.",posdbl:"Please enter a positive number.","record-num":"Please enter a valid record number.","require-group":"Field Required.",unique:"A unique value is required."});return f};window.init_record_type_form=function(f,
k,l){var n=null,h=null,t={},w="ORG_LEVEL_1 ORG_LEVEL_2 ORG_LEVEL_3 ORG_LEVEL_4 ORG_LEVEL_5 LOCATION_NAME SERVICE_NAME_LEVEL_1 SERVICE_NAME_LEVEL_2".split(" "),p=function(){b("#RecordTypeName").html(h);b.each(w,function(m){var q=l[m],x=b("#"+m);x.length&&(x[0].value=q)&&(b("#"+m+"_DISPLAY").text(q),b("#HIDDEN_"+x).prop("value",q))});b("#RECORD_TYPE").prop("value",n[0].value);e()},g=function(){var m=n[0].value;n.data("hasform")||(m=null);n=b(this).find("option:selected");var q=n[0].value;n.data("hasform")||
(q=null);q!==m?(t[m]=b("#copyFieldsForm").contents().detach(),t[q]?(b("#copyFieldsForm").append(t[q]),b("#RECORD_TYPE").prop("value",n[0].value)):b("#copyFieldsForm").load(k+" #copyFieldsInner",{NUM:f,RT:q||n[0].value},p)):e()};jQuery(function(){h=b("#RecordTypeName").html();var m=b("#RECORD_TYPE");b(document).on("change","#RECORD_TYPE",g);n=m.find("option:selected");e()})};window.init_validate_duplicate_org_names=function(f){var k=b.extend({selector:"#EntryForm",fields:"#ORG_LEVEL_1,#ORG_LEVEL_2,#ORG_LEVEL_3,#ORG_LEVEL_4,#ORG_LEVEL_5,#LOCATION_NAME,#SERVICE_NAME_LEVEL_1,#SERVICE_NAME_LEVEL_2,#OLS_SELECT input,#ORG_NUM",
only_warn:!0,num:null,org_levels:[],url:null},f),l=[],n=null!==k.num,h=[],t=b(k.selector).submit(function(w){var p,g=!1,m=b(k.fields).serializeArray();k.num&&m.push({name:"NUM",value:k.num});if(m.length!==l.length)g=!0;else for(p=0;p<m.length;p++){var q=m[p];if(q.name!==l[p].name||q.value!==l[p].value){g=!0;break}}if(!g&&n)return!0;var x=function(){k.only_warn&&confirm(k.confirm_string)?(n=!0,b("#duplicate_name_error_box").hide(),t.submit()):(b("#SUBMIT_BUTTON").prop("disabled",!1),b("#duplicate_name_error_box").show())};
w.stopPropagation();w.preventDefault();if(!g)return x(),!1;b.ajax({url:k.url,dataType:"json",method:"get",data:m,success:function(B){n=!0;h=[];for(var r=0;r<B.length;r++)n&=0===B[r].count,0!==B[r].count&&h.push(B[r].type);l=m;n?(b("#duplicate_name_error_box").hide(),t.submit()):x()}});return!1})}})();
