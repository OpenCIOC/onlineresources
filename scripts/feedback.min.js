...
document.getElementById("GEOCODE_TYPE_INTERSECTION").checked&&(d=L(),c=pageconstants.txt_geocode_intersection_change);c&&d!=l&&!confirm(c)&&(b.scrollIntoView(!0),document.documentElement.scrollTop-=10,a.preventDefault(),a.stopImmediatePropagation(),$("#SUBMIT_BUTTON").prop("disabled",!1))})}})();
...
"2"==c?(a.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-month-label").show(),a.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-week-label").hide()):(a.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-month-label").show(),a.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-week-label").hide())};b("#ScheduleEditArea").on("change",".recur-type-selector",c).on("click",".schedule-ui-accept-feedback",function(a){var c=b(this);a=c.parents(".EntryFormItemBox");c=c.data("schedule");
restore_form_values(a,c);return!1}).find(".recur-type-selector").each(c)}})();
