...
[];onbeforerestorevalues_fns=[];var l=function(){var c={form_values:get_form_values(h)};k.each(onbeforeunload_fns,function(d,v){v(c)});document.getElementById("cache_form_values").value=JSON.stringify(c)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&l()});document.addEventListener("freeze",l);window.addEventListener("pagehide",l);window.cache_register_onbeforeunload=function(c){onbeforeunload_fns.push(c)};window.cache_register_onbeforerestorevalues=function(c){onbeforerestorevalues_fns.push(c)};
window.restore_cached_state=function(){k(window).on("pageshow",function(c){if(!c.originalEvent.persisted&&(c=document.getElementById("cache_form_values"))&&c.value){var d=JSON.parse(c.value);k.each(onbeforerestorevalues_fns,function(v,x){x(d)});restore_form_values(h,d.form_values)}})}}})();
