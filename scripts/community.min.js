...
(function(){var b=jQuery,e={},d=function(f){null!==f&&"undefined"!==typeof f?b("#myListCount").text(f):(f=b("#myListCount").text())&&(f=parseInt(f,10));f?e.myListLink.show():e.myListLink.hide()},a=function(f){b.each(f,function(p,t){b("#added_to_list_"+t).show();b("#add_to_list_"+t).hide()})},c=function(f){return function(p){p.fail?alert(p.errinfo):(p.ids?d(b.grep(p.ids,g[e.domain]).length):p.count?d(p.count):d(0),a([f]))}},l=function(f){f=b(f.currentTarget).data("id");b.ajax({success:c(f),dataType:"json",
error:function(){alert("Error")},data:{id:f},type:"POST",url:e.current_list_url})},q=function(f){return function(p){if(p.fail)alert(p.errinfo);else{p=!0;if(f){var t=b("#results_table #remove_from_list_"+f);t.length?t.parent().parent().remove():(p=!1,t=b("#results_container #remove_from_list_"+f),t.parents(".dlist-result").remove())}(p?1!==b("#results_table tr").length:0!==b("#results_container .dlist-result").length)&&f||(b("#records_ui").hide(),b("#no_records_message").show())}}},n=function(f){f=
b(f.currentTarget).data("id");b.ajax({success:q(f),dataType:"json",error:function(){alert("Error")},data:{ID:f,RemoveItem:e.domain},type:"POST",url:e.current_list_url})},k=function(f){b.ajax({success:q(),dataType:"json",error:function(){alert("Error")},data:{ID:"all",RemoveItem:e.domain},type:"POST",url:e.current_list_url})},g={CIC:function(f){return/^[A-Za-z]{3}[0-9]{4,5}$/.test(f)},VOL:function(f){return/^[1-9][0-9]*$/.test(f)}},h=function(f){if(f.fail)m();else if(f.inrequest){var p=b.grep(f.ids,
g[e.domain]);a(p);d(p.length);(f=f.previous_ids)&&b.each(f,function(t,w){b("#ct_added_to_previous_request_"+w).show()});b(".ListUI img").prop("src",function(t,w){return w.replace("list","referral")});b("#list_header_text").hide();b("#ct_header_text").show();e.current_list_url=e.ct_update_url;r()}else m()},m=function(){var f=function(){e.list_view_mode||a(e.already_added||[]);e.current_list_url=e.list_view_mode&&"ct"===e.list_view_mode?e.ct_update_url:e.list_update_url;d((e.already_added||[]).length);
r()};e.list_view_mode||e.has_session?f():b.ajax({dataType:"json",error:null,url:e.list_update_url,data:{SessionTest:"on"},type:"POST",success:function(p){p.has_session&&f()}})},r=function(){b(".HideListUI").delegate(".add_to_list","click",l).delegate(".remove_from_list","click",n).removeClass("HideListUI");b("#remove_all_from_list").click(k);b.browser.msie&&"8.0">b.browser.msie&&b(".ListUI").removeClass("FixIE")};window.init_list_adder=function(f){e=f;e.myListLink=b("#myListLink").parent(":hidden").add("#myListLink");
...
case "object":if(!r)return"null";l+=q;var f=[];if("[object Array]"===Object.prototype.toString.apply(r)){var p=r.length;for(g=0;g<p;g+=1)f[g]=d(g,r)||"null";h=0===f.length?"[]":l?"[\n"+l+f.join(",\n"+l)+"\n"+m+"]":"["+f.join(",")+"]";l=m;return h}if(k&&"object"===typeof k)for(p=k.length,g=0;g<p;g+=1){var t=k[g];"string"===typeof t&&(h=d(t,r))&&f.push(e(t)+(l?": ":":")+h)}else for(t in r)Object.hasOwnProperty.call(r,t)&&(h=d(t,r))&&f.push(e(t)+(l?": ":":")+h);h=0===f.length?"{}":l?"{\n"+l+f.join(",\n"+
...
c=/[\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,l,q,n={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},k;"function"!==typeof JSON.stringify&&(JSON.stringify=function(g,h,m){var r;q=l="";if("number"===typeof m)for(r=0;r<m;r+=1)q+=" ";else"string"===typeof m&&(q=m);if((k=h)&&"function"!==typeof h&&("object"!==typeof h||"number"!==typeof h.length))throw Error("JSON.stringify");return d("",{"":g})});
"function"!==typeof JSON.parse&&(JSON.parse=function(g,h){function m(r,f){var p,t=r[f];if(t&&"object"===typeof t)for(p in t)if(Object.hasOwnProperty.call(t,p)){var w=m(t,p);void 0!==w?t[p]=w:delete t[p]}return h.call(r,f,t)}g=String(g);a.lastIndex=0;a.test(g)&&(g=g.replace(a,function(r){return"\\u"+("0000"+r.charCodeAt(0).toString(16)).slice(-4)}));if(/^[\],:{}\s]*$/.test(g.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
...
[];onbeforerestorevalues_fns=[];var d=function(){var a={form_values:get_form_values(b)};e.each(onbeforeunload_fns,function(c,l){l(a)});document.getElementById("cache_form_values").value=JSON.stringify(a)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&d()});document.addEventListener("freeze",d);window.addEventListener("pagehide",d);window.cache_register_onbeforeunload=function(a){onbeforeunload_fns.push(a)};window.cache_register_onbeforerestorevalues=function(a){onbeforerestorevalues_fns.push(a)};
window.restore_cached_state=function(){e(window).on("pageshow",function(a){if(!a.originalEvent.persisted&&(a=document.getElementById("cache_form_values"))&&a.value){var c=JSON.parse(a.value);e.each(onbeforerestorevalues_fns,function(l,q){q(c)});restore_form_values(b,c.form_values)}})}}})();
...
r){r.dpDiv.addClass("NoYearDatePicker")},onClose:function(m,r){r.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});h.dateFormat=g;n.datepicker(b.extend({},b.datepicker.regional[k],h)).prop("autocomplete","off")},q=function(n){b.getScript("//ajax.googleapis.com/ajax/libs/jqueryui/"+b.ui.version+"/i18n/jquery.ui.datepicker-"+n+".min.js",function(){d[n]=!0;b.each(a[n],function(){l.call(this)});delete a[n]})};b.datepicker.regional["en-CA"]=b.extend({},b.datepicker.regional[""]);b.fn.extend({autodatepicker:function(){return this.each(function(){var n=
b(this);var k=n.parents("[lang]").first().prop("lang")||"en-CA";k=c[k]||k;n.data("culture",k);d[k]?l.call(this):(n=a[k],n||(n=a[k]=[],q(k)),n.push(this))})}});jQuery(function(){b("input.DatePicker").autodatepicker()})})();
...
a.match_prop||"value";a.txt_not_found=a.txt_not_found||"Not Found";var q=d(a.delegate_root||document),n=a.added_values||[],k=function(f,p,t){var w=document.getElementById(c+"_ID_"+f);w?(w.checked=!0,a.after_add_new_value&&a.after_add_new_value.call(w)):(n.push({chkid:f,display:p,label:t}),f=a.add_new_html(f,p,t),a.after_add_new_value&&a.after_add_new_value.call(f))};create_checklist_onbefore_fns(d,c,n,k);var g={},h=function(f){d("#NEW_"+c).data({chkid:"",display:"",label:""}).prop("value","").focus();
d("#"+c+"_error").hide("slow",function(){d(this).remove()})},m=null,r=null;(function(f){d.isArray(f)?(r=function(p,t){var w=new RegExp(d.ui.autocomplete.escapeRegex(string_ci_ai(p.term)));t(d.grep(f,function(u){return w.test(string_ci_ai(u[a.match_prop]||u))}))},m=function(p,t){var w=string_ci_ai(p);p=d.grep(f,function(u){return string_ci_ai(string_ci_ai(u[a.match_prop]||u))===w});if(1===p.length)return t(p[0]),!0;t()}):"string"===typeof f?(r=create_caching_source_fn(d,f,g,a.match_prop),m=function(p,
t,w){var u=string_ci_ai(p);if(g.content){var v=d.grep(g.content,function(x){return string_ci_ai(x[a.match_prop])===u});if(1===v.length){t(v[0]);return}}w||string_ci_ai(g.term||"")===u?t():r({term:u},function(x){m(p,t,!0)})}):(r=f,m=a.look_for_fn)})(l);q.on("click","#add_"+c,function(f){f=d("#NEW_"+c);var p=f.data("chkid"),t=f.data("display"),w=f.data("label"),u=f[0].value;if(p&&t&&t==u)return k(p,t,w),h(),!1;m(f[0].value,function(v){v?(k(v.chkid,v.value,v.label),h()):0===d("#"+c+"_error").length&&
(d("#"+c+"_new_input_table").before(d("<p>").hide().addClass("Alert").prop("id",c+"_error").append(document.createTextNode(a.txt_not_found))),d("#"+c+"_error").show("slow"))});return!1});b[c]={focus:function(f,p){return!1},source:r,minLength:a.minLength,delay:a.delay,select:function(f,p){d("#NEW_"+c).data({chkid:p.item.chkid,display:p.item.value,label:p.item.label})}};q.on("keypress","#NEW_"+c,function(f){"13"==f.keyCode&&(f.preventDefault(),d("#NEW_"+c).autocomplete("close"),d("#add_"+c).trigger("click"))})}e(c)};
window.init_languages=function(d,a){var c=null,l=null,q=d("#LN_existing_add_table"),n=function(){var k=d(this).parents(".language-entry").find(".language-details-notes");this.checked?k.slideDown():k.slideUp()};init_autocomplete_checklist(d,{field:"LN",source:entryform.languages_source,txt_not_found:a,after_add_new_value:n,add_new_html:function(k,g){null===c&&(c=d("#add_LN"),l=c.data("newTemplate"));k=l.replace(/LANGNAMELANGNAME/g,g).replace(/IDIDID/g,k);q.append(k)}});q.on("change",".language-primary",
...
a=e("input[id^=NEW_]");a.length&&(e("#EntryForm").submit(function(c){var l=a.map(function(){return this.value?this:null});if(l.length&&!c.isDefaultPrevented()){if(!confirm(b)){c.preventDefault();e("#SUBMIT_BUTTON").prop("disabled",!1);c=e("#unadded_checklist_error_box");var q=e("#unadded_checklist_error_list");var n=c.is(":visible");n&&q.children("ul:first").hide("slow",function(){e(this).remove()});var k=e("<ul>");n&&k.hide();q.append(k);e.each(l.map(d),function(){k.append(this)});n?k.show("slow"):
c.show("slow")}}else c.isDefaultPrevented()&&(c=e("#unadded_checklist_error_box"),q=e("#unadded_checklist_error_list"),c.hide("slow",function(){q.children().remove()}))}),e(document).on("click",".UnaddedChecklistJump",function(){var c=e(this).data("jump_location");(c=document.getElementById(c))&&c.scrollIntoView(!0)}))};window.init_entryform_items=function(b,e,d){var a=jQuery;b.each(function(){var c=null,l=this,q=a(l),n=1,k=q.find(".EntryFormItemContainerIds")[0];if(k){var g=q.data("maxAdd");g&&=
parseInt(g,10);var h=[];k.value&&(h=a.map(k.value.split(","),function(u){return 0===u.indexOf("NEW")&&0!==u.indexOf("NEWFB")?null:u}));var m={},r=function(){q.find(".EntryFormItemBox").each(function(u){a(this).find(".EntryFormItemCount").text(u+1)})},f=function(u){if(g){var v=q.find(".EntryFormItemContent:hidden").parent().find("button.EntryFormItemDelete").prop("disabled",!u);w.prop("disabled",!u);u?(v.removeClass("ui-state-disabled"),w.removeClass("ui-state-disabled")):(v.addClass("ui-state-disabled"),
w.addClass("ui-state-disabled"))}},p=function(){var u=0,v;for(v in m)u++;return h.length-u},t=function(u){var v=0;if(g&&!u&&(v=p(),v>=g)){alert("over");return}var x=q.data("addTmpl");h.push("NEW"+n++);k.value=h.join(",");x=a(x.replace(/\[COUNT\]/g,h.length).replace(/\[ID\]/g,h[h.length-1]));x.find(".DatePicker").autodatepicker();x.find(".Province").combobox({source:window.cioc_province_source});entryform.service_titles&&x.find(".ServiceTitleField").combobox({source:entryform.service_titles});q.append(x);
c&&(c.sortable("refresh"),r());g&&!u&&v+1===g&&f(!1)},w=q.parent().find(".EntryFormItemAdd").click(function(){t(!1)});q.on("click","button.EntryFormItemDelete",function(){var u=this,v=a(this).parents(".EntryFormItemBox").find(".EntryFormItemContent"),x=this.id.split("_");x=x[x.length-2];if(v.is(":visible"))v.hide("slow",function(){m[x]=get_form_values(v);a(u).text(d);v.find("input,select,textarea").each(function(){"input"!==this.nodeName.toLowerCase()||"checkbox"!==this.type&&"radio"!==this.type?
"select"===this.nodeName.toLowerCase()?a(this).find("option").each(function(){this.selected=!1}):this.value="":this.checked=!1});f(!0)});else{var y=0;if(g&&(y=p(),y>=g)){alert("over");return}var z=m[x];z&&(delete m[x],restore_form_values(v,z),v.show("slow",function(){a(u).text(e)}),g&&y+1===g&&f(!1))}});q.hasClass("sortable")&&(c=q.sortable({items:".EntryFormItemBox"}),c.bind("sortstart",function(u,v){v.helper.bgiframe();q.addClass("sorting")}),c.bind("sortstop",function(){var u=c.sortable("toArray");
h=a.map(u,function(v){v=v.split("_");return v[v.length-2]});k.value=h.join(",");setTimeout(function(){q.removeClass("sorting");r()},1)}));cache_register_onbeforeunload(function(u){u[l.id+"_ids"]=h;u[l.id+"_deletes"]=m;c&&(u[l.id+"_order"]=c.sortable("toArray"))});cache_register_onbeforerestorevalues(function(u){u[l.id+"_ids"]&&a.each(u[l.id+"_ids"],function(v,x){0===x.indexOf("NEW")&&t(!0)});u[l.id+"_deletes"]&&(m=u[l.id+"_deletes"],q.find(".EntryFormItemContent").each(function(){var v=this.id.split("_");
v=v[v.length-2];m[v]&&a(this).hide().parent().find("button.EntryFormItemDelete").text(d)}));g&&p()>=g&&f(!1);c&&(u=u[l.id+"_order"])&&(u.reverse(),a.each(u,function(v,x){a("#"+x).detach().prependTo(q)}),c.sortable("refresh"),r())})}})};window.init_schedule=function(b){var e=function(){var d=b(this).parents(".EntryFormItemBox"),a=this.value;"0"===a?d.find(".recurs-ui").hide():"1"==a?(d.find(".repeat-every-ui, .repeats-on-ui, .recurs-week-label").show(),d.find(".repeat-week-of-month-ui, .repeat-day-of-month-ui, .recurs-month-label").hide()):
...
(function(){window.init_cm_checklist=function(b,e,d){var a={},c=create_caching_source_fn(b,e,a,"label"),l=d.parent_cmid_input||null,q=9999,n=b("#cm-checklist-template").html(),k=b("#cm-checklist-target");e=l?function(h,m,r){h.parent=l.val();return c(h,m,r)}:c;var g=function(h,m,r){var f=string_ci_ai(h);if(a.content){var p=b.grep(a.content,function(t){return string_ci_ai(t[d.match_prop])===f});if(1===p.length){m(p[0]);return}}r||string_ci_ai(a.term||"")===f?m():c({term:f},function(t){g(h,m,!0)})};
d.parent_cmid_input&&(l=b(d.parent_cmid_input));d.source=e;d.look_for_fn=g;d.add_new_html=function(h,m){to_add=b(n.replace(/\[COUNT\]/g,q++).replace(/\[ID\]/g,h).replace(/\[LABEL\]/g,b("<div>").text(m).html()));k.is(":hidden")?k.removeClass("hidden").append(to_add):k.append(to_add);return!1};init_autocomplete_checklist(b,d)};window.init_community_edit=function(b){var e=9999,d=null,a=null;b("#add-alternate-name").click(function(c){c.preventDefault();d||(d=b("#alt-name-template").html(),a=b("#alt-name-target"));
