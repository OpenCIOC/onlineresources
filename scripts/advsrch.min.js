...
[];onbeforerestorevalues_fns=[];var p=function(){var f=document.getElementById("cache_form_values");if(f){var d={form_values:get_form_values(c)};k.each(onbeforeunload_fns,function(m,C){C(d)});f.value=JSON.stringify(d)}};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&p()});document.addEventListener("freeze",p);window.addEventListener("pagehide",p);window.cache_register_onbeforeunload=function(f){onbeforeunload_fns.push(f)};window.cache_register_onbeforerestorevalues=
function(f){onbeforerestorevalues_fns.push(f)};window.restore_cached_state=function(){k(window).on("pageshow",function(f){if(!f.originalEvent.persisted&&(f=document.getElementById("cache_form_values"))&&f.value){var d=JSON.parse(f.value);k.each(onbeforerestorevalues_fns,function(m,C){C(d)});restore_form_values(c,d.form_values)}})}}})();
...
z){z.dpDiv.addClass("NoYearDatePicker")},onClose:function(A,z){z.dpDiv.removeClass("NoYearDatePicker")},changeYear:!1});r.dateFormat=l;w.datepicker(c.extend({},c.datepicker.regional[v],r)).prop("autocomplete","off")},C=function(w){c.getScript("https://ajax.googleapis.com/ajax/libs/jqueryui/"+c.ui.version+"/i18n/jquery.ui.datepicker-"+w+".min.js",function(){p[w]=!0;c.each(f[w],function(){m.call(this)});delete f[w]})};c.datepicker.regional["en-CA"]=c.extend({},c.datepicker.regional[""]);c.fn.extend({autodatepicker:function(){return this.each(function(){var w=
...
H+"&sensor=false&callback=maps_loaded&region=CA&language="+y+Y)}add_maps_loaded_callback(S)};var Z=function(){return F(function(y,H){y?z(y):alert(q(H))})},ca=function(){if(k){var y=document.getElementById("located_near_address").value;if(C==y)return!1;C=y;var H=Z();(pending_geocode_address=y)?f.geocode({address:y},H):(v(),l("",""))}return!1};window.searchform_map_loaded=function(){G();var y=$("#located_near_check_button").click(ca),H=$("#located_near_address").keydown(da).blur(function(){y.click()});
if(google.maps.places){var S=new google.maps.places.Autocomplete(H[0]);S.addListener("place_changed",function(){var V=S.getPlace();V.geometry&&V.geometry.location&&(pending_geocode_address=H.val(),z(V.geometry.location))})}};var da=function(y){null==y&&(y=window.event);if(13==y.keyCode)return document.getElementById("located_near_check_button").click(),!1}})();
