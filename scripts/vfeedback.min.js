...
[];onbeforerestorevalues_fns=[];var l=function(){var f={form_values:get_form_values(c)};n.each(onbeforeunload_fns,function(a,b){b(f)});document.getElementById("cache_form_values").value=JSON.stringify(f)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&l()});document.addEventListener("freeze",l);window.addEventListener("pagehide",l);window.cache_register_onbeforeunload=function(f){onbeforeunload_fns.push(f)};window.cache_register_onbeforerestorevalues=function(f){onbeforerestorevalues_fns.push(f)};
window.restore_cached_state=function(){n(window).on("pageshow",function(f){if(!f.originalEvent.persisted&&(f=document.getElementById("cache_form_values"))&&f.value){var a=JSON.parse(f.value);n.each(onbeforerestorevalues_fns,function(b,d){d(a)});restore_form_values(c,a.form_values)}})}}})();
