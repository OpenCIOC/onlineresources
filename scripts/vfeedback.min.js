...
"2"==b?(c.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-month-label").show(),c.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-week-label").hide()):(c.find(".repeat-week-of-month-ui, .repeats-on-ui, .recurs-month-label").show(),c.find(".repeat-every-ui, .repeat-day-of-month-ui, .recurs-week-label").hide())};a("#ScheduleEditArea").on("change",".recur-type-selector",c).on("click",".schedule-ui-accept-feedback",function(c){var b=a(this);c=b.parents(".EntryFormItemBox");b=b.data("schedule");
restore_form_values(c,b);return!1}).find(".recur-type-selector").each(c)}})();
