...
[];onbeforerestorevalues_fns=[];var k=function(){var c={form_values:get_form_values(a)};d.each(onbeforeunload_fns,function(g,p){p(c)});document.getElementById("cache_form_values").value=JSON.stringify(c)};document.addEventListener("visibilitychange",function(){"hidden"==document.visibilityState&&k()});document.addEventListener("freeze",k);window.addEventListener("pagehide",k);window.cache_register_onbeforeunload=function(c){onbeforeunload_fns.push(c)};window.cache_register_onbeforerestorevalues=function(c){onbeforerestorevalues_fns.push(c)};
window.restore_cached_state=function(){d(window).on("pageshow",function(c){if(!c.originalEvent.persisted&&(c=document.getElementById("cache_form_values"))&&c.value){var g=JSON.parse(c.value);d.each(onbeforerestorevalues_fns,function(p,t){t(g)});restore_form_values(a,g.form_values)}})}}})();
