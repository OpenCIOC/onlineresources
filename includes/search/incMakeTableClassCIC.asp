...
If Not Nl(intResultsPageSize) Then
	strSQL = strSQL & vbCrLf & _
		"OFFSET " & (intResultsPageSize * intResultsCurrentPage) & " ROWS FETCH NEXT " & intResultsPageSize & " ROWS ONLY" & vbCrLf & "END"
End If
...
ElseIf intTotalResultCount > 0 Then
	rsOrgList.MoveFirst
End If
%>
...
%>
<p>
	<a class="btn btn-info" role="button" href="<%=makeLink(ps_strPathToStart & "savedsearch_edit.asp","WhereClause=" & Server.URLEncode(strSaveSQL) & "&InclDel=" & IIf(bInclDeleted,"on",vbNullString) & "&Notes=" & strSaveNotes,vbNullString)%>">
...
%>
				<a class="btn" href="<%= strPageLinkTemplate %>0">
					<span class="fa fa-fast-backward" aria-hidden="true"></span> <%=TXT_FIRST%>
				</a>
				<a class="btn" href="<%= strPageLinkTemplate & intPrevPage %>">
...
%>
				<a class="btn" href="<%= strPageLinkTemplate & intNextPage %>">
					<%= TXT_NEXT %> <span class="fa fa-step-forward" aria-hidden="true"></span>
				</a>
				<a class="btn" href="<%= strPageLinkTemplate & intLastPage %>">
...
%>
<%
			If opt_fld_bOrgCIC And Not opt_fld_bNUM Then
%>
			<%=Nz(get_view_data_cic("ClickToViewDetails"), StringIf(Not bPagedResults," <br class=""hidden-lg"">") & TXT_CLICK_ON & TXT_ORG_NAME & TXT_VIEW_FULL)%>
<%
			Else
%>
			<%=StringIf(Not bPagedResults," <br class=""hidden-lg"">") & TXT_CLICK_ON%><%=TXT_RECORD_NUM%><%=TXT_VIEW_FULL%>
<%
			End If
%>
	</div>
...
<%
		...
		If user_bCIC Then
%>
		<a class="btn btn-info" href="javascript:openWin('<%=makeLink(ps_strPathToStart & "display_options.asp",vbNullString,vbNullString)%>');">
...
<%
			If g_bUseCIC _
				...
				And Nl(Request("SRCHID")) Then
%>
		<a class="btn btn-info" href="<%=makeLink(ps_strPathToStart & "savedsearch_edit.asp","WhereClause=" & Server.URLEncode(strSaveSQL) & "&InclDel=" & IIf(bInclDeleted,"on",vbNullString) & "&Notes=" & strSaveNotes,vbNullString)%>">
			<span class="fa fa-save" aria-hidden="true"></span>
			<%=TXT_SAVE_THIS_SEARCH%>
		</a>
<%
...
%>
		<a class="btn btn-info" href="<%=makeLink(ps_strPathToStart & "advsrch.asp","RS=" & Server.URLEncode(strRecentSearchKey),vbNullString)%>">
...
<%
			End If
%>
		<span id="show_subjects_display" class="NotVisible">
			<a id="show_subjects" class="btn btn-info">
...
<%
		...
		%>
		<a class="btn btn-info" href="<%= makeLink("printlist2.asp", "IDList=" & strRecordListIDs & "&ProfileID=" & intDefaultPrintProfile, vbNullString) %>" target="_BLANK">
			<span class="fa fa-print" aria-hidden="true"></span>
			<%=TXT_PRINT_RECORD_DETAILS %>
		</a>
		<%
			End If
		End If
...
%>
	<form name="RecordList" class="form" action="<%=ps_strPathToStart%>processRecordList.asp" method="post">
	<%=g_strCacheFormVals%>
...
%>
					<div class="form-group row">
						<label for="facet-<%=indOrgFldData.fFieldID%>" class="control-label col-xs-12 col-sm-3">
							<%= indOrgFldData.fLabel %>
						</label>
...
%>
						...
						<div class="col-xs-3 col-sm-2">
							<input type="button" class="btn btn-default" value="<%=TXT_RESET%>" onClick="$('#facet-<%=indOrgFldData.fFieldID%>').val('').trigger('change');"/>
						</div>
					</div>
<%
			Next
...
<%
	...
	If opt_fld_bOrgCIC Then
%>
				<th><%=Nz(get_view_data_cic("OrgProgramNames"), TXT_ORG_NAMES)%></th>
<%
	End If
	If opt_fld_bRecordOwnerCIC Then
%>
...
<%
	...
	If ps_intDbArea=DM_CIC And opt_bPub Then
%>
				<th><%=TXT_UPDATE_PUBS%></th>
<%
	End If
	If (opt_bMail Or opt_bEmailCIC) And user_bCanRequestUpdateCIC Then
%>
				<th class="MobileHideField"<%If opt_bMail And opt_bEmailCIC Then%> colspan="2"<%End If%>><%=TXT_REQUEST_UPDATE%></th>
				<th class="MobileShowFieldNormal"></th>
<%
	End If
	If Not g_bPrintMode And (opt_bListAddRecordCIC Or bEnableListViewMode) Then
		Call myListResultsAddRecordHeader()
	End If
%>
			</tr>
		</thead>
...
<%
		...
		While Not .EOF
			...
			strEmailLink = "&nbsp;"
			strDetailLink = Replace(Replace(strDetailLinkTemplate, "[NUMNUM]", fldNUM), "[NUMBERNUMBER]", Cstr(i))
