...
<p><input type="submit" value="<%=TXT_SEARCH%>" class="btn btn-default"> <input type="RESET" value="<%=TXT_CLEAR_FORM%>" class="btn btn-default"></p>
...
%>
		<div style="display:none"><input type="hidden" name="TMC" value=<%=AttrQs(strTMC)%>></div>
		<strong><%=TXT_MUST_MATCH_TERMS%></strong><%=strTermListDisplayAll%>
<%
		End If
		If Not Nl(strTermListDisplayAny) Then
%>
		<div style="display:none"><input type="hidden" name="ATMC" value=<%=AttrQs(strATMC)%>></div>
		<%=StringIf(Not Nl(strTermListDisplayAll),"<br>")%><strong><%=TXT_MATCH_ANY_TERMS%></strong><%=strTermListDisplayAny%>
<%
		End If
		If bTMCRestricted Then
%>
		<div style="display:none"><input type="hidden" name="TMCR" value="on"></div>
...
<tr> 
	...
	<td class="field-data-cell">
		<div class="radio-inline">
			<label for="SCon_A"><input type="radio" name="SCon" id="SCon_A" value="A" checked> <%=TXT_ALL_TERMS%></label>
		</div>
		<div class="radio-inline">
...
<%End If%>
		<input type="text" name="STerms" id="STerms" title=<%=AttrQs(TXT_SEARCH_TERMS)%> class="form-control">
		<div class="radio-inline">
			<label for="SType_A"><input type="radio" name="SType" id="SType_A" value="A" checked>&nbsp;<%=TXT_WORDS_ANYWHERE%></label>
		</div>
		<div class="radio-inline">
			<label for="SType_O"><input type="radio" name="SType" id="SType_O" value="O">&nbsp;<%=TXT_ORG_NAMES%></label>
		</div>
		<%If g_bUseThesaurusView Then%>
		<div class="radio-inline">
			<label for="SType_S"><input type="radio" name="SType" id="SType_S" value="S">&nbsp;<%=TXT_SUBJECTS%></label>
		</div>
		<%End If%>
		<%If g_bUseTaxonomyView Then%>
		<div class="radio-inline">
			<label for="SType_T"><input type="radio" name="SType" id="SType_T" value="T">&nbsp;<%=TXT_SERVICE_CATEGORIES%></label>
		</div>
		<%End If%>
		<%If g_bUseCIC And g_bUseThesaurusView Then%>
		<div class="checkbox">
			<label for="HideSubj"><input name="HideSubj" id="HideSubj" type="checkbox" checked> <%=TXT_HIDE_SUBJECT_SIDEBAR%></label>
		</div>
		<%End If%>
	</td>
</tr>
<tr>
	<td class="field-label-cell-widelabel"><label for="NUM"><%=TXT_RECORD_NUM%></label></td>
	<td class="field-data-cell">
...
%>
<tr>
	<td class="field-label-cell-widelabel"><label for="NAICS"><%=TXT_NAICS_SHORT%></label></td>
	<td class="field-data-cell">
...
<tr>
	...
	<td class="field-data-cell">
		<div class="form-inline form-inline-always">
			<%=TXT_SERVING_AGE%> <input type="text" name="Age1" title=<%=AttrQs(TXT_MIN_AGE)%> size="3" maxlength="3" class="form-control"> - <input type="text" name="Age2" title=<%=AttrQs(TXT_MAX_AGE)%> size="3" maxlength="3" class="form-control"> (<%=TXT_IN_YEARS%>)
		</div>
	<div class="radio-inline">
		<label for="AgeType_O"><input type="radio" name="AgeType" id="AgeType_O" value="" checked><%=TXT_ANY_IN_RANGE%></label>
	</div>
	<div class="radio-inline">
		<label for="AgeType_A"><input type="radio" name="AgeType" id="AgeType_A" value="A"><%=TXT_ALL_IN_RANGE%></label>
	</div>
	<br><span class="SmallNote"><%=TXT_AGE_INSTRUCTIONS%></span></td>
</tr>
<%
End If
...
%>
	<p><label for="VacancyTP"><%=TXT_HAS_CAPACITY_FOR%></label><%=makeVacancyTargetPopList(vbNullString, "VacancyTP", True, vbNullString)%></p>
<%
	Call closeVacancyTargetPopListRst()
%>
	<hr>
	<p><label for="Vacancy_N"><input type="radio" name="Vacancy" id="Vacancy_N" value="" checked>&nbsp;<%=TXT_NO_SEARCH_VACANCY%></label>
	<br><label for="Vacancy_Y"><input type="radio" name="Vacancy" id="Vacancy_Y" value="Y">&nbsp;<%=TXT_HAS_VACANCIES%></label>
	<br><label for="Vacancy_W"><input type="radio" name="Vacancy" id="Vacancy_W" value="W">&nbsp;<%=TXT_HAS_VACANCIES_OR_WAITLIST%></label></p>
	</td>
</tr>
<%
End If
...
<tr>
	...
	<br><label for="NumEmpType_T"><input type="radio" name="NumEmpType" id="NumEmpType_T" value="" checked>&nbsp;<%=TXT_TOTAL_EMPLOYEES%></label>
	<label for="NumEmpType_F"><input type="radio" name="NumEmpType" id="NumEmpType_F" value="F">&nbsp;<%=TXT_FULL_TIME%></label>
	<label for="NumEmpType_P"><input type="radio" name="NumEmpType" id="NumEmpType_P" value="P">&nbsp;<%=TXT_PART_TIME_SEASONAL%></label>
	</td>
</tr>
<%
...
<tr>
	...
	<td class="field-data-cell"><%=TXT_MORE_THAN%> <input type="text" name="LastEmail" title=<%=AttrQs(TXT_DAYS_SINCE_EMAIL_REQUESTING_UPDATE)%> id="LastEmail" size="3" maxlength="3"> <%=TXT_DAYS%></td>
</tr>
<%
End If
...
<tr> 
	...
	<br><select name="RO" id="RO" class="form-control" multiple>
<%
			While Not .EOF
...
<tr>
	...
	<td class="field-data-cell"><label for="VolType"><input type="radio" name="VolType" id="VolType" value="" checked><%=TXT_NO_SEARCH_OPPS%></label>
	<br><label for="VolType_A"><input type="radio" name="VolType" id="VolType_A" value="A"><%=TXT_HAS_ANY_OPPS%></label>
	<br><label for="VolType_V"><input type="radio" name="VolType" id="VolType_V" value="V"><%=TXT_HAS_ANY_OPPS%> <strong><%=TXT_IN_THIS_VIEW%></strong></label>
	<br><label for="VolType_C"><input type="radio" name="VolType" id="VolType_C" value="C"><%=TXT_HAS_CURRENT_OPPS%> <strong><%=TXT_IN_THIS_VIEW%></strong></label>
	<br><label for="VolType_P"><input type="radio" name="VolType" id="VolType_P" value="P"><%=TXT_HAS_PUBLIC_OPPS%> <strong><%=TXT_IN_THIS_VIEW%></strong></label>
	<br><label for="VolType_N"><input type="radio" name="VolType" id="VolType_N" value="N"><%=TXT_HAS_NO_OPPS%></label>
	<br><label for="VolType_E"><input type="radio" name="VolType" id="VolType_E" value="E"><%=TXT_HAS_ONLY_EXPIRED_OPPS%></label></td>
</tr>
<%
End If
...
<tr> 
	...
	<td class="field-data-cell"><label for="HasEmail_A"><input type="radio" name="HasEmail" id="HasEmail_A" value="" checked>&nbsp;<%=TXT_ALL_RECORDS%></label>
...
<tr> 
	...
	<td class="field-data-cell"><label for="PublicStatus_A"><input type="radio" name="PublicStatus" id="PublicStatus_A" value="" checked>&nbsp;<%=TXT_ALL_RECORDS%></label>
...
<tr> 
	...
	<td class="field-data-cell"><label for="EqStat_A"><input type="radio" name="EqStat" id="EqStat_A" value="" checked>&nbsp;<%=TXT_ALL_RECORDS%></label>
		<label for="EqStat_E"><input type="radio" name="EqStat" id="EqStat_E" value="E">&nbsp;<%=TXT_HAS_EQUIVALENT%></label>
		<label for="EqStat_N"><input type="radio" name="EqStat" id="EqStat_N" value="N">&nbsp;<%=TXT_HAS_NO_EQUIVALENT%></label></td>
</tr>
<%
End If
...
<tr> 
	...
	<td class="field-data-cell"><label for="CCRStat_A"><input type="radio" name="CCRStat" id="CCRStat_A" value="" checked>&nbsp;<%=TXT_ALL_RECORDS%></label>
		<label for="CCRStat_R"><input type="radio" name="CCRStat" id="CCRStat_R" value="R">&nbsp;<%=TXT_ONLY_CHILD_CARE_RESOURCES%></label>
		<label for="CCRStat_N"><input type="radio" name="CCRStat" id="CCRStat_N" value="N">&nbsp;<%=TXT_EXCLUDE_CHILD_CARE_RESOURCES%></label></td>
</tr>
<%
End If
...
<tr> 
	...
	<td class="field-data-cell"><label for="incDel"><input name="incDel" id="incDel" type="checkbox"> <%=TXT_INCLUDE_DELETED%></label></td>
</tr>
<%
End If
...
%>
<tr>
	<td class="field-label-cell-widelabel"><label for="Limit"><%=TXT_SQL%></label></td>
	<td class="field-data-cell"><a href="javascript:openWinXL('<%=makeLinkB("sql_help.asp")%>','sqlHelp')"><%=TXT_SQL_HELP%></a>
	<br><textarea name="Limit" id="Limit" rows="<%=TEXTAREA_ROWS_LONG%>" class="form-control"></textarea></td>
</tr>
<%
End If
...
%>
<tr>
	<td class="field-label-cell-widelabel"><label for="CheckListSource"><%=TXT_CHECKLISTS%></label></td>
	<td class="field-data-cell"><div id="CheckListSourceContainer">
