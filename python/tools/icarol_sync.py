...
def previous_files(dest_dir, prefix, suffix=""):
    return glob(
        os.path.join(dest_dir, prefix + "Full" + _date_pattern + suffix + ".zip")
    ) + glob(
        os.path.join(
            dest_dir,
            prefix
...
def calculate_previous_date_and_this_date(lang, dest_dir, prefix, force_previous):
    ...
    previous_dump_times = [
        x[-start:end] for x in previous_files(dest_dir, prefix, lang.file_suffix)
    ]
...
def calculate_destination_name(lang, type, dest_dir, previous, now, prefix):
    ...
    return os.path.join(
        dest_dir,
        prefix
        + _fname_formats[type].format(
            now=now, previous=previous, file_suffix=lang.file_suffix
        ),
    )
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--force-previous", dest="force_previous", action="store", default=None
    )
    ...
    if previous_filenames and args.type != "full":
        ...
        with open(dest_file, "wb") as fd:
            with bufferedzip.BufferedZipFile(fd, "w", zipfile.ZIP_DEFLATED) as zip:
                write_csv_to_zip(
                    zip,
                    itertools.chain([header], to_delete),
                    os.path.basename(dest_file)[:-4] + ".csv",
                )
        ...
        with open(dest_file, "wb") as fd:
            with bufferedzip.BufferedZipFile(fd, "w", zipfile.ZIP_DEFLATED) as zip:
                write_csv_to_zip(
                    zip, [header], os.path.basename(dest_file)[:-4] + ".csv"
                )
...
def download_kwargs(lang, args):
    kwargs = {
        "auth": (
            get_config_item(args, "airs_export_user"),
            get_config_item(args, "airs_export_password"),
        ),
        "params": {
            "version": get_config_item(args, "airs_export_xmlversion", "3_0"),
            "DST": get_config_item(args, "airs_export_dst"),
            "IncludeSiteAgency": get_config_item(
                args, "airs_export_include_site_agency", "on"
            ),
            "IncludeDeleted": "on",
...
def schema_part_prep(schema_doc, name, type):
    ...
    doc_root.append(
        etree.XML(
            '<xs:element xmlns:xs="http://www.w3.org/2001/XMLSchema" name="%s" type="%s"/>'
            % (name, type)
        )
    )

    pop = (
        "//xs:complexType[@name='%(type)s']/xs:sequence/xs:element[@type='tSite' or @type='tAgency' or @type='tSiteService']"
        % {"type": type}
    )
    elements = {}
...
def init_xmlschema():
    schema_path = os.path.join(
        __file__, "..", "..", "..", "import", "airs_3_0_modified.xsd"
    )
    ...
    element.getparent().remove(element)
    element = element[0]
    xmlschema_elements["tSource"] = (
        etree.XMLSchema(schema_doc.getroot()),
        {"Agency": ("tAgency", dict(element.attrib))},
    )
                ...
                for error in schema.error_log:
                    error_log.append(
                        (
                            ...
                            oldtag,
                            SchemaError(
                                element.sourceline, error.column, error.message
                            ),
                        )
                    )
...
def validate_download(args, counts):
    ...
    event, element = next(iterable)
    _validate_part(
        error_log, counts, iterable, element, "Source", *xmlschema_elements["tSource"]
    )
...
def email_log(args, outputstream, report, validation_errors, is_error):
    try:
        author = get_config_item(
            args, "airs_export_notify_from", const.CIOC_ADMIN_EMAIL
        )
        to = [
            x.strip()
            for x in get_config_item(
                args, "airs_export_notify_emails", const.CIOC_ADMIN_EMAIL
            ).split(",")
        ]
        email.send_email(
            ...
            author,
            to,
            "AIRS Export to iCarol%s"
            % (
                " -- Review Required!"
                if validation_errors
                else (" -- ERRORS!" if is_error else "")
            ),
...
def generate_report(args, counts):
    return 'AIRS export generated as "%s" with the following counts:\n%s\n' % (
        ...
        "".join(
            ...
            for x in ["tAgency", "tSite", "tSiteService"]
        ),
    )
...
def process_language(args, lang):
    ...
    try:
        args.previous, args.now = calculate_previous_date_and_this_date(
            lang, args.dest, args.filename_prefix, args.force_previous
        )
        args.dest_file = calculate_destination_name(
            lang, args.type, args.dest, args.previous, args.now, args.filename_prefix
        )
        if not args.file:
    ...
    if not args.file:
        try:
            to_delete_percent, to_delete_count, record_counts = calculate_deletion_list(
                lang, url, args, **download_kwargs(lang, args)
            )
        except requests.HTTPError as e:
            body = ""
            if e.response:
            ...
            if to_delete_count >= 50 and to_delete_percent >= 2:
                ...
                sys.stderr.write(
                    "Warning: scheduled to delete %s records (%s%% of total) from iCarol"
...
def main(argv):
    ...
    if args.email:
        if not get_config_item(args, "airs_export_notify_emails", None):
            sys.stderr.write(
                "ERROR: No value for airs_export_notify_emails set in config\n"
            )
            return 1
        else:
            sys.stdout = StringIO()
