...
def upload_file(bucket, location, filename, mime_type, content=None, filebase=None):
    ...
    if content:
        ...
        k.set_contents_from_filename(
            os.path.join(filebase, filename),
            cb=percent_cb,
            num_cb=10,
            policy="public-read",
        )
...
def main(argv):
    ...
    top = (
        open(os.path.join(const._app_path, "fonts", "fontello.css"))
        .read()
        .split("}", 1)[0]
        + "}"
    )
    bottom = (
        open(os.path.join(const._app_path, "styles", "sass", "_fontello.scss"))
        .read()
        .split("}", 1)[1]
    )
    ...
    css = sass.compile(
        string=scss, include_paths=[const._sass_dir], output_style="compressed"
    )
