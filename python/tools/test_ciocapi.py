...
def parse_args(argv):
	...
	parser.add_argument('--url', dest='url', action='store', default='http://fizban-current/')
...
def test_cic(args):
	r = requests.get(urljoin(args.url, '/rpc/orgsearch.asp'), params={'STerms': 'fun'}, **args.kwargs)
	results = r.json()
	...
	r = requests.get(urljoin(args.url, '/rpc/orgsearch.asp'), params={'format': 'xml', 'STerms': 'fun'}, **args.kwargs)
...
def test_vol(args):
	...
	r = requests.get(urljoin(args.url, '/rpc/oppsearch.asp'), params={'forOSSD': 'on'}, **args.kwargs)
	results = r.json()
	...
	r = requests.get(urljoin(args.url, '/rpc/oppsearch.asp'), params={'format': 'xml', 'forOSSD': 'on'}, **args.kwargs)
	xmlresults = ET.fromstring(r.content)
	...
	r = requests.post(urljoin(args.url, '/volunteer/volunteer2.asp'), params={'api': 'on'}, data=volunteer_args, **args.kwargs)
	...
	r = requests.post(urljoin(args.url, '/volunteer/volunteer2.asp'), params={'api': 'on', 'format': 'xml'}, data=volunteer_args, **args.kwargs)
	...
	r = requests.get(urljoin(args.url, '/rpc/browseoppbyorg'), params={'format': 'xml'}, **args.kwargs)
	xmlresults = ET.fromstring(r.content)
