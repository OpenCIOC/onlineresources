...
<div class="panel panel-default max-width-md">
	<div class="panel-heading">
		<a name="basic"></a>
		<h3>${_('Change Results Display')}</h3>
	</div>
	<div class="panel-body no-padding">
		<table class="BasicBorder cell-padding-4 full-width form-table inset-table responsive-table">
			...
			<tr>
				<td>
					<div class="row">
						<div class="col-xs-6 col-md-4">${renderer.checkbox(prefix + 'ShowID', label=_('Record #'))}</div>
						<div class="col-xs-6 col-md-4">${renderer.checkbox(prefix + 'ShowOwner', label=_('Record Owner'))}</div>
						<div class="col-xs-6 col-md-4">${renderer.checkbox(prefix + 'ShowOrg' , label=_('Org. Name(s)'))}</div>
						<div class="col-xs-6 col-md-4">${renderer.checkbox(prefix + 'ShowCommunity', label=_('Located In'))}</div>
						<div class="col-xs-6 col-md-4">${renderer.checkbox(prefix + 'ShowUpdateSchedule', label=_('Update Schedule'))}</div>
						<div class="col-xs-6 col-md-4">${renderer.checkbox(prefix + 'ShowAlert', label=_('Alert Box'))}</div>
					</div>
				</td>
			</tr>
			%else: #VOL
			...
			<tr>
				<td>
					<p>${_('Custom Fields:')} <span class="SmallNote">(${_('Hold CTRL to select/deselect multiple items')})</span></p>
					${renderer.errorlist(prefix + 'FieldIDs')}
					${renderer.select(prefix + 'FieldIDs', multiple=True, id='DisplayOptions_FieldIDs', options=disp_opt_field_descs, size=5, class_="form-control")}
					<input type="button" class="btn btn-default" value="${_('Clear Selections')}" onClick="$('#DisplayOptions_FieldIDs option').attr('selected', false); return false;">
				</td>
			</tr>
			%if domain.id == const.DM_CIC:
			<tr>
