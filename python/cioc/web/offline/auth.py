...
def verify_auth(request):
    ...
    if not request.dboptions.UseOfflineTools:
        ...
        raise AuthFailure(
            {"fail": True, "reason": _("Offline Tools not enabled", request)}
        )
    ...
    if not val:
        ...
        raise AuthFailure(
            {"fail": True, "reason": _("Can't find auth request record", request)}
        )
    ...
    with request.connmgr.get_connection("admin") as conn:
        user_type = conn.execute(
            "EXEC sp_GBL_LoginCheck_Offline ?, ?",
    ...
    try:
        public_key.verify(
            base64.b64decode(data["ChallengeSig"]),
            tocheck,
            padding.PSS(
