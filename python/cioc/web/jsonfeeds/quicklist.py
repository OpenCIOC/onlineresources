...
class JsonfeedsQuicklist(viewbase.ViewBase):
    def __call__(self):
        ...
        if pub_code:
            type_name = "Headings"
            sql = (
                "EXEC dbo.sp_CIC_GeneralHeading_l"
                + ("_Count" if add_count else "")
                + " ?,NULL, NULL,?,0,1,?"
            )
            args = [
                cic_view.ViewType if add_count else request.dboptions.MemberID,
                True
        ...
        return {"type": type_name, "quicklist": list(map(fixfn, quicklist))}
