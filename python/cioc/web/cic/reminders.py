...
class ReminderView(viewbase.ViewBase):
    ...
    def _reminders(self):
        ...
        if model_state.validate():
            ...
            for reminder in reminders:
                ...
                reminder.Agencies = self._dict_list_from_xml(
                    reminder.Agencies, "agency"
                )

            reminders = render(
                templateprefix + "reminder_list.mak",
                {
                    "reminders": reminders,
    ...
    def _reminder(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            reminder = conn.execute(
                sql, request.dboptions.MemberID, reminder_id
            ).fetchone()
        ...
        reminder_dict = {
            "ActiveDate": i18n.format_date(reminder.ActiveDate, request)
            if reminder.ActiveDate
            else None,
            "DueDate": i18n.format_date(reminder.DueDate, request)
            if reminder.DueDate
            else None,
            "PastDue": reminder.DueDate <= datetime.now()
            if reminder.DueDate
            else False,
    ...
    @view_config(
        route_name="reminder_add",
        request_method="POST",
        request_param="_force_method=GET",
    ...
    def _save(self, is_api=True):
        ...
        if model_state.validate():
            ...
            args.extend(
                ",".join(str(y).upper() for y in x) if x else None
                for x in (
                    data.get(z)
                    for z in ["reminder_agency_ID", "reminder_user_ID", "NUM", "VNUM"]
                )
            )
            ...
            if not result.Return:
                ...
                msg = (
                    _("The Reminder was successfully added", request)
                    if is_add
                    else _("The Reminder was successfully updated")
                )
                self._go_to_route(
                    "reminder_action",
                    action="edit",
    ...
    def _delete(self):
        ...
        return {
            "success": True,
            "form": render(
                "cioc.web:templates/confirmdelete#error_page.mak",
                namespace,
                request=request,
            ),
        }
    ...
    def _delete_confirm(self, is_api=True):
        ...
        if not result.Return:
            ...
            self._go_to_route(
                "reminder_index",
                _query=[
                    ("InfoMsg", _("The Reminder was successfully deleted.", request))
                ],
            )
