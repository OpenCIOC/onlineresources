...
class UpdatePubsSchema(Schema):
    ...
    DeleteFeedback = validators.DictConverter(
        {"Y": True, "N": False}, if_empty=False, if_missing=False
    )


EditValues = collections.namedtuple(
    "EditValues",
    "publication publication_descriptions linked_headings fullorg generalheadings taxonomyheadings feedback NUM BTPBID Number record_cultures",
)
...
class UpdatePubs(CicViewBase):
        ...
        if model_state.validate():
            ...
            if not result.Return:
                ...
                self._go_to_route(
                    ...
                    _query=base_params
                    + [
                        ("InfoMsg", _("The record was successfully updated.", request)),
                        ("BTPBID", BTPBID),
                    ],
                )
    ...
    def edit(self):
        ...
        if request.params.get("Delete"):
            self._go_to_route(
                "cic_updatepubs",
                action="delete",
                _query=base_params + [("BTPBID", request.params.get("BTPBID"))],
            )

        model_state = request.model_state
        model_state.validators = {"BTPBID": ciocvalidators.IDValidator(not_empty=True)}
    ...
    def _get_edit_info(self, num, BTPBID, number, base_params):
        ...
        record_cultures = [
            x
            for x in syslanguage.active_record_cultures()
    ...
    def delete(self):
        ...
        return self._create_response_namespace(
            ...
            dict(
                id_name="BTPBID",
                id_value=BTPBID,
                extra_values=base_params,
                route="cic_updatepubs",
                action="delete",
            ),
            no_index=True,
        )
    ...
    def delete_confirm(self):
        ...
        if not result.Return:
                ...
                self._go_to_page(
                    ...
                    dict(
                        ...
                        + [
                            (
                                "InfoMsg",
                                _("The Publication was successfully deleted.", request),
                            )
                        ]
                    ),
                )
            self._go_to_page(
                "~/",
                dict(InfoMsg=_("The Publication was successfully deleted.", request)),
            )
