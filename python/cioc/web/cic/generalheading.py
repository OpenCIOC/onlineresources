...
class GeneralHeadingSchema(validators.RootSchema):
	...
	descriptions = validators.CultureDictSchema(GeneralHeadingDescriptionSchema(), pre_validators=[validators.DeleteKeyIfEmpty()])

	RelatedHeadings = All(validators.Set(use_set=True), ForEach(validators.IDValidator()))
...
EditValues = collections.namedtuple('EditValues', 'generalheading generalheading_descriptions relatedheadings generalheadings headinggroups pubcode must_match match_any terms is_add PB_ID, GH_ID')


@view_defaults(route_name='cic_generalheading')
class GeneralHeading(CicViewBase):

	@view_config(route_name='cic_generalheading_index', renderer=templateprefix + 'index.mak')
	def index(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			headings = conn.execute('EXEC dbo.sp_CIC_GeneralHeading_l_Admin ?, ?', request.dboptions.MemberID, model_state.value('PB_ID')).fetchall()
			for heading in headings:
				heading.Descriptions = self._culture_dict_from_xml(heading.Descriptions, 'DESC')
				if heading.RelatedHeadings:
					heading.RelatedHeadings = Markup("<br>".join(escape(x['Name']) for x in self._dict_list_from_xml(heading.RelatedHeadings, 'HEADING')))

		title = _('Headings List', request)
		return self._create_response_namespace(title, title, dict(headings=headings), no_index=True)
	...
	def save(self):
		...
		if request.POST.get('Delete'):
			self._go_to_route('cic_generalheading', action='delete', _query=[('GH_ID', request.POST.get('GH_ID')), ('PB_ID', request.POST.get('PB_ID'))])
		...
		if model_state.validate():
			...
			args = [GH_ID, user.Mod, request.dboptions.MemberID, not request.dboptions.OtherMembersActive or user.cic.SuperUserGlobal, PB_ID]
			...
			if not result.Return:
				...
				if is_add:
					...
					msg = _('The General Heading has been successfully updated.', request)

				self._go_to_route('cic_generalheading', action='edit', _query=[('InfoMsg', msg), ("GH_ID", GH_ID), ('PB_ID', PB_ID)])
		...
		if is_add:
			...
			title = _('Edit Heading: %s', request) % edit_values['generalheading'].CurrentDisplayName
	...
	def _get_edit_info(self, is_add, PB_ID, GH_ID, extra_codes=None):
		...
		with request.connmgr.get_connection('admin') as conn:
			if not is_add:
				...
				cursor = conn.execute(sql, request.dboptions.MemberID, user.Agency, GH_ID)
			...
			cursor = conn.execute('''
						...
						EXEC dbo.sp_TAX_Term_l_GeneralHeading ?''', PB_ID, GH_ID, ','.join(codes))
		...
		if publication.MemberID is None and request.dboptions.OtherMembersActive and not request.user.cic.SuperUserGlobal and not publication.CanEditHeadingsShared:
		...
		for k, g in groupby(terms, key=attrgetter('MatchAny')):
			...
			target.extend({'Code': [x.Code for x in h]} for k, h in groupby(g, key=attrgetter('GH_TAX_ID')))

		return EditValues(generalheading, generalheading_descriptions, relatedheadings, generalheadings, headinggroups, publication.PubCode, must_match, match_any, term_map, is_add, PB_ID, GH_ID)
	...
	def delete(self):
		...
		if not model_state.validate():
			if model_state.is_error('PB_ID'):
				self._error_page(_('Publication ID:', request) + model_state.renderer.errorlist('PB_ID'))
		...
		return self._create_response_namespace(title, title, dict(id_name='GH_ID', id_value=GH_ID, route='cic_generalheading', action='delete', extra_values=[('PB_ID', PB_ID)]), no_index=True)
	...
	def delete_confirm(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			...
			cursor = conn.execute(sql, request.dboptions.MemberID, not request.dboptions.OtherMembersActive or user.cic.SuperUserGlobal, GH_ID, user.cic.PB_ID if user.cic.LimitedView else None)
			result = cursor.fetchone()
			cursor.close()

		if not result.Return:
			self._go_to_route('cic_publication', action='edit', _query=[('PB_ID', model_state.value('PB_ID')), ('InfoMsg', _('The General Heading was successfully deleted.', request))])

		if result.Return == 3:
			# XXX check that this is the only #3
			self._error_page(_('Unable to delete Heading:', request) + result.ErrMsg)

		self._go_to_route('cic_generalheading', action='edit', _query=[('ErrMsg', _('Unable to delete Heading: ') + result.ErrMsg), ('GH_ID', GH_ID)])
