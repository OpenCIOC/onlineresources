...
class LayoutSearch:
    ...
    def __call__(self, search_form, **kwargs):
        ...
        if use_cic and viewdata.UseThesaurusView:
            ...
            browse_subject_items = makeAlphaListItems(
                False, "browsebysubj.asp", request
            )
            ...
            if cic_user:
                service_category = [
                    (
                        ...
                        + _("Browse by Service Category"),
                    ),
                    (
                        makeLink("tax.asp"),
                        '<span class="glyphicon glyphicon-sort-by-attributes" aria-hidden="true"></span> '
                        + _("Basic Taxonomy Search"),
                    ),
                    (
                        makeLink("taxsrch.asp"),
                        '<span class="glyphicon glyphicon-filter" aria-hidden="true"></span> '
                        + _("Advanced Taxonomy Search"),
                    ),
                ]
                ...
                service_category = (
                    '<ul id="service-category-staff" class="nav nav-pills nav-stacked">%s</ul>'
                ...
                serv_cat_link_tmpl = makeLink("servcat.asp", "TC=TCTC").replace(
                    "TCTC", "{1}"
                )
                serv_cat_icon_tmpl = '<i class="fa fa-{}" aria-hidden="true"></i> '
                serv_cat_item_tmpl = (
                    '<li><a href="%s">{0}{2}</a></li>' % serv_cat_link_tmpl
                )

                with request.connmgr.get_connection() as conn:
                    service_category_list = conn.execute(
                        "EXEC dbo.sp_TAX_Term_l_CdLvl1"
                    ).fetchall()

                service_category_list = (
                    '<ul id="service-category-list" class="nav nav-pills nav-stacked">%s</ul>'
                    % "".join(
                        serv_cat_item_tmpl.format(
                            "" if not x.IconFA else serv_cat_icon_tmpl.format(x.IconFA),
                            *x[:-1]
                        )
                        for x in service_category_list
                    )
                )

        menu_groups = {}
        for key, group in itertools.groupby(
            self.menu_items_custom, lambda x: x.MenuGroup
        ):
        ...
        if user or self.menu_items_custom:
            for key, val in viewdata.Cultures.items():
                ...
                if key != passvars.DefaultCulture:
                    httpvals["Ln"] = key
                other_langs.append(
                    dict(Link=makeLink("~/", httpvals, ("Ln",)), Display=val)
                )
        ...
        if (
            user
            and request.dboptions.UseVOL
            and viewdata.VolunteerLink
            and Culture in request.viewdata.vol.Cultures
        ):
        ...
        if user:
            if dboptions.UseCIC and cic_user and user.SavedSearchQuota:
                ...
                search_menu_items.append(
                    (makeLink("savedsearch.asp"), icon + _("Saved Search"))
                )
            ...
            if dboptions.UseCIC and (
                (cic_user.ExportPermission and viewdata.HasExportProfile)
                or viewdata.HasExcelProfile
            ):
                icon = '<span class="glyphicon glyphicon-export" aria-hidden="true"></span> '
                search_menu_items.append((makeLink("export.asp"), icon + _("Export")))

            if cic_user.ImportPermission:
                icon = '<span class="glyphicon glyphicon-import" aria-hidden="true"></span> '
                search_menu_items.append(
                    (
                        makeLink("import/import.asp", {"DM": const.DM_CIC}),
                        icon + _("Import"),
                    )
                )
            ...
            if cic_user.CanUpdatePubs == const.UPDATE_ALL and dboptions.UseCIC:
                ...
                if cic_user.LimitedView:
                    search_menu_items.append(
                        (
                            makeLink("publication/edit", {"PB_ID": cic_user.PB_ID}),
                            icon + _("Publications"),
                        )
                    )
                else:
                    search_menu_items.append(
                        (makeLink("publication"), icon + _("Publications"))
                    )

            if cic_user.CanDeleteRecord:
                icon = '<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> '
                search_menu_items.append(
                    (makeLink("delete_manage.asp"), icon + _("Deleted Records"))
                )

            if cic_user.CanViewStats and dboptions.UseCIC:
                icon = '<span class="glyphicon glyphicon-stats" aria-hidden="true"></span> '
                search_menu_items.append(
                    (makeLink("stats.asp"), icon + _("Statistics"))
                )
            ...
            if cic_user.CanManageUsers:
                ...
                search_menu_items.append(
                    (makeLinkAdmin("users.asp"), icon + _("Manage Users"))
                )
            ...
            if cic_user.SuperUser:
                ...
                search_menu_items.append(
                    (
                        makeLinkAdmin("download.asp", dict(DM=const.DM_CIC)),
                        icon + _("Download"),
                    )
                )
                icon = '<span class="glyphicon glyphicon-wrench" aria-hidden="true"></span> '
                search_menu_items.append(
                    (makeLinkAdmin("setup.asp"), icon + _("Setup"))
                )

            if cic_user.WebDeveloper:
                icon = '<span class="glyphicon glyphicon-wrench" aria-hidden="true"></span> '
                search_menu_items.append(
                    (makeLinkAdmin("setup_webdev.asp"), icon + _("Setup"))
                )

            icon = '<span class="glyphicon glyphicon-user" aria-hidden="true"></span> '
            search_menu_items.append(
                (makeLinkAdmin("account.asp"), icon + _("My Account"))
            )
        ...
        show_cic_search_form = any(
            getattr(search_info, x)
            for x in [
                "BSrchAges",
                "BSrchLanguage",
                "BSrchNUM",
        ...
        quick_searches = [
            {
                "URL": makeLink("~/" + qs.PageName, qs.QueryParameters),
                "NAME": qs.Name,
                "PROMOTE": qs.PromoteToTab,
            }
        ...
        log.debug(
            "quick_searches: %s, %s, %s",
            quick_searches,
            quick_searches_tab,
            quick_searches_notab,
        )

        has_centre_section = (
            show_cic_search_form
            or bool(topic_searches)
            or bool(quick_searches)
            or service_category
            or (use_cic and search_info.CSrch)
            or (use_cic and viewdata.UseNAICSView)
            or browse
            or search_info.SearchCentreMessage
        )
        ...
        quick_list_clear_visible = lambda index: " ".join(
            "visible-" + y + "-block"
            for condition, sizes in quicklist_clear_at
            for y in sizes
        ...
        if viewdata.LimitedView or viewdata.QuickListPubHeadings:
            ...
            for key, group in itertools.groupby(self.quicklist, lambda x: x.Group):
                ...
                if key is None:
                    quicklist_split.extend(
                        [
                            {
                                "IDTYPE": "GHID",
                                "ID": row.GH_ID,
                                "NAME": row.GeneralHeading,
                                "ICON": make_icon_html(
                                    None, row.IconNameFull, False, "heading-icon"
                                ),
                                "HEADINGS": None,
                                "CLEAR_CLASS": quick_list_clear_visible(i),
                            }
                            for i, row in enumerate(group, group_row)
                        ]
                    )
                else:
                    group = list(group)
                    sub_heading = [
                        {
                            ...
                            "NAME": row.GeneralHeading,
                            "ICON": make_icon_html(
                                None, row.IconNameFull, False, "sub-heading-icon"
                            ),
                        }
                        for row in group
                    ]
                    row = group[0]
                    quicklist_split.append(
                        {
                            "IDTYPE": "GHID",
                            "ID": row.GroupID,
                            "IDLIST": ",".join(str(row.GH_ID) for row in group),
                            "NAME": row.Group,
                            "ICON": make_icon_html(
                                None, row.IconNameFullGroup, False, "heading-icon"
                            ),
                            "HEADINGS": sub_heading,
                            "CLEAR_CLASS": quick_list_clear_visible(group_row),
                        }
                    )
        else:
            quicklist_split = [
                {
                    "IDTYPE": "PBID",
                    "ID": row.PB_ID,
                    "NAME": row.PubName or row.PubCode,
                    "ICON": None,
                    "HEADINGS": None,
                    "CLEAR_CLASS": quick_list_clear_visible(i),
                }
                for i, row in enumerate(self.quicklist or [])
            ]

        namespace = {
            "TOGGLE_NAV_TITLE": gettext("Toggle navigation", request),
            "MENU_TITLE": search_info.MenuTitle
            or (_("Main Menu") if not not user else None),
            ...
            "BROWSE": browse,
            "BROWSE_BY_INDUSTRY_URL": makeLink("browsebyindustry.asp")
            if use_cic and viewdata.UseNAICSView
            else None,
            "NAICS_INTRO": gettext(
                "Browse for a Business / Organization using the ", request
            ),
            "NAICS_LINK_TEXT": gettext(
                "North American Industry Classification System (NAICS)", request
            ),
            ...
            "TOPIC_SEARCHES": [
                {
                    ...
                    "TITLE": t,
                    "DESC": d,
                    "LINK": request.passvars.route_path("cic_topicsearch", tag=g),
                }
                for g, t, d in topic_searches
            ],
            ...
            "QUICK_SEARCH_TITLE": search_info.QuickSearchTitle
            or gettext("Quick Search", request),
            ...
            "SEARCH_KEYWORD_TITLE": search_info.KeywordSearchTitle
            or gettext("New Search", request),
            "SEARCH_KEYWORD_GLYPH": search_info.KeywordSearchGlyph or "search",
            "SEARCH_OTHER_TITLE": search_info.OtherSearchTitle
            or gettext("Explore", request),
            "SEARCH_OTHER_GLYPH": search_info.OtherSearchGlyph or "hand-right",
            "SEARCH_OTHER_MESSAGE": search_info.OtherSearchMessage,
            "SEARCH_MENU": not not (
                search_info.MenuMessage or user or self.menu_items_custom
            ),
            ...
            "SERVICE_CATEGORY_URL": makeLink("servcat.asp")
            if use_cic and viewdata.UseTaxonomyView and not cic_user
            else None,
            "VOL_LINK": vol_link,
            ...
            or (
                ...
                and request.viewdata.cic.OtherCommunity
            ),
            "QUICKLIST": quicklist_split,
            "QUICKLIST_COL_CLASS": quicklist_col_class,
            "QUICKLIST_SEARCH": kwargs["searchform_quicklist"]
            if self.quicklist
            else None,
            ...
            "VOLUNTEER_ORG_URL": makeLink("results.asp", "HasVol=on")
            if search_info.BSrchVOL
            else None,
            "VOLUNTEER_OPPORTUNITIES": gettext("Volunteer Opportunities", request),
            "ORGS_WITH_OPS": request.viewdata.cic.OrganizationsWithVolOps
        ...
        namespace["HAS_LEFT_CONTENT"] = any(
            namespace[x] for x in ["SEARCH_ALERT", "SEARCH_MENU", "SEARCH_LEFT_CONTENT"]
        )
        namespace["HAS_RIGHT_OR_SERVCAT"] = any(
            namespace[x] for x in ["SEARCH_RIGHT_CONTENT", "SERVICE_CATEGORY"]
        )

        return template.apply_html_values(
            self.template_values.SearchLayoutHTML or "", namespace
        )


@view_config(
    route_name="cic_basic_search", renderer="cioc.web.cic:templates/bsearch.mak"
)
class BasicSearch(CicViewBase):
    ...
    def __call__(self):
        ...
        with request.connmgr.get_connection("admin" if request.user else None) as conn:
            ...
            if request.dboptions.UseCIC:
                if not (cic_view.LimitedView or cic_view.QuickListPubHeadings):
                    ...
                    args.extend(
                        [
                            cic_view.QuickListPubHeadings or cic_view.PB_ID,
                            True
                            if cic_view.CanSeeNonPublicPub is None
                            else cic_view.CanSeeNonPublicPub,
                        ]
                    )
            if request.user:
        ...
        if layout_info.SystemLayout and layout_info.LayoutHTMLURL:
            layout_info.SearchLayoutHTML = read_file(
                os.path.join(_system_layout_dir, layout_info.LayoutHTMLURL)
            )
        ...
        title = (
            cic_view.SearchTitleOverride
            or request.pageinfo.PageTitle
            or _("Organization / Program Search")
        )
        ...
        namespace = self._create_response_namespace(
            ...
            dict(
                ...
                ages=ages,
                viewslist=viewslist,
                vacancy_targets=vacancy_targets,
                quicklist=quicklist,
                languages=languages,
        ...
        if search_info.CSrch and request.dboptions.UseCIC:
            ...
            csearchform, mapsbottomjs, _ = csearch.childcare_form(
                dict(namespace), communities
            )
        else:
            csearchform = mapsbottomjs = ""

        namespace["makeSearchForm"] = LayoutSearch(
            request,
            layout_info,
            search_info,
            topic_searches,
