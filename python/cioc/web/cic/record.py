...
def link_org_level(request, record, level):
    ...
    if level <= record.LINK_ORG_TO and should_link_levels(request):
        ...
        return link_template % (
            request.passvars.makeLink("~/bresults.asp", crit),
            org_data,
        )
...
def link_service_name_level(request, record, level):
    ...
    if (
        getattr(record, "LINK_SERVICE_NAME_%d" % level)
...
def link_location_name(request, record):
    ...
    if record.LINK_LOCATION_NAME and org_data and should_link_levels(request):
        crit = [("ORGNUM", record.ORG_NUM or record.NUM), ("LL1", org_data)]
...
def parse_language_xml(record):
    ...
    return ET.fromstring(
        ("<RECORD_LANG>" + (record.RECORD_LANG or "") + "</RECORD_LANG>").encode(
            "utf-8"
        )
    )
...
_optgroup_template = Markup('<optgroup label="%s">%s</optgroup>')
_option_template = Markup('<option id="%s" href="%s"%s>%s</option>')


def build_nav_dropdown(
    request,
    record,
    num_link,
    num_number_link,
        ...
        options.append(
            (
                ...
                makeLink(
                    ...
                    + (
                        ""
                        if cur_culture == restore_culture
                        else "&UpdateLn=" + cur_culture
                    ),
                ),
                "",
                _("Update Record", request),
            )
        )
    ...
    if request.multilingual:
        for lang in lang_xml:
            ...
            if lang.get("HAS_LANG") == "0" and lang.get("CAN_UPDATE") == "1":
                options.append(
                    (
                        "AL_Equivalent" + culture,
                        makeLink("~/copy.asp", num_number_link + "&CopyLn=" + culture),
                        "",
                        _("Create Equivalent - %s", request) % lang.get("LanguageName"),
                    )
                )

    if request.dboptions.UseCIC and record.CAN_UPDATE != 0 and cicuser.CanCopyRecord:
        options.append(
            ("AL_Copy", makeLink("~/copy.asp", num_link), "", _("Copy Record", request))
        )
    ...
    if (cicuser.SuperUser or cicuser.CanDeleteRecord) and record.CAN_UPDATE == 1:
        if not record.DELETION_DATE:
            options.append(
                (
                    "AL_Delete",
                    makeLink("~/delete_mark.asp", "IdList=%d" % record.BTD_ID),
                    "",
                    _("Delete Record", request),
                )
            )
        else:
            options.append(
                (
                    "AL_Restore",
                    makeLink(
                        "~/delete_mark.asp", "IdList=%d&Unmark=on" % record.BTD_ID
                    ),
                    "",
                    _("Restore Record", request),
                )
            )
    ...
    if options:
        optgroups.append(
            (
                _("Data Management", request),
                Markup("").join(_option_template % x for x in options),
            )
        )
    ...
    if request.dboptions.UseCIC:
        ...
        if request.viewdata.cic.UseTaxonomyView and record.CAN_INDEX:
            options.append(
                (
                    "AL_Taxonomy",
                    makeLink("~/update_tax.asp", num_number_link),
                    "",
                    _("Update Service Categories", request),
                )
            )
        ...
        if cicuser.CanUpdatePubs != const.UPDATE_NONE:
            if not cicuser.LimitedView:
                options.append(
                    (
                        "AL_Pubs",
                        makeLink("~/update_pubs.asp", num_number_link),
                        "",
                        _("Update Publications", request),
                    )
                )
                ...
                options.append(
                    (
                        ...
                        makeLink(
                            "~/updatepubs/edit",
                            num_number_link + "&BTPBID=%d" % record.BT_PB_ID,
                        ),
                        "",
                        _("Update Description/Headings", request),
                    )
                )
        ...
        # Request Update
        if (
            cicuser.CanRequestUpdate
            and (cicuser.SuperUser or request.user.Agency == record.RECORD_OWNER)
            and (
                record.MemberID == request.dboptions.MemberID or record.CAN_UPDATE == 1
            )
        ):
            if record.CAN_EMAIL and not request.dboptions.NoEmail:
                options.append(
                    (
                        "AL_EmailUpdate",
                        makeLink(
                            "~/admin/email_prep.asp",
                            idlist_link + "&DM=%d" % const.DM_CIC,
                        ),
                        "",
                        _("Email Update Request", request),
                    )
                )
            ...
            options.append(
                (
                    "AL_MailForm",
                    makeLink("~/mailform.asp", num_number_link + "&PrintMd=on"),
                    " newwindow",
                    _("Mail Form (New Window)", request),
                )
            )
    ...
    log.debug(
        "%s %s %s",
        request.dboptions.UseVOL,
        request.viewdata.cic.VolunteerLink,
        request.user.vol.CanAddRecord,
    )
    if (
        request.dboptions.UseVOL
        and request.viewdata.cic.VolunteerLink
        and request.user.vol.CanAddRecord
    ):
        options.append(
            (
                "AL_CreateOpp",
                makeLink("~/volunteer/entryform.asp", num_link),
                "",
                _("Create Volunteer Opportunity", request),
            )
        )
    ...
    if optgroups:
        return Markup("").join(_optgroup_template % x for x in optgroups)
    ...
    if len(search_list) > 1 and number > 0:
        search_list_ui.append(
            ...
            % {
                ...
                "prev_url": makeDetailsLink(
                    search_list[number - 1], "Number=%d" % (number - 1)
                ),
                "first": _("First", request),
                "previous": _("Previous", request),
            }
        )
    ...
    if number < len(search_list) - 1:
        ...
        search_list_ui.append(
            ...
            % {
                ...
                "next_url": makeDetailsLink(
                    search_list[number + 1], "Number=%d" % (number + 1)
                ),
                "last_num": search_list[-1],
                "last_url": makeDetailsLink(
                    search_list[-1], "Number=%d" % (len(search_list) - 1)
                ),
                "next": _("Next", request),
                "last": _("Last", request),
            }
...
"""
            ...
            field_sql = "\n".join(
                "%s, " % x.FieldSelect
                for x in field_list
                if x.FieldName not in skip_fields
            )
...
"""
            ...
            check_update_pub = (
                user_cic.CanUpdatePubs != const.UPDATE_NONE
                and user_cic.LimitedView
                and user_cic.PB_ID == viewdata.PB_ID
            )
            record_sql = """\
SET NOCOUNT ON
...
SET NOCOUNT OFF
			""" % {
                ...
                "vol_ops": (
                    ...
                    + request.viewdata.WhereClauseVOLNoDel
                    + ") THEN 1 ELSE 0 END AS HAS_VOL,"
                )
            ...
            try:
                ...
                log.exception(
                    "Error running SQL with params(%s):\n%s", params, record_sql
                )
                self._error_page(
                    _("Error: An unkown error occurred", request),
        ...
        if record is None:
            ...
            return self._render_to_response(
                ...
                _("Record Details", request),
                _("Record Details", request),
                {
                    "ErrMsg": _("Error: No record exists with the ID: %s.", request)
                    % num
                },
            )
        ...
        if record.BTD_ID is None:
            ...
            other_langs_links = link_other_langs(
                request, lang_xml, num, cur_culture, number
            )
            errmsg = _(
                "Error: This record is not available in the selected language.", request
            )
            ...
            errmsg = _(
                "Error: The record you requested (%s) exists in the database, but access to it has been restricted from this area. This record may be incomplete or waiting to be updated, the program or service may no longer be offered, or the type of service may have changed making it no longer appropriate for the record to be listed here.",
                request,
            )
            title = _("Record Details", request)
            record_owner = recordowner.get_record_owner_info(
                request, record.RECORD_OWNER, const.DM_CIC
            )
            return self._render_to_response(
                "cioc.web.cic:templates/record_contact_agency.mak",
                title,
                title,
                dict(
                    ErrMsg=errmsg % record.ORG_NAME_FULL,
                    record=record,
        ...
        for key, group in itertools.groupby(
            field_list, key=lambda x: (x.DisplayFieldGroupID, x.DisplayFieldGroupName)
        ):
            g = (
                (x, getattr(record, x.FieldName))
                for x in group
                if x.FieldName not in more_skip
            )
        ...
        if request.user:
            ...
            nav_dropdown = build_nav_dropdown(
                request,
                record,
                num_link,
                num_number_link,
        ...
        if (
            len(org_locations) == 1
            and org_locations[0].NUM == record.NUM
            and len(org_services) == 1
            and org_services[0].NUM == record.NUM
        ):
            org_locations = []
            org_services = []
        ...
        context = {
            ...
            "org_level_2to5_linked": Markup(", ").join(
                x
                for x in (link_org_level(request, record, i) for i in range(2, 6))
                if x
            ),
            "service_levels_linked": Markup(", ").join(
                x
                for x in (
                    link_service_name_level(request, record, i) for i in range(1, 3)
                )
                if x
            ),
        ...
        if is_pdf:
            ...
            for map_canvas in document.xpath("//div[@id='map_canvas']"):
                ...
                if lat and lng and googlemaps.hasGoogleMapsAPI(request):
                    ...
                    if False:
                        map_canvas.tag = "img"
                        map_canvas.attrib["src"] = (
                            "https://maps.googleapis.com/maps/api/staticmap?zoom=13&size=300x300&center=%s,%s&markers=color:red%%7Clabel:A%%7C%s,%s&%s"
                            % (lat, lng, lat, lng, maps_key)
                        )
            ...
            if request.params.get("DebugPDF") != "True":
                ...
                vw_tmp = params.pop("UseCICVwTmp", None)
                if vw_tmp:
                    params["UseCICVw"] = vw_tmp
                exclude = ["Number", "InlineResults", "ErrMsg", "InfoMsg"]
                params = [
                    (x.encode("utf-8"), y.encode("utf-8"))
                    for x, y in params.items()
                    if x not in exclude
                ]
                params = urlencode(params)

                url = request.path_url.replace("/pdf", "") + (
                    ("?" + params) if params else ""
                )

                namespace["srcurl"] = url

                footer_html = self._render(
                    "cioc.web:templates/pdffooter.mak", namespace
                )
