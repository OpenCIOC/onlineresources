...
class TopicSearch(CicViewBase):
    ...
    @view_config(
        route_name="cic_topicsearch_index", renderer=templateprefix + "index.mak"
    )
    def index(self):
        ...
        with request.connmgr.get_connection() as conn:
            topicsearches = conn.execute(
                "EXEC dbo.sp_CIC_View_ls_TSrch ?", cic_view.ViewType
            ).fetchall()

        title = _("List of Available Topic Searches", request)
        return self._create_response_namespace(
            title, title, dict(topicsearches=topicsearches), no_index=True
        )

    @view_config(
        route_name="cic_topicsearch", renderer=templateprefix + "searchform.mak"
    )
    def searchform(self):
        cursor = None
