...
class ChildCareSearch(CicViewBase):
	...
	def __call__(self):
		...
		childsearchform, mapsbottomjs, search_info = self.childcare_form(basic_namespace)

		if childsearchform is None:
			self._security_failure()

		basic_namespace.update(dict(childsearchform=Markup(childsearchform()), mapsbottomjs=Markup(mapsbottomjs()), search_info=search_info))
	...
	def childcare_form(self, basic_namespace, communities=None, languages=None):
		...
		namespace.update(dict(
			search_info=search_info, age_groups=age_groups, communities=communities,
			bus_routes=bus_routes, languages=languages, schools=schools,
			types_of_program=types_of_program, types_of_care=types_of_care, located_near=[]))

		childsearchform = partial(self._render, templateprefix + 'csearch_form#childsearchform.mak', namespace)
		mapsbottomjs = partial(self._render, templateprefix + 'csearch_form#mapsbottomjs.mak', namespace)
