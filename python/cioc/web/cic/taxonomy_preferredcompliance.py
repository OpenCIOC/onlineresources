...
@view_defaults(
    ...
    match_param="action=preferredcompliance",
    renderer=templateprefix + "preferredcompliance.mak",
)
class activationRecView(CicViewBase):
    ...
    def preferredcompliance(self):
        ...
        act_global = bool(
            ...
            or (request.params.get("Global") and user.cic.SuperUserGlobal)
        )

        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC dbo.sp_TAX_Term_l_PreferredTermCompliance ?, ?",
        ...
        return self._create_response_namespace(
            title,
            title,
            dict(compliancelist=compliancelist, act_global=act_global),
            no_index=True,
        )
    ...
    def preferredcompliance_save(self):
        ...
        if model_state.validate():
            ...
            with request.connmgr.get_connection("admin") as conn:
                cursor = conn.execute(
                    "EXEC dbo.sp_TAX_Term_u_PreferredTermCompliance ?, ?, ?", *args
                )
                cursor.close()
            ...
            self._go_to_route(
                "cic_taxonomy", action="preferredcompliance", **extra_args
            )
