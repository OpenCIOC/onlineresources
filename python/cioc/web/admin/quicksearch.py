...
def should_skip_item(quicksearch):
    ...
    if quicksearch.get("QuickSearchID") == "NEW":
        ...
        if all(
            ...
            if not descriptions or all(
                not v for d in descriptions.values() for v in d.values()
            ):
                return True
...
class PostSchema(Schema):
    ...
    pre_validators = [cull_skippable_items]
    quicksearch = foreach.ForEach(QuickSearchBaseSchema())


@view_defaults(
    route_name="admin_view",
    match_param="action=quicksearch",
    renderer=templateprefix + "quicksearch.mak",
    request_param="DM=" + str(const.DM_CIC),
)
class QuickSearch(viewbase.AdminViewBase):
            ...
            for i, quicksearch in enumerate(model_state.form.data["quicksearch"]):
                ...
                try:
                    qp = urlencode(
                        [
                            (k, v)
                            for k, v in parse_qsl(qp, True)
                            if (k not in skip and (k != "page" or v == "all"))
                        ]
                    )
                except ValueError:
                    pass
            ...
            if not result.Return:
                ...
                self._go_to_route(
                    ...
                    _query=[
                        (
                            "InfoMsg",
                            _("The Quick Searches were successfully updated.", request),
                        ),
                        ("DM", domain.id),
                        ("ViewType", ViewType),
                    ],
    ...
    def _get_edit_info(self, domain, ViewType):
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC sp_%s_View_QuickSearch_lf ?, ?, ?" % domain.str,
                request.dboptions.MemberID,
                user.Agency,
                ViewType,
            )
