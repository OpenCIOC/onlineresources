...
class ThesaurusSourceBaseSchema(Schema):
	...
	Descriptions = ciocvalidators.CultureDictSchema(ThesaurusSourceDescriptionSchema(), record_cultures=False, allow_extra_fields=True, fiter_extra_fields=False, delete_empty=True, pre_validators=[ciocvalidators.DeleteKeyIfEmpty()])
...
@view_defaults(route_name='admin_thesaurus', match_param='action=source', renderer=templateprefix + 'source.mak')
class ThesaurusSource(AdminViewBase):
	...
	def source(self):
		...
		return self._create_response_namespace(title, title,
				{'sources': sources, 'usage': usage}, no_index=True)
	...
	def save_source(self):
		...
		if model_state.validate():
			...
			for i,source in enumerate(model_state.form.data['source']):
				...
				if src_id == 'NEW' and not any(x.get('SourceName') for x in descriptions.values()):
					continue
			...
			if not result.Return:
				...
				self._go_to_route('admin_thesaurus', action='source', 
						_query=[('InfoMsg', _('The Thesaurus Sources have been successfully updated.', request))])
		...
		sources = edit_values['sources'] = variabledecode.variable_decode(request.POST)['source']
