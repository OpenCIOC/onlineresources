...
class Notice(viewbase.AdminViewBase):
    @view_config(
        route_name="admin_notices_index", renderer=templateprefix + "index.mak"
    )
    def index(self):
        ...
        return self._create_response_namespace(
            title, title, notices_info._asdict(), no_index=True
        )
    ...
    def new(self):
        ...
        valdict = {
            "AreaCode": validators.String(
                max=20, if_invalid=None, if_empty=None, if_missing=None
            ),
        ...
        with request.connmgr.get_connection("admin") as conn:
            areas = conn.execute(
                "EXEC sp_GBL_Admin_Area_l ?, NULL", request.dboptions.MemberID
            ).fetchall()
        ...
        return self._create_response_namespace(
            title, title, dict(areas=areas), no_index=True
        )

    @view_config(
        match_param="action=new",
        renderer=templateprefix + "new.mak",
        request_method="POST",
    )
    def new_save(self):
        request = self.request
        ...
        if model_state.validate():
            args = [
                user.Mod,
                user.User_ID,
                model_state.value("AdminAreaID"),
                model_state.value("RequestDetail"),
            ]
            ...
            with request.connmgr.get_connection("admin") as conn:
                ...
                if not result.Return:
                    return self._go_to_page(
                        "~/admin/setup.asp", {"InfoMsg": _("Change request sent.")}
                    )
    ...
    def close(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            notice = conn.execute("EXEC sp_GBL_Admin_Notice_s ?", NoticeID).fetchone()
    ...
    @view_config(
        match_param="action=close",
        renderer=templateprefix + "close.mak",
        request_method="POST",
    )
    def close_save(self):
        request = self.request
        ...
        if model_state.validate():
            ...
            args = [
                ...
                ",".join(map(str, domains)),
                user.Mod,
                model_state.value("ActionTaken"),
                model_state.value("ActionNotes"),
            ]
        ...
        return self._create_response_namespace(
            title,
            title,
            dict(notice=notice, NoticeID=NoticeID, ErrMsg=ErrMsg),
            no_index=True,
        )
