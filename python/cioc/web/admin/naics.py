...
class Naics(viewbase.AdminViewBase):
    ...
    def save(self):
        ...
        if request.POST.get("Delete"):
            self._go_to_route(
                "admin_naics",
                action="delete",
                _query=[("Code", request.POST.get("Code"))],
            )
        ...
        with request.connmgr.get_connection("admin") as conn:
            if not is_add:
                cursor = conn.execute(
                    "EXEC dbo.sp_NAICS_s ?", model_state.value("Code")
                )
        ...
        descs = sorted(
            list(descriptions.items()), key=lambda x: x[0] != request.language.Culture
        )
        ...
        if not is_add:
            ...
            classification = (
                descs[0][1].Classification if descs else _("Unknown", request)
            )
        else:
    ...
    def edit(self):
        ...
        model_state = request.model_state
        model_state.validators = {"Code": NaicsCode()}
        ...
        return self._create_response_namespace(
            title,
            title,
            dict(naics=naics, Code=Code, is_add=is_add, classification=classification),
            no_index=True,
        )
    ...
    def delete_confirm(self):
        ...
        if not result.Return:
            self._go_to_route(
                "admin_naics_index",
                _query=[
                    ("InfoMsg", _("The NAICS Code was successfully deleted.", request))
                ],
            )
