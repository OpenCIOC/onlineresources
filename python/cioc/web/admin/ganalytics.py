...
@view_defaults(
    route_name="admin_ganalytics", renderer=templateprefix + "ganalytics.mak"
)
class GoogleAnalyticsView(viewbase.AdminViewBase):
    ...
    def post(self):
        ...
        if model_state.validate():
            ...
            args = [
                ...
                user.Mod,
                ET.tostring(domains, encoding="unicode"),
            ]
            ...
            if not result.Return:
                ...
                msg = _(
                    "The Google Analytics Configuration successfully updated.", request
                )
    ...
    def _get_edit_info(self, all=True):
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC sp_GBL_View_DomainMap_l @MemberID=?", request.dboptions.MemberID
            )
