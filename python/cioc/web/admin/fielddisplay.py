...
@view_defaults(route_name='admin_fielddisplay', renderer=templateprefix + 'fielddisplay.mak')
class FieldDisplay(viewbase.AdminViewBase):
	...
	def index(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			cursor = conn.execute('EXEC sp_%s_FieldOption_l_Display ?' % ('GBL' if domain.id == const.DM_CIC else domain.str), request.MemberID)
	...
	def save(self):
		...
		if model_state.validate():
			...
			if not result.Return:
				...
				self._go_to_route('admin_fielddisplay',
		...
		fields = []
		with request.connmgr.get_connection('admin') as conn:
			fields = conn.execute('EXEC sp_%s_FieldOption_l_Display ?, 0' % ('GBL' if domain.id == const.DM_CIC else domain.str), request.MemberID).fetchall()
