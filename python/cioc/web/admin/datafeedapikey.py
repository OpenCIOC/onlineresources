...
class DataFeedApiKeyView(viewbase.AdminViewBase):
        ...
        if not model_state.validate():
            ...
            with request.connmgr.get_connection("admin") as conn:
                ...
                if not result.Return:
                    ...
                    log.debug("Redirect")
                    self._go_to_route(
                        "admin_datafeedapikey",
                        action="edit",
                        _query=[("InfoMsg", msg), ("FeedAPIKey", result.FeedAPIKey)],
                    )
    ...
    @view_config(
        match_param="action=inactive",
        request_method="POST",
        renderer=templateprefix + "confirm.mak",
    )
    def confirm_inactive(self):
        ...
        self._go_to_route(
            "admin_datafeedapikey",
            action="edit",
            _query=[("InfoMsg", msg), ("FeedAPIKey", FeedAPIKey)],
        )

    @view_config(
        route_name="admin_datafeedapikey_index", renderer=templateprefix + "index.mak"
    )
    def index(self):
        request = self.request
        user = request.user
        ...
        return self._create_response_namespace(
            title, title, {"keys": keys}, no_index=True
        )
