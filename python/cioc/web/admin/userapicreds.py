...
class AdminUserApiCredsView(AdminViewBase):
	...
	@view_config(route_name='admin_userapicreds_index', renderer=templateprefix + 'index.mak')
	def index(self):
		context = self.request.context
		title = _('User API Credentials for %s') % context.user.UserName
		return self._create_response_namespace(
			title, title, {'cred_list': context.cred_list, 'cred_user': context.user}, no_index=True)
	...
	def save(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			...
			result = conn.execute(
				sql, request.dboptions.MemberID, None if user.SuperUser else user.Agency, user.Mod, context.user.User_ID,
				'{' + cred_id + '}', salt, repeat, hash, usage_note).fetchone()
	...
	def delete(self):
		...
		if not model_state.validate():
			self._error_page(_('Credential ID:', request) + model_state.renderer.errorlist('CredID'))
		...
		return self._create_response_namespace(
			title, title, dict(
				id_name='CredID', id_value=CredID, route='cic_generalheading',
				action='delete', extra_values=[('User_ID', request.context.user.User_ID)]),
			no_index=True)
	...
	def delete_confirm(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			...
			result = conn.execute(
				sql, request.dboptions.MemberID, None if request.user.SuperUser else request.user.Agency,
				request.context.user.User_ID, CredID).fetchone()

		if not result.Return:
			self._go_to_route('admin_userapicreds_index', _query=[('User_ID', request.context.user.User_ID), ('InfoMsg', _('The API credential was successfully deleted.', request))])

		self._go_to_route('admin_userapicreds_index', _query=[('User_ID', request.context.user.User_ID), ('ErrMsg', _('Unable to delete API credential: ', request) + result.ErrMsg)])
