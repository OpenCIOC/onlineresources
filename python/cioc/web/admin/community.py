...
class Community(viewbase.AdminViewBase):
    @view_config(
        route_name="admin_community_index", renderer=templateprefix + "index.mak"
    )
    def index(self):
    ...
    def save(self):
        ...
        if not is_add and request.POST.get("Delete"):
            self._go_to_route(
                "admin_community", action="delete", _query=[("CM_ID", CM_ID)]
            )
        ...
        if model_state.validate():
            ...
            for name in model_state.form.data.get("alt_names") or []:
                if (
                    name.get("Delete")
                    or not name.get("AltName")
                    or (
                        not name.get("New")
                        and name["Culture"].replace("_", "-") not in shown_cultures
                    )
                ):
                    continue
            ...
            if not result.Return:
                ...
                if is_alt_area:
                    if is_add:
                        msg = _(
                            "The Alternate Search Area was successfully added.", request
                        )
                    else:
                        msg = _(
    ...
    def delete_confirm(self):
        ...
        if not result.Return:
            self._go_to_route(
                "admin_community_index",
                _query=[
                    ("InfoMsg", _("The Community was successfully deleted.", request))
                ],
            )
        ...
        self._go_to_route(
            ...
            action="edit",
            _query=[
                ("ErrMsg", _("Unable to delete Community: ") + result.ErrMsg),
                ("CM_ID", CM_ID),
            ],
        )
    ...
    def autocomplete(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            if search_areas:
                ...
                cursor = conn.execute(
                    "EXEC sp_GBL_Community_ls_ParentSelector ?, ?", cur_parent, terms
                )
    ...
    def list(self):
        ...
        for community in communities:
            ...
            community.AltNames = "; ".join(
                self._list_from_xml(community.AltNames, "AltNames")
            )
            community.AltAreaSearch = "; ".join(
                self._list_from_xml(community.AltAreaSearch, "Name")
            )
        ...
        if request.params.get("csv"):
            ...
            headings = [_("ID")] + [
                _("Name (%s)") % culture_map[culture].LanguageName
            ...
            form_cultures = [
                culture_map[culture].FormCulture for culture in active_cultures
            ]

            base_field_getter = attrgetter(*fields)
            name_field_getter = lambda x: tuple(
                x.Names.get(y, {}).get("Name") for y in form_cultures
            )
            ...
            with BufferedZipFile(file, "w", zipfile.ZIP_DEFLATED) as zip:
                write_csv_to_zip(
                    zip,
                    itertools.chain([headings], map(row_getter, communities)),
                    "communities.csv",
                )
            ...
            res.headers[
                "Content-Disposition"
            ] = "attachment;filename=communities-%s.zip" % (
