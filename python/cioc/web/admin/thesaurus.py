...
class ThesaurusFullSchema(validators.RootSchema):
	...
	descriptions = validators.CultureDictSchema(
		...
		chained_validators=[validators.FlagRequiredIfNoCulture(ThesaurusDescriptionSchema)]
	)
	UseSubj_ID = All(validators.Set(use_set=True), ForEach(validators.IDValidator()))
	BroaderSubj_ID = All(validators.Set(use_set=True), ForEach(validators.IDValidator()))
	RelatedSubj_ID = All(validators.Set(use_set=True), ForEach(validators.IDValidator()))
...
class Thesaurus(AdminViewBase):
	...
	def save(self):
		...
		if request.POST.get('Delete'):
			self._go_to_route('admin_thesaurus', action='delete', _query=[('SubjID', request.POST.get('SubjID'))])
		...
		if model_state.validate():
			...
			with request.connmgr.get_connection('admin') as conn:
				...
				''' % (stored_proc_variant, subj_id_output, kwargstr)
			...
			if not result.Return:
				...
				self._go_to_route('admin_thesaurus', action='edit', _query=(('InfoMsg', msg), ("SubjID", SubjID)))
		...
		with request.connmgr.get_connection('admin') as conn:
			extra_subjs = set(request.POST.getall('UseSubj_ID') +
								request.POST.getall('BroaderSubj_ID') +
								request.POST.getall('ReleatedSubj_ID'))
			log.debug('params: %s, %s, %s', request.dboptions.MemberID, SubjID, extra_subjs)
			cursor = conn.execute('EXEC dbo.sp_THS_Subject_s_FormLists ?, ?, ?', request.dboptions.MemberID, SubjID, ','.join(extra_subjs))
		...
		return self._create_response_namespace(title, title,
				dict(is_add=is_add, subject=context.subject,
					usage=usage, other_term_descs=other_term_descs,
					categories=categories, sources=sources,
					used_for=used_for, narrower=narrower,
					usesubjects=usesubjects, broadersubjects=broadersubjects,
					relatedsubjects=relatedsubjects,
					SubjID=request.POST.get('SubjID'), ErrMsg=ErrMsg),
				no_index=True)
	...
	def edit(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			...
			cursor = conn.execute('EXEC dbo.sp_THS_Subject_s_FormLists ?, ?', request.dboptions.MemberID, SubjID)
		...
		title = _('Manage Subjects', request)
		return self._create_response_namespace(title, title,
				dict(is_add=is_add, subject=subject,
					usage=usage, other_term_descs=other_term_descs,
					categories=categories, sources=sources,
					used_for=used_for, narrower=narrower,
					usesubjects=usesubjects, broadersubjects=broadersubjects,
					relatedsubjects=relatedsubjects, SubjID=SubjID),
				no_index=True)
	...
	def delete(self):
		...
		model_state.validators = {
				'SubjID': validators.IDValidator(not_empty=True)
				}
		...
		title = _('Manage Subjects', request)
		return self._create_response_namespace(title, title, dict(id_name='SubjID', id_value=SubjID, route='admin_thesaurus', action='delete'), no_index=True)
	...
	def delete_confirm(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			...
			cursor = conn.execute(sql, SubjID, None if request.user.cic.SuperUserGlobal else request.dboptions.MemberID)
			result = cursor.fetchone()
			cursor.close()

		if not result.Return:
			self._go_to_page('~/admin/thesaurus.asp', {'InfoMsg': _('The Subject Term has been successfully deleted.', request)})

		if result.Return == 3:
			# XXX check that this is the only #3
			self._error_page(_('Unable to delete Subject:', request) + result.ErrMsg)

		self._go_to_route('admin_thesaurus', action='edit', _query=[('ErrMsg', _('Unable to delete Subject: ') + result.ErrMsg), ('SubjID', SubjID)])
