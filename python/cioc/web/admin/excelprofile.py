...
class ExcelProfile(AdminViewBase):
	...
	@view_config(route_name='admin_excelprofile_index', renderer=templateprefix + 'index.mak')
	def index(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			cursor = conn.execute('EXEC sp_CIC_ExcelProfile_l ?, NULL', request.dboptions.MemberID)
			profiles = cursor.fetchall()
			cursor.close()

		title = _('Manage Excel Profiles', request)
		return self._create_response_namespace(title, title, dict(profiles=profiles), no_index=True)
	...
	def save(self):
		...
		if request.POST.get('Delete'):
			self._go_to_route('admin_excelprofile', action='delete', _query=[('ProfileID', request.POST.get('ProfileID'))])
		...
		if model_state.validate():
			...
			args = [ProfileID, user.Mod, request.dboptions.MemberID, model_state.form.data['profile'].get('ColumnHeadersWeb')]
			...
			if not result.Return:
				...
				self._go_to_route('admin_excelprofile', action='edit', _query=[('InfoMsg', msg), ("ProfileID", ProfileID)])
		...
		with request.connmgr.get_connection('admin') as conn:
			cursor = conn.execute('EXEC dbo.sp_CIC_ExcelProfile_s_FormLists ?, ?', request.MemberID, model_state.value('ProfileID'))
			...
			if not is_add:
				profile = conn.execute('EXEC dbo.sp_CIC_ExcelProfile_s ?, ?', request.dboptions.MemberID, model_state.value('ProfileID')).fetchone()
		...
		model_state.form.data['profile.ColumnHeadersWeb'] = column_headers_reverse.get(val, val)
		...
		return self._create_response_namespace(title, title,
				dict(action=action, profile=profile, profile_descriptions=profile_descriptions,
					ProfileID=model_state.value('ProfileID'),
					fieldorder=fieldorder, field_descs=field_descs,
	...
	def edit(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			cursor = conn.execute('EXEC dbo.sp_CIC_ExcelProfile_s ?, ?', request.dboptions.MemberID, ProfileID)
			...
			cursor = conn.execute('EXEC dbo.sp_CIC_ExcelProfile_s_FormLists ?, ?', request.MemberID, ProfileID)
		...
		model_state.form.data['Fields'] = sorted(fields, key=lambda x: field_sort_key.get(x.FieldID, 99999))
		...
		title = _('Manage Excel Profiles', request)
		return self._create_response_namespace(title, title,
				dict(action=action, profile=profile,
					profile_descriptions=profile_descriptions,
					ProfileID=ProfileID, fieldorder=fieldorder, field_descs=field_descs, view_descs=view_descs),
				no_index=True)
	...
	def delete(self):
		...
		title = _('Manage Excel Profiles', request)
		return self._create_response_namespace(title, title, dict(id_name='ProfileID', id_value=ProfileID, route='admin_excelprofile', action='delete'), no_index=True)
	...
	def delete_confirm(self):
		...
		if not result.Return:
			self._go_to_route('admin_excelprofile_index', _query=[('InfoMsg', _('The Profile was successfully deleted.', request))])

		if result.Return == 3:
			self._error_page(_('Unable to delete Excel Profile: ', request) + result.ErrMsg)

		self._go_to_route('admin_excelprofile', action='edit', _query=[('ErrMsg', _('Unable to delete Excel Profile: ') + result.ErrMsg), ('ProfileID', ProfileID)])
