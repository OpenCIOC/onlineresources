...
class PagesView(AdminViewBase):
	...
	def save(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			cursor = conn.execute('EXEC dbo.sp_GBL_Page_s ?, ?, ?, ?',
						request.dboptions.MemberID, user.Agency, domain.id, page_id)
		...
		return self._create_response_namespace(
			title, title,
			dict(
				PageID=page_id, page=page, is_add=is_add, views=views,
				domain=domain, ErrMsg=ErrMsg), no_index=True)
	...
	def delete(self):
		...
		return self._create_response_namespace(title, title, dict(id_name='PageID', id_value=PageID, route='admin_pages', action='delete', extra_values=extra_values), no_index=True)
	...
	def delete_confirm(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			...
			cursor = conn.execute(sql, request.dboptions.MemberID, request.user.Agency, domain.id, PageID)
			result = cursor.fetchone()
			cursor.close()
		...
		if not result.Return:
			self._go_to_route('admin_pages_index', _query=[('InfoMsg', _('Page was successfully deleted.', request))] + query)

		if result.Return == 3:
			self._error_page(_('Unable to delete Page: ', request) + result.ErrMsg)

		self._go_to_route('admin_pages', action='edit', _query=[('ErrMsg', _('Unable to delete Page: ') + result.ErrMsg), ('PageID', PageID)] + query)
	...
	def get_domain(self):
		...
		if (domain.id == const.DM_CIC and not user.cic.SuperUser and not user.cic.WebDeveloper) or (domain.id == const.DM_VOL and not user.vol.SuperUser and not user.vol.WebDeveloper):
			self._security_failure()
