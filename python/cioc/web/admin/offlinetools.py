...
class OfflineTools(viewbase.AdminViewBase):
	...
	@view_config(route_name='admin_offlinetools', request_method='POST', renderer=templateprefix+'offlinetools.mak')
	def post(self):
	...
	@view_config(route_name='admin_offlinetools', renderer=templateprefix+'offlinetools.mak')
	def get(self):
		request = self.request
		user = request.user
	...
	def _get_edit_info(self, all=True):
		...
		offline_machines = []
		with request.connmgr.get_connection('admin') as conn:
			cursor = conn.execute('EXEC sp_CIC_Offline_Machines_l ?, ?, ?', request.dboptions.MemberID, user.Agency, all)
