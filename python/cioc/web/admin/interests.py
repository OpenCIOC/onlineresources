...
class InterestSchema(validators.Schema):
	...
	descriptions = validators.CultureDictSchema(InterestDescriptionSchema(), record_cultures=True,
			delete_empty=False, chained_validators=[validators.FlagRequiredIfNoCulture(InterestDescriptionSchema)])
	groups = validators.All(
		validators.Set(use_set=True, if_missing=validators.NoDefault),
		validators.ForEach(validators.IDValidator(not_empty=True), not_empty=True, if_missing=validators.NoDefault)
	)
...
class Interests(viewbase.AdminViewBase):
	...
	@view_config(route_name='admin_interests_index', renderer=templateprefix + 'index.mak', request_method="POST")
	def hide(self):
		request = self.request
		user = request.user
		...
		if model_state.validate():
			...
			return self._go_to_route('admin_interests_index', _query=[('InfoMsg', _('Visibility Settings Saved', request))])
	...
	@view_config(route_name='admin_interests_index', renderer=templateprefix + 'index.mak')
	def index(self):
		request = self.request
		user = request.user
	...
	def delete(self):
		...
		model_state.validators = {
			'AI_ID': validators.IDValidator(not_empty=True)
		}
		model_state.method = None
		...
		title = _('Manage Areas of Interest', request)
		return self._create_response_namespace(title, title, dict(id_name='AI_ID', id_value=AI_ID, route='admin_interests', action='delete'), no_index=True)
	...
	def delete_confirm(self):
		...
		if not result.Return:
			self._go_to_route('admin_interests_index', _query=[('InfoMsg', _('The Interest was successfully deleted.', request))])

		if result.Return == 3:
			self._error_page(_('Unable to delete Interest: ', request) + result.ErrMsg)

		self._go_to_route('admin_interests', action='edit', _query=[('ErrMsg', _('Unable to delete Interest: ') + result.ErrMsg), ('AI_ID', AI_ID)])
