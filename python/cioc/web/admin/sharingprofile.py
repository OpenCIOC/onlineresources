...
_revoke_subject = _(
    "A Sharing Profile has been revoked and will end on %(ExpireDate)s."
)
_revoke_template = _(
    """\
Hello,
...
_revoke_records_subject = _(
    "Removal of shared records in your database as of %(ExpireDate)s"
)
...
class SharingProfileContext(rootfactories.BasicRootFactory):
    def __init__(self, request):
        ...
        if not domain:
            error_page(
                request,
                _("No domain selected", request),
                const.DM_GLOBAL,
                const.DM_GLOBAL,
                _("Manage Sharing Profiles", request),
            )

        if not request.dboptions.OtherMembersActive and (
            (domain.id == const.DM_CIC and not user.cic.SuperUser)
        ...
        if ProfileID:
            with request.connmgr.get_connection("admin") as conn:
                profile = conn.execute(
                    "EXEC dbo.sp_%s_SharingProfile_s_Basic ?, ?" % domain.str,
    ...
    def addable(self):
        ...
        return (profile.Active or profile.ReadyToAccept) and (
            not profile.RevokedDate or profile.RevokedDate >= datetime.now()
        )
    ...
    def partnerreview(self):
        ...
        return (
            profile
            and (profile.ReadyToAccept or profile.Active or profile.RevokedDate)
            and self.request.dboptions.MemberID != profile.MemberID
        )
    ...
    def partneracceptable(self):
        ...
        return (
            profile
            and profile.ReadyToAccept
            and not (profile.Active or profile.RevokedDate)
            and self.request.dboptions.MemberID != profile.MemberID
        )
...
class EmailListValidator(All):
    validators = [
        ciocvalidators.String(max=1000, not_empty=True),
...
profile_core_fields = [
    x for x in ProfileBaseSchema.fields if x not in ["Views", "Fields", "EditLangs"]
]
...
class SharingProfile(AdminViewBase):
    @view_config(
        route_name="admin_sharingprofile_index", renderer=templateprefix + "index.mak"
    )
    def index(self):
        ...
        if not domain:
            self._error_page(
                _("No domain selected", request), _("Manage Sharing Profiles", request)
            )
    ...
    @view_config(
        ...
        custom_predicates=[lambda c, r: c.editable],
    )
    @view_config(
        match_param="action=edit",
        request_method="POST",
        renderer=templateprefix + "additems.mak",
        custom_predicates=[lambda c, r: c.addable],
    )
    @view_config(
    ...
    def save(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC dbo.sp_%s_SharingProfile_s_FormLists ?, ?" % domain.str,
                request.MemberID,
            ...
            if not is_add:
                cursor = conn.execute(
                    "EXEC dbo.sp_%s_SharingProfile_s_Edit ?, ?" % domain.str,
                    request.dboptions.MemberID,
                    model_state.value("ProfileID"),
                )
        ...
        model_state.form.data["profile.CanUseAnyView"] = request.POST[
            "profile.CanUseAnyView"
        ]
    ...
    @view_config(
        match_param="action=edit",
        renderer=templateprefix + "revoked.mak",
        custom_predicates=[lambda c, r: c.revoked],
    )
    ...
    def edit(self):
        ...
        if not model_state.validate():
            ...
            self._error_page(
                _("Invalid ID", request), _("Manage Sharing Profiles", request)
            )
            ...
            if is_add:
                ...
                if emailaddresses:
                    model_state.form.data[
                        "profile.NotifyEmailAddresses"
                    ] = emailaddresses.NotifyEmailAddresses

            cursor.close()

        model_state.form.data["profile"] = profile
        model_state.form.data["descriptions"] = profile_descriptions
        views = model_state.form.data["profile.Views"] = [
            str(v.ViewType) for v in views
        ]
        model_state.form.data["profile.EditLangs"] = [x.Culture for x in edit_languages]
        fields = model_state.form.data["profile.Fields"] = [
            str(f.FieldID) for f in fields
        ]
        model_state.form.data["profile.CanUseAnyView"] = (
            "Y" if not profile or profile.CanUseAnyView else "N"
        )
    ...
    def _get_partner_review_data(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC dbo.sp_%s_SharingProfile_s_Review ?, ?" % domain.str,
                request.MemberID,
                ProfileID,
            )
    ...
    @view_config(
        match_param="action=edit",
        renderer=templateprefix + "review.mak",
        custom_predicates=[lambda c, r: c.partnerreview],
    )
    def partnerreview(self):
        request = self.request
        namespace = self._get_partner_review_data()

        request.model_state.form.data["ShareNotifyEmailAddresses"] = namespace[
            "profile"
        ].ShareNotifyEmailAddresses
        title = _("Manage Sharing Profiles", request)
        return self._create_response_namespace(title, title, namespace, no_index=True)
    ...
    def delete_confirm(self):
        ...
        if result.Return == 3:
            self._error_page(
                _("Unable to delete Sharing Profile:", request),
                _("Manage Sharing Profiles", request),
            )
        ...
        self._go_to_route(
            ...
            _query=[
                (
                    "ErrMsg",
                    _("Unable to delete Sharing Profile: ", request) + result.ErrMsg,
                ),
                ("ProfileID", ProfileID),
                ("DM", domain.id),
            ],
    ...
    @view_config(
        match_param="action=send",
        renderer=templateprefix + "send.mak",
        custom_predicates=[lambda c, r: c.editable],
    )
    def send(self):
        request = self.request
    ...
    def send_confrm(self):
        ...
        if not result.Return:
            ...
            if emails:
                ...
                message = _(_send_template, request) % {
                    "MemberName": profile.SharingMemberName,
                ...
                send_email(
                    request,
                    getattr(request.dboptions, "DefaultEmail%s" % context.domain.str),
                    emails,
                    subject,
                    message,
                )
                ...
                query_args.append(
                    ("InfoMsg", _("The Profile was successfully sent.", request))
                )
            else:
                query_args.append(
                    (
                        "InfoMsg",
                        _(
                            "The Profile is available to be accepted, but email notifications could not be sent.",
                            request,
                        ),
                    )
                )
            ...
            query_args.append(
                (
                    "ErrMsg",
                    _("Unable to send Sharing Profile: ", request) + result.ErrMsg,
                )
            )
    ...
    @view_config(
        match_param="action=revoke",
        renderer=templateprefix + "revoke.mak",
        request_method="POST",
        custom_predicates=[lambda c, r: c.addable or c.partnerreview],
    )
    def revoke_confirm(self):
        ...
        model_state = request.model_state
        model_state.schema = BaseSchema(
            RevocationDate=ciocvalidators.DateConverter(not_empty=True, min=min_date)
        )
        ...
        if model_state.validate():
            with request.connmgr.get_connection("admin") as conn:
                ...
                cursor = conn.execute(
                    ...
                    model_state.value("RevocationDate"),
                )
            ...
            if not result.Return:
                if emails:
                    profile = context.profile
                    vars = {
                        "ExpireDate": format_date(
                            model_state.value("RevocationDate"), request
                        ),
                        "MemberName": profile.SharingMemberName
                        if profile.MemberID == request.dboptions.MemberID
                        else profile.ReceivingMemberName,
                        "ProfileName": profile.Name,
                    }
                    ...
                    send_email(
                        request,
                        getattr(
                            request.dboptions, "DefaultEmail%s" % context.domain.str
                        ),
                        emails,
                        subject,
                        message,
                ...
                query_args.append(
                    (
                        "InfoMsg",
                        _("The Sharing Profile was successfully revoked.", request),
                    )
                )

            else:
                query_args.append(
                    ("ErrMsg", _("Unable to revoke Sharing Profile: ") + result.ErrMsg)
                )
    ...
    @view_config(
        match_param="action=accept",
        renderer=templateprefix + "review.mak",
        request_method="POST",
        custom_predicates=[lambda c, r: c.partneracceptable],
    )
    def accept(self):
        request = self.request
        context = request.context
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            cursor = conn.execute(
                ...
                context.ProfileID,
                request.user.User_ID,
                model_state.value("ShareNotifyEmailAddresses"),
            )
        ...
        if not result.Return:
            ...
            query_args.append(
                ("InfoMsg", _("The Profile was successfully accepted.", request))
            )

        else:
            query_args.append(
                ("ErrMsg", _("Unable to accept Sharing Profile: ") + result.ErrMsg)
            )

        self._go_to_route("admin_sharingprofile", action="edit", _query=query_args)

    @view_config(
        match_param="action=records",
        renderer=templateprefix + "records.mak",
        custom_predicates=[
            lambda c, r: c.partnerreview or c.profile.MemberID == r.dboptions.MemberID
        ],
    )
    def records(self):
        request = self.request
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            records = conn.execute(
                ...
                request.dboptions.MemberID,
                profile.ProfileID,
                getattr(request.viewdata, domain.str.lower()).ViewType,
            ).fetchall()
        ...
        if context.addable or context.partnerreview:
            ...
            request.model_state.form.data["RevocationDate"] = format_date(
                min_date, request
            )
    ...
    @view_config(
        match_param="action=records",
        request_param="confirmed=on",
        renderer="cioc.web:templates/error.mak",
        request_method="POST",
        custom_predicates=[lambda c, r: c.editable or c.addable],
    )
    def records_add_confirm(self):
            ...
            if not result.Return:
                self._go_to_route(
                    ...
                    _query=[
                        (
                            "InfoMsg",
                            _("%d records were added.", request) % result.RecordsAdded,
                        ),
                        ("ProfileID", context.ProfileID),
                        ("DM", context.domain.id),
                    ],
                )
        ...
        title = _("Manage Sharing Profiles", request)
        return self._create_response_namespace(
            title, title, dict(ErrMsg=ErrMsg), no_index=True
        )

    @view_config(
        match_param="action=records",
        renderer=templateprefix + "records_add.mak",
        request_method="POST",
        custom_predicates=[lambda c, r: c.editable or c.addable],
    )
    def records_add(self):
        ...
        if not ErrMsg:
            ...
            if not add_info.WillBeAdded:
                ErrMsg = _(
                    "None of the selected records can be added to the Profile. %d records were already in the Profile, and %d records are in another Profile with this Member.",
                    request,
                ) % (add_info.AlreadyAdded, add_info.OtherProfile)
    ...
    @view_config(
        match_param="action=records",
        request_param="Remove=on",
        renderer=templateprefix + "records_remove.mak",
        custom_predicates=[
            lambda c, r: c.partnerreview or c.profile.MemberID == r.dboptions.MemberID
        ],
    )
    def remove_records_from_bulk(self):
        ...
        if not ErrMsg:
            ...
            if not remove_info.WillBeRemoved:
                ErrMsg = _(
                    "No records were removed. They may already have been removed, or be scheduled for removal in the next few days.",
                    request,
                )
    ...
    @view_config(
        match_param="action=remove_records",
        renderer=templateprefix + "records.mak",
        request_method="POST",
        custom_predicates=[
            lambda c, r: c.partnerreview or c.profile.MemberID == r.dboptions.MemberID
    ...
    def remove_records(self):
        ...
        if domain.id == const.DM_CIC:
            ...
            id_name = "VNUM"
            validator["VNUM"] = ciocvalidators.CSVForEach(
                ciocvalidators.VNumValidator()
            )

        if context.addable or context.partnerreview:

            validator["RevocationDate"] = ciocvalidators.DateConverter(
                not_empty=True, min=min_date
            )
        ...
        if model_state.validate():
            ...
            if not result.Return:
                if (
                    (
                        (context.profile.Active and context.addable)
                        or context.partnerreview
                    )
                    and records
                    and emails
                ):
                    ...
                    vars = {
                        ...
                        "ProfileName": profile.Name,
                        "ProfileURL": domain_name[0]
                        + request.route_path(
                            "admin_sharingprofile",
                            action="records",
                            _query=[
                                ("ProfileID", str(context.ProfileID)),
                                ("DM", str(domain.id)),
                            ],
                        )
                        if domain_name
                ...
                self._go_to_route(
                    "admin_sharingprofile",
                    action="records",
                    _query=[("ProfileID", context.ProfileID), ("DM", domain.id)],
                )
    ...
    @view_config(
        match_param="action=changeemail",
        renderer=templateprefix + "changeemail.mak",
        custom_predicates=[lambda c, r: c.partnerreview],
    )
    def changeemail(self):
        ...
        request.model_state.form.data[
            "ShareNotifyEmailAddresses"
        ] = profile.ShareNotifyEmailAddresses

        title = _("Manage Sharing Profiles", request)
        return self._create_response_namespace(
            title,
            title,
            dict(ProfileID=context.ProfileID, profile=context.profile, domain=domain),
            no_index=True,
        )
    ...
    def changeemail_save(self):
        ...
        if not model_state.validate():
            ...
            if not result.Return:
                ...
                self._go_to_route(
                    "admin_sharingprofile", action="edit", _query=query_args
                )
