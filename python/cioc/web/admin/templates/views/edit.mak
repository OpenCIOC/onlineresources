	...
	<table class="BasicBorder cell-padding-4 full-width form-table inset-table responsive-table">
		...
		<tr>
			...
			<td class="field-data-cell">
				...
				%endif
				<br>${_('This view is available in the following languages: ')}
				%for culture in culture_order:
				<% lang = culture_map[culture] %>
				${lang.LanguageName}
				%if len(culture_order) > 1:
				[<a href="${route_path('admin_view', action='delete_lang', _query=[('DM', domain.id), ('ViewType',ViewType), ('Culture', culture)])}">${_('Delete')}</a>]
				%endif
				%endfor
				%if missing_cultures:
				<br>${_('Add language to this view:')}
		...
		<tr>
			...
			<td class="field-data-cell">
				%if view_descs:
				%if usage.IsDefaultView:
				<span class="Alert">${_('Warning: this is the Default View. Any View selected below will immediately become publicly available.')}</span>
				<br>
				%endif
				<strong>${_('Identify the other Views this View can see:')}</strong>
				%for rview in view_descs:
				<br>${renderer.ms_checkbox('Views', str(rview.ViewType), label=rview.ViewName, id='view_' + str(rview.ViewType))}
				%endfor
				%else:
...
<div class="panel-body no-padding">
	<table class="BasicBorder cell-padding-4 full-width form-table inset-table responsive-table">
		...
		<tr>
			...
			<td class="field-data-cell">
				${_('Include the following search types on the Advanced Search page:')}
				<div class="row">
					<div class="col-sm-6">
						<div class="checkbox">
				...
				<p>
					...
					<br>${renderer.ms_checkbox("AdvSearchCheckLists", str(field_id), label=label)}
					%endfor
				</p>
			</td>
		</tr>
		...
		<tr>
			<td class="field-label-cell" rowspan="${3 if domain.id == const.DM_CIC and dboptions.UseCIC else 2}">${_('Search Results')}</td>
			<td class="field-data-cell">
