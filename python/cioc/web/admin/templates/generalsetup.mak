		...
		<div class="panel-body no-padding">
			<table class="BasicBorder cell-padding-4 full-width form-table inset-table responsive-table">
				...
				<tr>
					...
					_('The Default Email address is used whenever an Email message is generated by the system. If a specific Agency Email is available, that will be used as a Reply To address.'), is_required)}
					<td class="field-data-cell">
						${renderer.errorlist('settings.DefaultEmail'+domain)}
						${renderer.email('settings.DefaultEmail'+domain, class_='form-control')}
					</td>
				</tr>
				%if domain != 'VOLProfile':
				<tr>
					${self.fieldLabelCell('settings.DefaultEmailName' + domain,_('Default Email Name (%s)') % dm_label,
					_('The Name used as part of the From address whenever Default Email address is used.'), is_required)}
					<td class="field-data-cell">
						${renderer.errorlist('settings.DefaultEmailName'+domain)}
						${renderer.text('settings.DefaultEmailName'+domain, maxlength=100, class_='form-control')}
					</td>
				</tr>
				%endif
				%if domain == "CIC":
				<tr>
					${self.fieldLabelCell('settings.ImportNotificationEmail' + domain,_('Import Notification Email (%s)') % dm_label,
					_('The email address to be used to send email notifications of issues encountered while running automatic imports'), False)}
					<td class="field-data-cell">
						${renderer.errorlist('settings.ImportNotificationEmail'+domain)}
						${renderer.text('settings.ImportNotificationEmail'+domain, class_='form-control', maxlength=1000)}
					</td>
				</tr>
				%endif
				%endfor
				...
				<tr>
					...
					<td class="field-data-cell">
						${renderer.errorlist('settings.DefaultProvince')}
						<div class="form-inline">
				...
				<tr>
					...
					<td class="field-data-cell">
						${renderer.errorlist('settings.DefaultGCType')}
				...
				<tr>
					...
					<td class="field-data-cell">
						${renderer.errorlist('settings.PreventDuplicateOrgNames')}
