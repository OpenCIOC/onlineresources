...
<select name="${name}" id="${name}" class="form-control">
	...
	<optgroup label="${culture_map[culture].LanguageName}">
		%for page in group:
		<option value="${page.PageID}">${'** ' if page.PublishAsArticle else ''}${page.Title} (${page.Slug})</option>
		%endfor
	</optgroup>
	%endfor
</select>
...
<form action="${request.route_path('admin_pages', action='edit')}" method="get" class="form">
