...
class ListValues(viewbase.AdminViewBase):
    ...
    def _get_request_info(self, list_type, title_template):
        ...
        title = (
            ...
            else title_template.format(list_type.ListName)
        )
    ...
    @view_config(
        ...
        custom_predicates=[lambda c, r: not c.SuperUserGlobal],
    )
    ...
    def edit(self):
        ...
        retval = self._get_request_info(
            list_type, _(list_type.PageTitleTemplate, self.request)
        )
    ...
    def save(self):
        ...
        if list_type.ID:
            extra_validators = {
                list_type.ID
                or "OldValue": Any(list_type.id_validator, validators.OneOf(["NEW"]))
            }
...
class ListContactHonorific(ListValuesModel):
    ...
    SearchLink1 = (
        ...
        dict(
            incDel="on",
            Limit="EXISTS(SELECT * FROM GBL_Contact WHERE GblNUM IS NOT NULL AND bt.NUM=GblNUM AND NAME_HONORIFIC='IDIDID')",
        ),
    )
    SearchLink2 = (
        ...
        dict(
            incDel="on",
            Limit="EXISTS(SELECT * FROM GBL_Contact WHERE VolVNUM IS NOT NULL AND vo.VNUM=VolVNUM AND NAME_HONORIFIC='IDIDID')",
        ),
    )
