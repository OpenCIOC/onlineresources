...
TopicSearchEditValues = collections.namedtuple(
    "TopicSearchEditValues",
    "ViewType TopicSearchID topic_search descriptions publications view_info is_edit",
)
...
class ViewBaseCICSchema(ViewBaseSchema):
    ...
    QuickListMatchAll = validators.DictConverter(
        QuickListMatchAllOptions, if_empy=False
    )
...
class ViewSchema(Schema):
    ...
    AdvSearchCheckLists = All(
        validators.Set(use_set=True), ForEach(ciocvalidators.IDValidator())
    )
...
class ViewSchemaCIC(ViewSchema):
    ...
    pre_validators = [
        viewbase.cull_extra_cultures(
            "descriptions", ensure_active_cultures=False, record_cultures=False
        )
    ]
...
class View(viewbase.AdminViewBase):
    ...
    def index(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC sp_%s_View_l ?, ?, 1, NULL" % domain.str,
    ...
    def save(self):
        ...
        domain, shown_cultures = viewbase.get_domain_and_show_cultures(
            request.params, ensure_active_cultures=False, record_cultures=False
        )
        if not domain:
            self._error_page(_("Invalid Domain", request))
        ...
        if model_state.validate():
            ...
            argnames.extend(
                ["Descriptions", "Views", "AdvSrchCheckLists", "DisplayOptFields"]
            )
            ...
            if not result.Return:
                self._go_to_route(
                    ...
                    _query=[
                        (
                            "InfoMsg",
                            _("The View has been successfully updated.", request),
                        ),
                        ("ViewType", ViewType),
                        ("DM", domain.id),
                    ],
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC dbo.sp_%s_View_s_FormLists ?, ?, ?" % domain.str,
                request.dboptions.MemberID,
                user.Agency,
                ViewType,
            )
        ...
        if domain.id == const.DM_CIC:
            ...
            data["item.SrchCommunityDefault"] = RSrchCommunityDefaultOptions.get(
                val, val
            )
        ...
        return self._create_response_namespace(
            ...
            dict(
                ...
                chk_field_descs=chk_field_descs,
    ...
    def edit(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            cursor = conn.execute(
                "EXEC dbo.sp_%s_View_s ?, ?" % domain.str,
                ViewType,
                request.dboptions.MemberID,
            )
        ...
        data["HasLabelOverrides"] = any(
            getattr(x, y, None)
            for x in descriptions.values()
            for y in label_override_fields
        )
    ...
    def delete_confirm(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            cursor = conn.execute(
                sql, ViewType, request.dboptions.MemberID, user.Agency
            )
            result = cursor.fetchone()
            cursor.close()
        ...
        if not result.Return:
            self._go_to_route(
                "admin_view_index",
                _query=[("InfoMsg", _("View Deleted", request)), ("DM", domain.id)],
            )
        ...
        self._go_to_route(
            ...
            action="edit",
            _query=[
                ("ErrMsg", _("Unable to delete View: ") + result.ErrMsg),
                ("ViewType", ViewType),
                ("DM", domain.id),
            ],
        )
    ...
    def add_lang(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            cursor = conn.execute(
                sql, ViewType, user.Mod, request.dboptions.MemberID, user.Agency, LangID
            )
            result = cursor.fetchone()
            cursor.close()
    ...
    @view_config(
        match_param="action=delete_lang", renderer=templateprefix + "delete_lang.mak"
    )
    def delete_lang(self):
        ...
        return self._create_response_namespace(
            title,
            title,
            dict(ViewType=ViewType, Culture=Culture, domain=domain),
            no_index=True,
        )
    ...
    def delete_lang_confirm(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            cursor = conn.execute(
                sql, ViewType, request.dboptions.MemberID, user.Agency, LangID
            )
            result = cursor.fetchone()
            cursor.close()
    ...
    @view_config(
        match_param="action=topicsearches",
        renderer=templateprefix + "topicsearches.mak",
    )
    def topicsearch(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            cursor = conn.execute(
                sql, request.dboptions.MemberID, user.Agency, ViewType
            )
        ...
        title = _("Manage Topic Searches For %s", request) % _(
            view_info.ViewName, request
        )
        return self._create_response_namespace(
            title,
            title,
            dict(ViewType=ViewType, topic_searches=topic_searches),
            no_index=True,
            print_table=False,
        )

    @view_config(
        match_param="action=topicsearch",
        request_method="POST",
        renderer=templateprefix + "topicsearch.mak",
    )
    def topicsearch_edit_save(self):
        ...
        if request.POST.get("Delete"):
            ...
            self._go_to_route(
                "admin_view",
                action="topicsearch_delete",
                _query=[("ViewType", ViewType), ("TopicSearchID", TopicSearchID)],
            )
        ...
        title = _("Manage Topic Searches For %s", request) % _(
            edit_info.view_info.ViewName, request
        )
        return self._create_response_namespace(
            title, title, template_values, no_index=True, print_table=False
        )

    @view_config(
        match_param="action=topicsearch", renderer=templateprefix + "topicsearch.mak"
    )
    def topicsearch_edit(self):
        ...
        if edit_info.is_edit:
            ...
            request.model_state.defaults = {
                ...
                "descriptions": {
                    k.replace("-", "_"): v for k, v in edit_info.descriptions.items()
                },
            }
        ...
        return self._create_response_namespace(
            title, title, edit_info._asdict(), no_index=True, print_table=False
        )

    def _get_topicsearch_id(self, required=False):
        validator = ciocvalidators.IDValidator(not_empty=required)
        try:
            TopicSearchID = validator.to_python(
                self.request.params.get("TopicSearchID")
            )
    ...
    def _get_topic_search_edit_info(self, ViewType, TopicSearchID):
        ...
        with request.connmgr.get_connection("admin") as conn:
            ...
            cursor = conn.execute(
                sql, TopicSearchID, ViewType, user.Agency, request.dboptions.MemberID
            )
            view_info = cursor.fetchone()
            cursor.nextset()
        ...
        return TopicSearchEditValues(
            ViewType,
            TopicSearchID,
            topic_search,
            descriptions,
            format_pub_list(publications, True),
            view_info,
            is_edit,
        )

    @view_config(
        match_param="action=topicsearch_delete",
        renderer="cioc.web:templates/confirmdelete.mak",
    )
    def topicsearch_delete(self):
        ...
        return self._create_response_namespace(
            ...
            title,
            dict(
                id_name="TopicSearchID",
                id_value=TopicSearchID,
                route="admin_view",
                action="topicsearch_delete",
                extra_values=[("ViewType", ViewType)],
            ),
            no_index=True,
            print_table=False,
        )
    ...
    def topicsearch_delete_confirm(self):
        ...
        if not result.Return:
            self._go_to_route(
                "admin_view",
                action="topicsearches",
                _query=[
                    ("InfoMsg", _("Topic Search Deleted", request)),
                    ("ViewType", ViewType),
                ],
            )
        ...
        if result.Return == 3:
            self._error_page(
                _("Unable to delete Topic Search: ", request) + result.ErrMsg
            )
        ...
        self._go_to_route(
            ...
            action="topicsearch",
            _query=[
                ("ErrMsg", _("Unable to delete Topic Search: ") + result.ErrMsg),
                ("ViewType", ViewType),
                ("TopicSearchID", TopicSearchID),
            ],
        )
