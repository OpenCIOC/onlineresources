...
class SocialMediaSchema(Schema):
	...
	descriptions = ciocvalidators.CultureDictSchema(SocialMediaDescriptionSchema(), record_cultures=True, delete_empty=False)


@view_defaults(route_name='admin_socialmedia')
class SocialMedia(viewbase.AdminViewBase):

	@view_config(route_name='admin_socialmedia_index', renderer=templateprefix + 'index.mak')
	def index(self):
		...
		return self._create_response_namespace(title, title, dict(descs=descs), no_index=True)
	...
	def delete(self):
		...
		title = _('Manage Social Media Types', request)
		return self._create_response_namespace(title, title, dict(id_name='SM_ID', id_value=SM_ID, route='admin_socialmedia', action='delete'), no_index=True)
	...
	def delete_confirm(self):
		...
		if not result.Return:
			self._go_to_route('admin_socialmedia_index', _query=[('InfoMsg', _('The Social Media Type was successfully deleted.', request))])

		if result.Return == 3:
			self._error_page(_('Unable to delete Social Media Type: ', request) + result.ErrMsg)

		self._go_to_route('admin_socialmedia', action='edit', _query=[('ErrMsg', _('Unable to delete Social Media Type: ') + result.ErrMsg), ('SM_ID', SM_ID)])
