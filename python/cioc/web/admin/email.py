...
class DescriptionSchemaNotAllOpps(DescriptionSchemaBase):
    chained_validators = [
        validators.RequireAtLeastOne(["StdDetailDesc", "StdFeedbackDesc"])
    ]
...
class EmailValues(AdminViewBase):
    ...
    def index(self):
        ...
        with request.connmgr.get_connection("admin") as conn:
            emails = conn.execute(
                "EXEC dbo.sp_GBL_StandardEmailUpdate_l ?, ?, ?",
        ...
        return self._create_response_namespace(
            title, title, dict(emails=emails, options=options), no_index=True
        )
    ...
    def save(self):
        ...
        if options.MR:
            extra_validators = {
                "descriptions": validators.CultureDictSchema(DescriptionSchemaBase())
            }
        else:
            extra_validators = {
                "descriptions": validators.CultureDictSchema(
                    DescriptionSchemaNotAllOpps()
                )
            }
    ...
    def edit(self):
        ...
        model_state.schema = validators.RootSchema(
            EmailID=validators.IDValidator(not_empty=not is_add)
        )

        if not model_state.validate():
            self._error_page(
                _("Unable to load Standard Email Update Values: %s", request)
                % model_state.renderer.errorlist("EmailID")
            )
        email_id = model_state.value("EmailID")
        ...
        if email_id:
            with request.connmgr.get_connection("admin") as conn:
                ...
                cursor.nextset()
                descriptions = {
                    x.Culture.replace("-", "_"): x for x in cursor.fetchall()
                }
        ...
        return self._create_response_namespace(
            title,
            title,
            dict(EmailID=email_id, email=email, is_add=is_add, options=options),
            no_index=True,
        )
    ...
    def _get_options(self):
        ...
        if not (
            (options.DM.id == const.DM_CIC and user.cic.SuperUser)
    ...
    def delete_confirm(self):
            ...
            self._go_to_route(
                ...
                _query=[
                    (
                        "InfoMsg",
                        _(
                            "The Standard Email Update Text was successfully deleted.",
                            request,
                        ),
                    )
                ]
        ...
        if result.Return == 3:
            self._error_page(
                _("Unable to delete Standard Email Update Text: ", request)
                + result.ErrMsg
            )
        ...
        self._go_to_route(
            ...
            _query=[
                (
                    "ErrMsg",
                    _("Unable to delete Standard Email Update Text: ") + result.ErrMsg,
                ),
                ("EmailID", EmailID),
            ]
            + query,
