...
@view_defaults(renderer=templateprefix + 'auto_ips.mak')
class Stats1View(viewbase.ViewBase):
	...
	@view_config(route_name='stats_auto_ips_cic')
	@view_config(route_name='stats_auto_ips_vol')
	def index(self):
		...
		with request.connmgr.get_connection('admin') as conn:
			if not model_state.validate():
				...
				if request.params.get('submit'):
					...
					args_param = '?'
					sql = 'EXEC sp_%s_AutoReport_Stats_IP_Total %s'
					if any(model_state.value(x) for x in ['DateRange', 'FirstDate', 'LastDate']):
						...
						sql = prefix + 'EXEC sp_%s_Stats_IP_Total %s'
					...
					stat_rows = {x.TheMonth: x for x in stat_rows}
