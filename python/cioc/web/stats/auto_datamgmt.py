		...
		with request.connmgr.get_connection('admin') as conn:
			if not model_state.validate():
				...
				if request.params.get('submit'):
					...
					if request.pageinfo.DbArea == const.DM_CIC:
						tax_section = [StatType(_('Last Modified - Taxonomy'), True, True)]
					else:
						display_until_section = [StatType(_('Display Until'), True, True)]
					...
					link_template = Markup('<b>%(title)s</b>: <a href="%(url)s">%(count)d</a>')
					...
					for st in stat_types:
						...
						for k, g in groupby(cursor.fetchall(), attrgetter('TheMonth')):
							...
							if st.hasother:
								other = Markup('<br>').join((link_template if x[3] else nolink_template) % {
									'title': x[1],
									'url': makeLink(results, 'Shared=Y&' + x.Criteria, exclude),
									'count': x[3]} for x in rows)
							...
							local = Markup('<br>').join((tmpl if x[2] else nolink_template) % {
								'title': x[1],
								'url': makeLink(results, local_shared_criteria + x.Criteria, exclude) if st.hascriteria else '',
								'count': x[2]} for x in rows)
