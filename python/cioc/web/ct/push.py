...
class InRequest(viewbase.CicViewBase):
	...
	def inrequest(self):
		...
		if remove:
			...
			if not record:
				return {'fail': True, 'errinfo': _('Error: No record exists with the ID: %s', request) % idstring}

			org_name = ', '.join(x for x in (getattr(record, 'ORG_LEVEL_%i' % y) for y in range(1,6)) if x)
			...
			if domain == 'CIC':
				url = request.application_url + request.passvars.makeDetailsLink(idstring)
			else:
				url = request.application_url + request.passvars.makeVOLDetailsLink(idstring)
		...
		url = request.dboptions.ClientTrackerRpcURL + ('remove_resource' if remove else 'add_resource')
