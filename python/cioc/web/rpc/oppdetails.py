	...
	def __call__(self):
		...
		with request.connmgr.get_connection() as conn:
			if opid and not vnum:
				vnum = conn.execute('SELECT VNUM FROM VOL_Opportunity WHERE OP_ID=?', opid).fetchone()
				if not vnum:
					raise HTTPNotFound()

				vnum = vnum.VNUM

			fields = conn.execute('EXEC sp_VOL_View_DisplayFields ?, ?, ?, ?, ?',
						viewdata.ViewType, False, vnum, False, None).fetchall()
			...
			sql = ['''DECLARE @ViewType int
					...
					(SELECT Culture,LangID,LanguageName,LanguageAlias,LCID,Active
						FROM STP_Language LANG WHERE LangID<>@@LANGID AND dbo.fn_VOL_RecordInView(vo.VNUM,@ViewType,LangID,0,GETDATE())=1 AND ''', "ActiveRecord=1" if viewdata.ViewOtherLangs else "EXISTS(SELECT * FROM VOL_View_Description WHERE ViewType=@ViewType AND LangID=LANG.LangID)",
						''' ORDER BY CASE WHEN Active=1 THEN 0 ELSE 1 END, LanguageName FOR XML AUTO) AS RECORD_LANG,''']

			exclude = {'OP_ID', 'VNUM', 'NUM', 'RECORD_OWNER', 'NON_PUBLIC', 'DELETION_DATE', 'MODIFIED_DATE', 'UPDATE_DATE', 'UPDATE_SCHEDULE', 'DISPLAY_UNTIL', 'POSITION_TITLE'}
			...
			sql.append(''', vod.POSITION_TITLE, vod.VNUM AS LangVNUM
			...
			data = conn.execute(sql, viewdata.ViewType, request.viewdata.cic.ViewType, vnum).fetchone()
		...
		fields = [OrderedDict([('value', htmlvalue(x)), ('name', x.FieldName), ('display_name', x.FieldDisplay), ('allow_html', x.CheckHTML)])
					for x in fields]
		...
		if data.RECORD_LANG:
			...
			other_langs = [OrderedDict([('name', x.get('LanguageName')), ('culture', x.get('Culture')), ('link', route_url('rpc_oppdetails', vnum=vnum, _query=[('Ln' if x.get('Active') == '1' else 'TmpLn', x.get('Culture'))] + extra_link_args))]) for x in xml.findall('./LANG')]
		else:
			other_langs = []
		...
		if data.IN_CIC_VIEW:
			full_info['org_link'] = request.host_url + request.passvars.makeDetailsLink(data.NUM, [('TmpLn' if tmp_culture else 'Ln', cur_culture)] + extra_link_args)
		...
		full_info.update([
			...
			('fields', fields),
			('volunteer_api_url', makeLink('~/volunteer/volunteer2.asp', [('api', 'on')] + extra_link_args)),
		])
