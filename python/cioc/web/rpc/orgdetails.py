...
# 3rd party
from pyramid.httpexceptions import HTTPUnauthorized, HTTPInternalServerError, HTTPNotFound
...
def link_service_name_level(request, record, level):
	...
	if getattr(record, 'LINK_SERVICE_NAME_%d' % level) and not (org_data == loc_data and record.LINK_LOCATION_NAME):
...
def link_location_name(request, record):
	...
	if record.LINK_LOCATION_NAME and org_data:
		crit = [(u'ORG_NUM', record.ORG_NUM or record.NUM), ('LL1', org_data)]
		return request.host_url + request.passvars.makeLink('~/rpc/orgsearch.asp', crit)
...
class RpcOrgDetails(viewbase.CicViewBase):
	...
	def __call__(self):
		...
		with request.connmgr.get_connection() as conn:
			fields = conn.execute('EXEC sp_CIC_View_DisplayFields ?, ?, ?, ?, ?, ?',
						num, viewdata.ViewType, False, False, None, None).fetchall()

			sql = ['''DECLARE @ViewType int
					SET @ViewType = ?
					...
					(SELECT Culture,LangID,LanguageName,LanguageAlias,LCID,Active
						FROM STP_Language LANG WHERE LangID<>@@LANGID AND dbo.fn_CIC_RecordInView(bt.NUM,@ViewType,LangID,0,GETDATE())=1 AND ''', "ActiveRecord=1" if viewdata.ViewOtherLangs else "EXISTS(SELECT * FROM CIC_View_Description WHERE ViewType=@ViewType AND LangID=LANG.LangID)",
						'''ORDER BY CASE WHEN Active=1 THEN 0 ELSE 1 END, LanguageName FOR XML AUTO) AS RECORD_LANG,''']

			if viewdata.UseSubmitChangesTo:
				sql.append("ISNULL(btd.SUBMIT_CHANGES_TO_PROTOCOL, 'https://') + btd.SUBMIT_CHANGES_TO AS FEEDBACK_LINK,")

			if viewdata.MapSearchResults:
				sql.append('bt.LATITUDE, bt.LONGITUDE,')

			exclude = {'RSN', 'NUM', 'RECORD_OWNER', 'NON_PUBLIC', 'DELETION_DATE', 'MODIFIED_DATE', 'UPDATE_DATE', 'UPDATE_SCHEDULE'}
			exclude.update('ORG_LEVEL_%d' % i for i in range(1, 6))
			sql.append(','.join(x.FieldSelect for x in fields if x.FieldName not in exclude))

			sql.append(''',btd.NUM AS LangNUM
		...
		if not data.IN_VIEW:
			return make_401_error(_(u'Record not in view', request))
		...
		field_groups = [
			{
				'name': k,
				'fields': [OrderedDict([('value', htmlvalue(x)), ('name', x.FieldName), ('display_name', x.FieldDisplay), ('allow_html', x.CheckHTML)]) for x in g]
		...
		if data.RECORD_LANG:
			xml = ET.fromstring((u'<langs>%s</langs>' % data.RECORD_LANG).encode('utf-8'))
			other_langs = [OrderedDict([('name', x.get('LanguageName')), ('culture', x.get('Culture')), ('link', route_url('rpc_orgdetails', num=num, _query=[('Ln' if x.get('Active') == '1' else 'TmpLn', x.get('Culture'))] + extra_link_args))]) for x in xml.findall('./LANG')]
		else:
			other_langs = []
		...
		full_info.update(('ORG_LEVEL_%d' % i, getattr(data, 'ORG_LEVEL_%d' % i)) for i in range(1, 6))
		full_info['LOCATION_NAME'] = data.LOCATION_NAME
		full_info.update(('SERVICE_NAME_LEVEL_%d' % i, getattr(data, 'SERVICE_NAME_LEVEL_%d' % i)) for i in range(1, 3))
		if viewdata.LinkOrgLevels:
			full_info.update(('ORG_LEVEL_%d_SEARCH' % i, link_org_level(request, data, i)) for i in range(1, 5))
			full_info['LOCATION_NAME_SEARCH'] = link_location_name(request, data)
			full_info.update(('SERVICE_NAME_LEVEL_%d_SEARCH' % i, link_service_name_level(request, data, i)) for i in range(1, 2))
