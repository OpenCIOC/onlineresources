...
_info_icon_template = Markup(
    """<span class="ui-button-icon-primary ui-icon ui-icon-info" aria-hidden="true"></span>"""
)
...
def render_ui(request):
    if request.viewdata.dom.GoogleTranslateWidget:
        ...
        if request.viewdata.dom.GoogleTranslateDisclaimer:
            ...
            disclaimer = Markup(
                """<div id="google-translate-dialog" data-title="%s" data-modal="true" style="display: none;">%s</div>"""
                % (about_gtranslate, request.viewdata.dom.GoogleTranslateDisclaimer)
            )
            disclaimer = Markup(
                """<button class="ui-corner-all ui-widget ui-state-default ui-button ui-button-text-icon-primary" data-toggle="dialog" data-target="#google-translate-dialog" role="button">%s <span class="ui-button-text">%s</span></button>%s"""
            ) % (_info_icon_template, about_gtranslate, disclaimer)
        else:
            disclaimer = ""
        return (
            Markup(
