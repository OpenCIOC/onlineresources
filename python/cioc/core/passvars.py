...
class PassVars(object):
	...
	def initialize(self):
		...
		self.setDefaultCultureVars(request.dboptions.DefaultCulture or syslanguage.CULTURE_ENGLISH_CANADIAN, True)
	...
	def _get_http_items(self, exclude_keys=None):
		...
		if not exclude_keys:
			exclude_keys = ()
		elif isinstance(exclude_keys, basestring):
			exclude_keys = (exclude_keys,)

		items = [(key, val) for (key, val) in self.httpvals.iteritems() if key not in exclude_keys]
		return items
	...
	def makeDetailsLink(self, num, httpvals=None, exclude_keys=None):
		if self.record_root:
			return self.makeLink(posixpath.join(self.path_from_start, self.record_root, str(num)), httpvals, exclude_keys)

		vals = {'NUM': num}
		if httpvals:
			if isinstance(httpvals, basestring):
				httpvals = dict((k, ','.join(v)) for (k, v) in parse_qs(httpvals, True).iteritems())
			vals.update(httpvals)

		return self.makeLink(posixpath.join(self.path_from_start, 'details.asp'), vals, exclude_keys)

	def makeVOLDetailsLink(self, vnum, httpvals=None, exclude_keys=None):
		if self.record_root:
			return self.makeLink(posixpath.join(self.path_from_start, 'volunteer', self.record_root, str(vnum)), httpvals, exclude_keys)
		...
		return self.makeLink(posixpath.join(self.path_from_start, 'volunteer', 'details.asp'), vals, exclude_keys)
		...
		if httpvals:
			if isinstance(httpvals, basestring):
				...
				if isinstance(httpvals, dict):
					httpvals = httpvals.items()

				force_utf8 = lambda x: x if not isinstance(x, unicode) else x.encode('utf-8')
				vars.append(urlencode([(force_utf8(k), force_utf8(v)) for k, v in httpvals]))
	...
	def route_url(self, route_name, exclude_keys=None, **kw):
		return self._route_core(lambda x: self.request.route_url(route_name, **x), exclude_keys, **kw)

	def route_path(self, route_name, exclude_keys=None, **kw):
		return self._route_core(lambda x: self.request.route_path(route_name, **x), exclude_keys, **kw)

	def current_route_path(self, exclude_keys=None, **kw):
		return self._route_core(lambda x: self.request.current_route_path(**x), exclude_keys, **kw)

	def makeLinkAdmin(self, url, httpvals=None):
		if not httpvals:
			httpvals = {}
		elif isinstance(httpvals, basestring):
			httpvals = dict((k, ','.join(v)) for (k, v) in parse_qs(httpvals, True).iteritems())
