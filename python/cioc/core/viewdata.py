...
non_bool_view_values = {
	'ViewType', 'ViewName', 'HidePastDueBy', 'Template', 'PrintTemplate',
	'CommSrchWrapAt', 'PB_ID', 'QuickListDropDown', 'QuickListName', 'QuickListWrapAt', 'QuickListPubHeadings',
	'TaxDefnLevel', 'Title', 'BottomMessage', 'Culture',
	'CommunitySetID', 'AreaServed', 'CanSeeNonPublicPub', 'AlsoNotify', 'AssignSuggestionsTo',
	'SearchTitleOverride', 'OrganizationNames', 'OrganizationsWithWWW',
	'OrganizationsWithVolOps', 'BrowseByOrg', 'FindAnOrgBy',
...
class ViewData(object):
	...
	def __init__(self, request):
		...
		with request.connmgr.get_connection() as conn:
			...
			if self.cic.ViewType != passvars.UseViewCIC and self.cic.ViewType != request.params.get('UseCICVwTmp', None):
			...
			tmpl = u'<div id="page-message-%s" class="page-message">%s</div>'
			page_messages = ((x.PageMsgID, x.PageMsg) for x in cursor.fetchall() if x.VisiblePrintMode or not self.PrintMode)
			self.PageMsgs = u"".join(tmpl % x for x in page_messages)

			cursor.close()

		pv_use_cic_vw = passvars.UseViewCIC

		if (use_view_cic_tmp is not None and use_view_cic_tmp != self.cic.ViewType) or (pv_use_cic_vw is not None and pv_use_cic_vw != self.cic.ViewType):
			passvars.UseViewCIC = None

		pv_use_vol_vw = passvars.UseViewVOL
		if (use_view_vol_tmp is not None and use_view_cic_tmp != self.vol.ViewType) or (pv_use_vol_vw is not None and pv_use_vol_vw != self.vol.ViewType):
	...
	def WhereClauseCIC(self):
		...
		if self.request.dboptions.OtherMembersActive:
			parts.append("(bt.MemberID=%d OR EXISTS(SELECT * FROM GBL_BT_SharingProfile pr "
				...
				"WHERE NUM=bt.NUM AND ShareMemberID_Cache=%d))" % (self.request.dboptions.MemberID, self.cic.ViewType, self.request.dboptions.MemberID))
		...
		pbid = cic.PB_ID
		if pbid:
			parts.append("(EXISTS(SELECT pb.BT_PB_ID FROM CIC_BT_PB pb WHERE pb.NUM=bt.NUM AND pb.PB_ID=%d))" % pbid)

		past_due = cic.HidePastDueBy
		if past_due is not None:
			parts.append("(btd.UPDATE_SCHEDULE IS NOT NULL AND (DATEDIFF(d,btd.UPDATE_SCHEDULE,GETDATE()) < %d))" % past_due)
	...
	def WhereClauseVOL(self):
		...
		if self.request.dboptions.OtherMembersActive:
			parts.append("(vo.MemberID=%d OR EXISTS(SELECT * FROM VOL_OP_SharingProfile pr "
				...
				"WHERE VNUM=vo.VNUM AND ShareMemberID_Cache=%d))" % (self.request.dboptions.MemberID, self.vol.ViewType, self.request.dboptions.MemberID))
		elif self.request.dboptions.OtherMembers:
			parts.append("(vo.MemberID=%d)" % self.request.dboptions.MemberID)

		com_set = vol.CommunitySetID
		if com_set:
			parts.append("EXISTS(SELECT * FROM VOL_OP_CommunitySet vcs "
		...
		if past_due is not None:
			parts.append("(vod.UPDATE_SCHEDULE IS NOT NULL AND "
	...
	def PrintMode(self):
		...
		print_mode = request.params.get('PrintMd') == 'on' and (request.user or request.dboptions.PrintModePublic)
		if print_mode:
			...
			if request.pageinfo.ThisPageFull.startswith('record/'):
				...
				print_mode = bool(_printmode_cic_details_re.match(request.pageinfo.ThisPage))
...
import re
_printmode_re = re.compile("^(details.asp)|(browseby.*)|(mailform.asp)|(processRecordList.asp)|(report_.*)|(.*results.asp)|(.*_list.asp)|(.*stats.*)|(whatsnew.asp)|(chklst_edit.asp)|(users(_history)?.asp)|(viewlist.asp)|(checklist)|(listvalues)|(interests)|(publication)$", re.I)
