...
class ReadlineCountingProxy(object):
	...
	def readline(self, size=0):
		...
		while line:
			if line.startswith('<script ') and 'language="python"' in line and 'runat="server"' in line:
				self.in_python = True
...
def extract_asp(fileobj, keywords, comment_tags, options):
	...
	for (lineno, funcname, message, comments) in extract_python(proxy, keywords, comment_tags, options):
