...
def get_view_template_info(request):
	...
	with request.connmgr.get_connection() as conn:
		...
		cursor = conn.execute('EXEC sp_GBL_Template_s ?, ?, ?', request.MemberID, template_id, preview_template_id)
	...
	template_values = dict(zip((d[0] for d in template_values.cursor_description), template_values))
	...
	template_urls.extend(glob.glob(os.path.join(const._sass_dir, 'bootstrap', '*.scss')))
	template_urls.extend(glob.glob(os.path.join(const._sass_dir, 'bootstrap', 'mixins', '*.scss')))
	...
	version_dates = [int(os.path.getmtime(os.path.join(_system_layout_dir, x)) * 100) for x in template_urls]
	version_dates.append(int(time.mktime(template_values['VersionDate'].timetuple()) * 100))

	template_values['VersionDate'] = max(version_dates)
	template_values['menus'] = dict((k, list(g)) for k, g in itertools.groupby(menu_items, lambda x: x.MenuType))

	request.template_values = template_values


class RenderInfo(object):
	def __init__(self, request, page_name, doc_title, no_cache, no_index, print_table, add_to_header="", focus="", show_message=False):
...
def render_css(request):
	...
	base_template += ''.join('$' + k + ':' + (v or 'null') + ' !default;\n' for k, v in css_values.items())
	...
	if request.matchdict['which'] == 'basic':
		...
		jqui_css = inline_css_file(os.path.join(_template_dir, 'jqueryui', 'jquery.ui.all.css'))
		...
		# theme
		base_template += '@import "ciocthemesetup";\n @import "ciocbasic";\n@import "theme";\n'
...
_jqui_icon = jquiicons.jQueryUIIcons(os.path.join(_template_dir, 'jqueryui', 'images', 'ui-icons_222222_256x240.png'))
...
def get_css_template_info(request):
	...
	with request.connmgr.get_connection() as conn:
		cursor = conn.execute('EXEC sp_GBL_Template_s_css ?, ?', request.dboptions.MemberID, template_id)
	...
	return {'template_values': dict(zip((d[0] for d in values.cursor_description), values)) if values else None,
			'layout_css': layout_css}

_jquery_sections = ['Content', 'Header', 'Default', 'Hover', 'Active', 'Highlight', 'Error']
_jquery_defaults = [(x % y, '') for x in ['bgImgUrl%s', 'bg%sXPos', 'bg%sYPos', 'bg%sRepeat']
...
_jquery_variables = [p + s for s in _jquery_sections for p in _jquery_prefixes] + ['fsDefault', 'cornerRadius']

_cioc_sections = ['Title', 'Footer', 'Menu', 'Info']
_cioc_variables_like_jquery = [p + s for p in _jquery_prefixes for s in _cioc_sections]
_cioc_variables = ['Background', 'BackgroundColour', 'bgColorLogo', 'FontFamily', 'FontColour', 'FieldLabelColour', 'MenuFontColour', 'MenuBgColour', 'TitleFontColour', 'TitleBgColour', 'LinkColour', 'ALinkColour', 'VLinkColour', 'AlertColour', 'bgColorInfo', 'fcInfo', 'fcLabel', 'BannerRepeat', 'BannerHeight', 'ContainerFluid', 'ContainerContrast', 'SmallTitle']
_all_variables = frozenset(_jquery_variables + _cioc_variables + _cioc_variables_like_jquery)
...
def fixup_css_values(values):
	...
	retval.update((k, str(v) if v is not None else 'null') for k, v in values.items() if k in _all_variables)

	icon_template = 'url(/styles/d/%d/images/ui-icons_%s_256x240.png)'
	icon_update = int(_jqui_icon.get_mtime())

	icon_values = ((section, (retval['iconColor' + section] or '000000').replace('#', '')) for section in _jquery_sections + _cioc_sections)
	retval.update(('icons' + section, icon_template % (icon_update, colour)) for (section, colour) in icon_values)
...
_html_operator = {
	'IF': _html_operator_if, 'FOR': _html_operator_for, 'FN': _html_operator_fn, 'IFNOT': _html_operator_ifnot,
	'IFANY': _html_operator_ifany, 'IFALL': _html_operator_ifall,
	'IFNOTANY': _html_operator_ifnotany, 'IFNOTALL': _html_operator_ifnotall
}

_html_block_re = re.compile(r'\[(IF|FOR|FN|IFNOT|IFANY|IFALL|IFNOTANY|IFNOTALL):([^]\s]+)\](.*?)\[END\1:\2\]', re.I | re.DOTALL)
...
class LayoutHeader(object):
	...
	def __call__(self):
		...
		if domain_root is not None:
			vd_dom = request.viewdata.cic if pageinfo.DbArea == const.DM_CIC else request.viewdata.vol
			if template_values['HeaderSearchLink']:
				icon = ''
				if template_values['HeaderSearchIcon']:
					if request.user:
						icon = u'<span class="glyphicon glyphicon-menu-hamburger" aria-hidden="true"></span> '
					else:
						icon = u'<span class="glyphicon glyphicon-search" aria-hidden="true"></span> '
				search_menu.append({'Link': passvars.makeLink('~/' + domain_root), 'Display': icon + (gettext('Main Menu', request) if request.user else gettext('Search', request))})

			if template_values['HeaderSuggestLink']:
				icon = ''
				if template_values['HeaderSuggestIcon']:
					if pageinfo.DbArea == const.DM_CIC:
						icon = u'<span class="glyphicon glyphicon-hand-right" aria-hidden="true"></span> '
					else:
						icon = u'<span class="glyphicon glyphicon-pushpin" aria-hidden="true"></span> '
				...
				site_menu.append({
					'Link': passvars.makeLink(PathToStart + domain_root + 'feedback.asp'),
					'Display': icon + label})

			if request.user and request.user.dom:
				if not (request.pageinfo.DbArea == const.DM_CIC and not dboptions.UseCIC):
					search_menu.append({'Link': passvars.makeLink(''.join((pageinfo.PathToStart, domain_root, 'advsrch.asp'))), 'Display': gettext('Advanced Search', request)})
				search_menu.append({'Link': passvars.makeLink(''.join((pageinfo.PathToStart, domain_root, 'recentsearch'))), 'Display': gettext('Recent Searches', request)})

			if pageinfo.DbArea != const.DM_GLOBAL:
				new_search_link = passvars.makeLink(pageinfo.PathToStart + domain_root)
				suggest_link = passvars.makeLink(pageinfo.PathToStart + domain_root + 'feedback.asp')
		...
		view_info_text = None
		help_link = None
		if ('SITE_BAR_MENU' in header_layout_template or 'SITE_BAR_ITEMS' in header_layout_template) and request.user and not request.viewdata.PrintMode:
			site_bar_menu = []
			if vd_dom:
				view_info_text = u'%s%s %s' % (gettext('View', request), gettext(':', request), vd_dom.ViewName)
				site_bar_menu.append(u'<li><a>%s</a></li>' % view_info_text)

			menu_items = []
			if domain_root is None:  # const.DM_GLOBAL
				menu_items.append((passvars.makeLink('~/'),
							gettext('CIC Menu', request)))
				if dboptions.UseVOL:
					menu_items.append((passvars.makeLink('~/volunteer/'),
							gettext('Volunteer Menu', request)))

				if request.user.SuperUser:
					icon = u'<span class="glyphicon glyphicon-wrench" aria-hidden="true"></span> '
					menu_items.append((passvars.makeLinkAdmin('setup.asp'),
				...
				if user_dom.CanEditRecord or user_dom.CanAddRecord or user_dom.FeedbackAlert:
					icon = u'<span class="glyphicon glyphicon-comment" aria-hidden="true"></span> '
					menu_items.append((passvars.makeLink(PathToStart + domain_root + 'revfeedback.asp'),
							icon + gettext('Feedback', request)))

				icon = u'<span class="glyphicon glyphicon-file" aria-hidden="true"></span> '
				if user_dom.CanAddRecord:
					...
					menu_items.append((passvars.makeLink(PathToStart + domain_root + 'entryform.asp'),
							icon + label))
					...
					menu_items.append((passvars.makeLink(PathToStart + domain_root + 'feedback.asp'),
							icon + label))
			...
			if request.user.NoticeCount or past_due:
				icon = Markup(u'<span class="ui-state-error" style="background-color: transparent; border: none;"><span class="ui-icon ui-icon-alert" style="display: inline-block; background-color: transparent; border: none;">%s</span></span> ') % (gettext('Some reminders are past due or require action.', request))
			elif reminders:
				icon = Markup(u'<img src="' + request.pageinfo.PathToStart + 'images/remind.gif" style="vertical-align: text-top"> ')
			else:
				icon = u''

			menu_items.append((passvars.makeLink('~/reminders'), gettext('Reminders', request), icon))

			icon = u'<span class="glyphicon glyphicon-user" aria-hidden="true"></span> '
			menu_items.append((passvars.makeLinkAdmin('account.asp'), icon + request.user.Login))
			...
			if pageinfo.HasHelp:
				icon = u'<span class="glyphicon glyphicon-question-sign" aria-hidden="true"></span> '
				help_link = passvars.makeLinkAdmin('pagehelp', {'Page': pageinfo.ThisPageFull})
				site_bar_menu.append(u'<li><a href="%s" target="pHelp" onClick="openWinL(\'%s\', \'pHelp\')">%s</a></li>' % (escape(help_link), escape(help_link.replace("'", "\\'")), escape(gettext('Page Help', request))))
		...
		mylist = ''
		if domain_root is not None:
			mylist = ((u'<a id="myListLink" href="%s" class="ListUI" style="display:none;">'
						'<span aria-hidden="true" class="glyphicon glyphicon-list-alt"></span> %s'
						' (<span id="myListCount"></span>)</a>') %
							(passvars.makeLink(''.join((pageinfo.PathToStart, domain_root, 'viewlist.asp'))),
		...
		logo_alt_text = escape(template_values['LogoAltText'] or template_values['LogoLink'])
		...
		namespace.update({
			...
			'LOGO_ALT_TEXT_ORIG': template_values['LogoAltText'] if not logolink else None,
			...
			'PAGE_TITLE': '<span id="page_name">%s</span>' % self.renderinfo.page_name,
			'PRINT_MODE': request.viewdata.PrintMode,
			...
			'SITE_MENU': site_menu,
			'SITE_MENU_GROUP_1': [encode_link_values(x) for x in menu_groups.get(1, [])],
			'SITE_MENU_GROUP_2': [encode_link_values(x) for x in menu_groups.get(2, [])],
			'SITE_MENU_GROUP_3': [encode_link_values(x) for x in menu_groups.get(3, [])],
			'SITE_MENU_GROUP_4': [encode_link_values(x) for x in menu_groups.get(4, [])],
...
class LayoutFooter(object):
	...
	def __call__(self):
		...
		menu_groups = {}
		for key, group in itertools.groupby(template_values['menus'].get('footer', []), lambda x: x.MenuGroup):
			menu_groups[key] = list(group)
		...
		terms_of_use_label = template_values['TermsOfUseLabel'] or gettext('Terms of Use', request)
		...
		namespace = {
			...
			'NOT_LOGGED_IN': not request.user,
			'MAKE_LINK': make_linkify_fn(request),
			'FOOTER_MENU_GROUP_1': [encode_link_values(x) for x in menu_groups.get(1, [])],
			'FOOTER_MENU_GROUP_2': [encode_link_values(x) for x in menu_groups.get(2, [])],
			'FOOTER_MENU_GROUP_3': [encode_link_values(x) for x in menu_groups.get(3, [])],
		}

		namespace['HAS_CONTACT_SECTION'] = any(namespace[x] for x in ['FOOTER_NOTICE_CONTACT','CONTACT_AGENCY','CONTACT_ADDRESS','CONTACT_PHONE','CONTACT_EMAIL','CONTACT_WEB','CONTACT_FACEBOOK','CONTACT_TWITTER','CONTACT_INSTAGRAM','CONTACT_LINKEDIN','CONTACT_YOUTUBE'])

		return apply_html_values(layout_template, namespace)


from lxml.html.clean import Cleaner
html_cleaner = Cleaner(add_nofollow=True)
...
def sanitize_html(html):
	...
	return html_cleaner.clean_html('<div>' + html + '</div>')[5:-6]
