...
def Crypt(salt, password, repeat):
    ...
    return (
        base64.b64encode(pbkdf2.derive(password.encode("utf-8")))
        .decode("ascii")
        .strip()
    )
...
class UserType:
    ...
    def _get(self, key):
        ...
        SuperUserGlobal = (
            (not not user_type.SuperUserGlobal or TechAdmin) if user_type else False
        )
        SuperUser = (
            (not not user_type.SuperUser or SuperUserGlobal) if user_type else False
        )
...
class User:
    def __init__(self, request):
        ...
        if user_info:
            self.Mod = (
                self.Initials
                if request.dboptions.UseInitials
                else " ".join((self.FirstName, self.LastName))
            )
        else:
            self.Mod = None
    ...
    def _process_login(self):
        ...
        with request.connmgr.get_connection() as conn:
            cursor = conn.execute(
                "EXEC dbo.sp_GBL_Users_s_Security ?, ?, ?",
            ...
            if http_auth:
                ...
                hash = Crypt(
                    user.PasswordHashSalt.strip(), password, user.PasswordHashRepeat
                )
                with request.connmgr.get_connection("admin") as conn2:
                    conn2.execute(
                        "EXEC dbo.sp_GBL_Users_u_Login ?,?,?,?,?",
                        request.dboptions.MemberID,
                        user.UserName,
                ...
                if hash != user.PasswordHash:
                    if (
                        retry_limit
                        and login_attempts + 1 >= retry_limit
                        and (user.AgencyEmail or user.Email)
                    ):
                        ...
                        send_email(
                            ...
                            from_,
                            to,
                            gettext("Locked Account in your CIOC database", request),
                            body,
                        )
...
_non_logged_in_page_whitelist = [
    "login.asp",
    "login_check.asp",
    "logout.asp",
    "security_failure.asp",
    "clbcexport.mvc",
    "clbcupdate.mvc",
    "billinginfo",
]
...
def is_basic_security_failure(request, require_login):
    ...
    if (
        not request.dboptions.AllowPublicAccess
        and not user
        and pageinfo.ThisPage.lower() not in _non_logged_in_page_whitelist
    ):
        return True
    ...
    if (
        pageinfo.DbArea == const.DM_CIC
        and not request.dboptions.UseCIC
        and not user
...
def do_login(request, principal, login_key):
    ...
    request.session.adjust_session_timeout(3600 * 24 * 5)
