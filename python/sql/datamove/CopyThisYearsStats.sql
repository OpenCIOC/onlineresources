SELECT COUNT(*) FROM CIC_Stats_RSN WHERE AccessDate >= '2015-01-01'

DECLARE @STATFIRSTID int, @STATLASTID int, @STATCURRENTID int, @INCREMENT int

SELECT @STATFIRSTID = MIN(Log_ID), @STATLASTID = MAX(Log_ID) FROM CIC_Stats_RSN WHERE AccessDate >= '2015-01-01'
...
SELECT 
	...
	stat.LangID, 
	(SELECT CASE WHEN stat.User_ID IS NULL THEN NULL ELSE ISNULL(u2.User_ID, -1) END FROM GBL_Users u LEFT JOIN OntarioCioc_2012_11.dbo.GBL_Users u2 ON u.UserUID=u2.UserUID WHERE u.User_ID=stat.User_ID) AS User_ID,
	(SELECT vw.ViewType FROM CIC_View_Description vwd 
	 ...
	 WHERE vwd.ViewType=stat.ViewType AND vwd.LangID = 0 AND vw.MemberID=stat.MemberID) AS ViewType,
	stat.RobotID, stat.API
 FROM CIC_Stats_RSN stat 
 WHERE AccessDate >= '2015-01-01' AND Log_ID >= @STATCURRENTID AND Log_ID < (@STATCURRENTID + @INCREMENT)
...
SELECT COUNT(*) FROM VOL_Stats_OPID WHERE AccessDate >= '2015-01-01'
SELECT @STATFIRSTID = MIN(Log_ID), @STATLASTID = MAX(Log_ID) FROM VOL_Stats_OPID WHERE AccessDate >= '2015-01-01'
PRINT N'GETTING ROWS FROM ' + CAST(@STATFIRSTID AS nvarchar) + N' TO ' + CAST(@STATLASTID AS nvarchar)
