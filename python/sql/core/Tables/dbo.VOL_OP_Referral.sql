...
ALTER TABLE [dbo].[VOL_OP_Referral] ADD 
CONSTRAINT [PK_VOL_Referral] PRIMARY KEY CLUSTERED  ([REF_ID]) ON [PRIMARY]
CREATE NONCLUSTERED INDEX [IX_VOL_Referral] ON [dbo].[VOL_OP_Referral] ([VNUM]) ON [PRIMARY]

ALTER TABLE [dbo].[VOL_OP_Referral] WITH NOCHECK ADD
CONSTRAINT [CK_VOL_OP_Referral_ContactRequired] CHECK ((NOT ([VolunteerPhone] IS NULL AND [VolunteerEmail] IS NULL AND [VolunteerAddress] IS NULL)))
GO
