...
CREATE FUNCTION [dbo].[fn_GBL_SharingProfile_CIC_Fld_l](
	@MemberID int,
	@ViewType int,
	@FieldID int
)
...
SELECT @returnStr =  COALESCE(@returnStr + ',','') + CAST(shp.ProfileID AS varchar)
	...
	INNER JOIN GBL_SharingProfile_CIC_Fld shpf
...
GO
GRANT EXECUTE ON  [dbo].[fn_GBL_SharingProfile_CIC_Fld_l] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[fn_GBL_SharingProfile_CIC_Fld_l] TO [cioc_login_role]
GRANT EXECUTE ON  [dbo].[fn_GBL_SharingProfile_CIC_Fld_l] TO [cioc_vol_search_role]
GO
