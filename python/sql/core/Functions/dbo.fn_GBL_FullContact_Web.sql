...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[fn_GBL_FullContact_Web](
...
SELECT @returnStr = ISNULL(CMP_Name,'')
		...
		+ CASE WHEN EMAIL IS NULL THEN '' ELSE CASE WHEN COALESCE(CMP_Name,TITLE,ORG,CMP_PhoneFull,CMP_Fax) IS NULL THEN '' ELSE @semiColonStr END + cioc_shared.dbo.fn_SHR_STP_ObjectName('Email') + @colonStr + cioc_shared.dbo.fn_SHR_GBL_Link_Email(EMAIL) END
FROM GBL_Contact c
	WHERE (
...
GO
GRANT EXECUTE ON  [dbo].[fn_GBL_FullContact_Web] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[fn_GBL_FullContact_Web] TO [cioc_login_role]
GRANT EXECUTE ON  [dbo].[fn_GBL_FullContact_Web] TO [cioc_vol_search_role]
GO
