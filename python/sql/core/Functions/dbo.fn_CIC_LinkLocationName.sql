...
CREATE FUNCTION [dbo].[fn_CIC_LinkLocationName](
	@ViewType int,
	@NUM varchar(8),
	@ORG_NUM varchar(8),
...
WHERE btd.NUM<>@NUM
	...
	AND (
		...
		OR EXISTS(SELECT * FROM GBL_BT_LOCATION_SERVICE WHERE LOCATION_NUM=@NUM AND SERVICE_NUM=bt.NUM)
	)
...
GRANT EXECUTE ON  [dbo].[fn_CIC_LinkLocationName] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[fn_CIC_LinkLocationName] TO [cioc_login_role]
GO
