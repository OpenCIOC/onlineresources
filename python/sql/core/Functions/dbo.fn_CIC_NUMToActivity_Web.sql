...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[fn_CIC_NUMToActivity_Web](
	@NUM varchar(8),
...
SELECT @returnStr =  COALESCE(@returnStr + @conStr + @conStr,'') 
		+ dbo.fn_CIC_NUMToActivity_Unit_Web(
			[Status],
			ActivityName,
			ActivityDescription,
			Notes
...
GO
GRANT EXECUTE ON  [dbo].[fn_CIC_NUMToActivity_Web] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[fn_CIC_NUMToActivity_Web] TO [cioc_login_role]
GO
