...
CREATE FUNCTION [dbo].[fn_CIC_Reminders](
	@NUM varchar(8),
	@User_ID int,
	@LangID smallint,
	@Today smalldatetime
...
SET @returnVal = (SELECT COUNT(*) AS Total, SUM(CASE WHEN DueDate IS NOT NULL AND  DueDate<=@Today THEN 1 ELSE 0 END) AS PastDue
	FROM GBL_Reminder reminders
		WHERE Dismissed=0 
			...
			AND EXISTS(SELECT * FROM GBL_BT_Reminder WHERE reminders.ReminderID=ReminderID AND NUM=@NUM)
...
GO
GRANT EXECUTE ON  [dbo].[fn_CIC_Reminders] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[fn_CIC_Reminders] TO [cioc_login_role]
GO
