...
CREATE PROCEDURE [dbo].[sp_VOL_PrintProfile_l]
	@MemberID int,
	@ViewType int
...
WHERE Domain=2
	AND ((@ViewType IS NULL AND MemberID=@MemberID) OR EXISTS(SELECT * FROM VOL_View_PrintProfile vw WHERE vw.ViewType=@ViewType AND vw.ProfileID=pp.ProfileID))
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_PrintProfile_l] TO [cioc_login_role]
GO
