...
CREATE PROCEDURE [dbo].[sp_VOL_ExtraDropDown_l]
	@MemberID [int],
	...
	LEFT JOIN VOL_ExtraDropDown_Name exdn
		ON exd.EXD_ID=exdn.EXD_ID
			AND exdn.LangID=CASE
				...
				ELSE (SELECT TOP 1 LangID FROM VOL_ExtraDropDown_Name WHERE EXD_ID=exd.EXD_ID ORDER BY CASE WHEN LangID=@@LANGID THEN 0 ELSE 1 END, LangID)
			END
WHERE exd.FieldName=@FieldName
	...
	OR ((exd.MemberID IS NULL OR @MemberID IS NULL OR exd.MemberID=@MemberID)
		AND (
			@ShowHidden=1
			OR NOT EXISTS(SELECT * FROM VOL_ExtraDropDown_InactiveByMember WHERE EXD_ID=exd.EXD_ID AND MemberID=@MemberID)
			)
		))
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_ExtraDropDown_l] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[sp_VOL_ExtraDropDown_l] TO [cioc_login_role]
GRANT EXECUTE ON  [dbo].[sp_VOL_ExtraDropDown_l] TO [cioc_vol_search_role]
GO
