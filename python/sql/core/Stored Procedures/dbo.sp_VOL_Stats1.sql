...
CREATE PROCEDURE [dbo].[sp_VOL_Stats1]
	@MemberID int,
	@ViewType int,
	@Agency varchar(3),
	@SL_ID int,
...
WHERE st.MemberID=@MemberID
	...
	AND (@SL_ID IS NULL OR (EXISTS(SELECT * FROM GBL_Users us WHERE us.SL_ID_VOL=@SL_ID AND us.[User_ID]=st.[User_ID])))
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_Stats1] TO [cioc_login_role]
GO
