...
USING (SELECT bt2.NUM AS LOCATION_NUM, bt1.NUM AS SERVICE_NUM
	...
	INNER JOIN GBL_BaseTable bt1
		ON bt1.NUM=(SELECT TOP 1 NUM FROM GBL_BaseTable bt WHERE bt.NUM<>@NUM AND (bt.NUM=tm.SERVICE_ID OR (EXTERNAL_ID=tm.SERVICE_ID AND ((@SOURCE_DB_CODE IS NULL AND SOURCE_DB_CODE IS NULL) OR (@SOURCE_DB_CODE IS NOT NULL AND SOURCE_DB_CODE=@SOURCE_DB_CODE))))
...
SELECT @BadNUMs = COALESCE(@BadNUMs + ' ; ','') + bad.SERVICE_ID
	FROM (
		SELECT tm.SERVICE_ID FROM @ServiceIDs tm
			WHERE NOT EXISTS(SELECT * FROM GBL_BaseTable bt WHERE bt.NUM<>@NUM AND (bt.NUM=tm.SERVICE_ID OR (EXTERNAL_ID=tm.SERVICE_ID AND ((@SOURCE_DB_CODE IS NULL AND SOURCE_DB_CODE IS NULL) OR (@SOURCE_DB_CODE IS NOT NULL AND SOURCE_DB_CODE=@SOURCE_DB_CODE)))))
	) bad
