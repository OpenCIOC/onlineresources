...
CREATE PROCEDURE [dbo].[sp_THS_SBJ_UseInstead_With_sl_Count]
	@Subj_ID int,
	@ViewType int,
	@NoDeleted bit
...
SELECT sj.Subj_ID, sjn.Name AS SubjectTerm, COUNT(btd.NUM) AS UsageCount
	FROM THS_Subject sj
	...
	LEFT JOIN CIC_BT_SBJ pr
		ON ui.UsedSubj_ID=pr.Subj_ID
	INNER JOIN THS_SBJ_UseInstead ui2
		ON ui.Subj_ID=ui2.Subj_ID
			AND ui2.UsedSubj_ID=@Subj_ID
	LEFT JOIN CIC_BT_SBJ pr2
		ON ui2.UsedSubj_ID = pr2.Subj_ID
			AND pr.NUM=pr2.NUM
	LEFT JOIN GBL_BaseTable bt
		ON pr.NUM=bt.NUM 
			AND EXISTS(SELECT * FROM CIC_BT_SBJ pr2 WHERE pr2.NUM=bt.NUM AND pr2.Subj_ID=@Subj_ID)
...
HAVING sj.UseAll = 0
	AND @UseZeroSubjects=1 OR COUNT(btd.NUM) > 0
ORDER BY sjn.Name
...
GO
GRANT EXECUTE ON  [dbo].[sp_THS_SBJ_UseInstead_With_sl_Count] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[sp_THS_SBJ_UseInstead_With_sl_Count] TO [cioc_login_role]
GO
