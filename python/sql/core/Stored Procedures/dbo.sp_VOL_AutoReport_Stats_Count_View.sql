...
CREATE PROCEDURE [dbo].[sp_VOL_AutoReport_Stats_Count_View] (
	@MemberID int
)
...
SELECT DATENAME(mm, st.AccessDate) + ' ' + CAST(YEAR(st.AccessDate) AS varchar) AS TheMonth,
	...
	GROUP BY ViewName, YEAR(AccessDate), MONTH(AccessDate), DATENAME(mm, AccessDate)
...
GRANT EXECUTE ON  [dbo].[sp_VOL_AutoReport_Stats_Count_View] TO [cioc_login_role]
GO
