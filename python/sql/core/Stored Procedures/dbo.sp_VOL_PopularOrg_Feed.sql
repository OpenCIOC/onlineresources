...
WHERE (@FeedAPIKey IS NULL OR EXISTS(SELECT * FROM dbo.GBL_FeedAPIKey WHERE FeedAPIKey=@FeedAPIKey AND Inactive=0 AND VOL=1))
...
RETURN CASE WHEN @FeedAPIKey IS NULL OR EXISTS(SELECT * FROM dbo.GBL_FeedAPIKey WHERE FeedAPIKey=@FeedAPIKey AND Inactive=0 AND VOL=1) THEN 0 ELSE -1 END
