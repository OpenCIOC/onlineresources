...
-- Language is active ?
END ELSE IF NOT EXISTS(SELECT * FROM dbo.STP_Language WHERE LangID=@LangID AND Active=1) BEGIN
	SET @Error = 3 -- No Such Record
...
END ELSE IF EXISTS(SELECT * FROM dbo.CIC_View_Description WHERE ViewType=@ViewType AND LangID=@LangID) BEGIN
