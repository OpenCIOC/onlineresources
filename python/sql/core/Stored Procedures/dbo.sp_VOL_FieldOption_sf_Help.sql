...
CREATE PROCEDURE [dbo].[sp_VOL_FieldOption_sf_Help]
	@FieldID [int],
	@MemberID [int]
...
SELECT	fo.FieldID,
		fo.FieldName,
		(SELECT fod.FieldDisplay FROM VOL_FieldOption_Description fod WHERE fod.FieldID=fo.FieldID AND LangID=@@LANGID) AS FieldDisplay
...
GRANT EXECUTE ON  [dbo].[sp_VOL_FieldOption_sf_Help] TO [cioc_login_role]
GO
