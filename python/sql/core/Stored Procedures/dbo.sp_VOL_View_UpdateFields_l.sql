...
CREATE PROCEDURE [dbo].[sp_VOL_View_UpdateFields_l]
	@MemberID int,
	@AgencyCode char(3),
	@ViewType int
...
SELECT	fo.FieldID,
		...
		CASE WHEN EXISTS(SELECT * FROM VOL_View_UpdateField WHERE ViewType=@ViewType AND FieldID=fo.FieldID)
...
WHERE CanUseUpdate=1
	AND (fi.FieldID IS NULL OR EXISTS(SELECT * FROM VOL_View_UpdateField WHERE ViewType=@ViewType AND FieldID=fo.FieldID))
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_View_UpdateFields_l] TO [cioc_login_role]
GO
