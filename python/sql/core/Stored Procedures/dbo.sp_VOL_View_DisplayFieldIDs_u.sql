...
END ELSE BEGIN
	...
	IF @Error = 0 BEGIN
		INSERT INTO VOL_View_DisplayField (ViewType, FieldID) SELECT ViewType=@ViewType, tm.FieldID
			FROM @tmpFieldIDs tm
		WHERE NOT EXISTS(SELECT * FROM VOL_View_DisplayField pr WHERE ViewType=@ViewType AND pr.FieldID=tm.FieldID)
		EXEC @Error = cioc_shared.dbo.sp_STP_UnknownErrorCheck @@ERROR, @ViewFieldObjectName, @ErrMsg
		IF @Error = 0 BEGIN
			UPDATE VOL_View
