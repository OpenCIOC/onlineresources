...
CREATE PROCEDURE [dbo].[sp_CIC_SharingProfile_u]
...
-- Profile ID exists ?
END ELSE IF @ProfileID IS NOT NULL AND NOT EXISTS (SELECT * FROM GBL_SharingProfile WHERE ProfileID=@ProfileID AND Domain=1) BEGIN
	SET @Error = 3 -- No Such Record
...
IF @Error = 0 BEGIN
	...
	IF @Error=0 AND @ProfileID IS NOT NULL BEGIN
		...
		IF @CanUseAnyView=1 BEGIN
			DELETE FROM GBL_SharingProfile_CIC_View WHERE ProfileID=@ProfileID
		END ELSE BEGIN
			MERGE INTO GBL_SharingProfile_CIC_View vw
		...
		MERGE INTO GBL_SharingProfile_CIC_Fld fld
...
GRANT EXECUTE ON  [dbo].[sp_CIC_SharingProfile_u] TO [cioc_login_role]
GO
