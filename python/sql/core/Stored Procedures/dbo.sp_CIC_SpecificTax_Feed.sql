...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[sp_CIC_SpecificTax_Feed]
	@ViewType int,
...
WHERE (@FeedAPIKey IS NULL OR EXISTS(SELECT * FROM GBL_FeedAPIKey WHERE FeedAPIKey=@FeedAPIKey AND Inactive=0 AND CIC=1))
	...
	AND EXISTS(SELECT * FROM dbo.CIC_BT_TAX tl INNER JOIN dbo.CIC_BT_TAX_TM tlt ON tlt.BT_TAX_ID = tl.BT_TAX_ID WHERE tl.NUM=bt.NUM AND tlt.Code LIKE @Code + '%')
...
GRANT EXECUTE ON  [dbo].[sp_CIC_SpecificTax_Feed] TO [cioc_cic_search_role]
GO
