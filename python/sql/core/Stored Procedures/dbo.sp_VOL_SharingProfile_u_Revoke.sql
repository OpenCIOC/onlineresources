...
CREATE PROCEDURE [dbo].[sp_VOL_SharingProfile_u_Revoke]
	@ProfileID [int],
	@User_ID [int],
	@RevocationDate date,
	...
	IF @RevocationDate <= GETDATE() BEGIN
		DELETE vo
			...
			INNER JOIN VOL_OP_SharingProfile ops
				ON vo.VNUM = ops.VNUM AND ProfileID=@ProfileID 
				
		DELETE FROM VOL_OP_SharingProfile WHERE ProfileID=@ProfileID
	END
END
...
GRANT EXECUTE ON  [dbo].[sp_VOL_SharingProfile_u_Revoke] TO [cioc_login_role]
GO
