...
CREATE PROCEDURE [dbo].[sp_CIC_Feedback_s]
	@FB_ID int,
	@User_ID int,
	@ViewType int
...
END ELSE BEGIN
	SET @CanUpdateRecord = CASE
		WHEN EXISTS(SELECT * FROM CIC_SecurityLevel sl INNER JOIN GBL_Users u ON sl.SL_ID=u.SL_ID_CIC AND u.User_ID=@User_ID WHERE sl.CanAddRecord=1)
			AND EXISTS(SELECT * FROM GBL_FeedbackEntry fbe WHERE fbe.FB_ID=@FB_ID AND fbe.MemberID=@MemberID)
			THEN 1 ELSE 0 END
END
...
END ELSE IF NOT EXISTS (SELECT * FROM GBL_FeedbackEntry WHERE FB_ID=@FB_ID) BEGIN
	...
	SET @FB_ID = NULL
-- Record in View ?
END ELSE IF @NUM IS NOT NULL AND NOT dbo.fn_CIC_RecordInView(@NUM,@ViewType,@@LANGID,0,GETDATE())=1 BEGIN
...
SET @ExtraFieldSQL = 'SELECT fbe.FB_ID, 
...
GRANT EXECUTE ON  [dbo].[sp_CIC_Feedback_s] TO [cioc_login_role]
GO
