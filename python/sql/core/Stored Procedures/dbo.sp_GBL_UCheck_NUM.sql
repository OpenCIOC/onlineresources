...
END ELSE BEGIN
	SET @NUM = NULL
	SELECT @NUM=NUM FROM GBL_BaseTable WHERE EXTERNAL_ID=@EXTERNAL_ID AND ((@SOURCE_DB_CODE IS NULL AND SOURCE_DB_CODE IS NULL) OR (@SOURCE_DB_CODE IS NOT NULL AND SOURCE_DB_CODE=@SOURCE_DB_CODE))
	IF @NUM IS NOT NULL
		RETURN 1
	ELSE
		RETURN 0
