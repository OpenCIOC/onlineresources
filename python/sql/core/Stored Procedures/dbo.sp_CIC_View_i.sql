...
IF @Error = 0 BEGIN
    ...
    ELSE IF EXISTS (
        ...
        WHERE   ViewName = @ViewName AND MemberID = @MemberID
    ) BEGIN
        SET @Error = 6; -- Value in use
        SET @ErrMsg = cioc_shared.dbo.fn_SHR_STP_FormatError(@Error, @ViewName, @ViewNameObjectName);
    -- View we are copying exists ?
    END;
    ELSE IF @ViewType IS NOT NULL AND   NOT EXISTS (SELECT  * FROM  dbo.CIC_View WHERE  ViewType = @ViewType) BEGIN
        SET @Error = 3; -- No Such Record
    ...
    END;
    ELSE IF @ViewType IS NOT NULL AND   NOT EXISTS (
             SELECT *
             FROM   dbo.CIC_View
             WHERE  ViewType = @ViewType AND MemberID = @MemberID
         ) BEGIN
    ...
    ELSE BEGIN
        ...
        IF @DefaultView IS NULL BEGIN
            ...
            IF @DefaultTemplate IS NULL BEGIN
                EXEC @Error = sp_GBL_Template_Default_Check
                    @DefaultTemplate OUTPUT,
                    @ErrMsg OUTPUT;
            END;

            INSERT INTO dbo.CIC_View (CREATED_DATE, CREATED_BY, MODIFIED_DATE, MODIFIED_BY, MemberID, Template)
            VALUES
            (GETDATE(), @MODIFIED_BY, GETDATE(), @MODIFIED_BY, @MemberID, @DefaultTemplate);
        ...
        IF @Error = 0 AND   @DefaultView IS NOT NULL BEGIN
            ...
            SELECT  @DD_ID = DD_ID
            FROM    dbo.GBL_Display
            WHERE   ViewTypeCIC = @DefaultView;
            ...
            IF @DD_ID IS NOT NULL BEGIN
                ...
                IF @DD_ID_NEW IS NOT NULL BEGIN
                    ...
                    SELECT  @DD_ID_NEW, FieldID FROM    dbo.GBL_Display_Fld WHERE  DD_ID = @DD_ID;
                END;
            END;
            ...
            FETCH NEXT FROM FieldGroupCursor
            INTO
                @DisplayFieldGroupID,
                @DisplayFieldGroupDisplayOrder;
            WHILE @@FETCH_STATUS = 0 BEGIN
                INSERT INTO dbo.CIC_View_DisplayFieldGroup (ViewType, DisplayOrder)
                VALUES
                (@ViewType, @DisplayFieldGroupDisplayOrder);
                SET @NewDisplayFieldGroupID = SCOPE_IDENTITY();
                INSERT INTO dbo.CIC_View_DisplayFieldGroup_Name (DisplayFieldGroupID, LangID, [Name])
                SELECT
        ...
        ELSE BEGIN
            ...
            INSERT INTO dbo.CIC_View_DisplayFieldGroup_Name (DisplayFieldGroupID, LangID, [Name])
            VALUES
            (@DisplayFieldGroupID, @@LANGID, cioc_shared.dbo.fn_SHR_STP_ObjectName('Record Details'));
            INSERT INTO dbo.CIC_View_DisplayField (DisplayFieldGroupID, FieldID)
            SELECT
                @DisplayFieldGroupID AS DisplayFieldGroupID,
                FieldID
