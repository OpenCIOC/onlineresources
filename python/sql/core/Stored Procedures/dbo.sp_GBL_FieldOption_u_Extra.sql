...
CREATE PROCEDURE [dbo].[sp_GBL_FieldOption_u_Extra]
	@SuperUSerGlobal bit,
	@OwnerMemberID int,
	@MemberID int,
	@FieldID int,
...
IF @Error=0 BEGIN
	IF @FieldID IS NOT NULL AND NOT EXISTS (SELECT * FROM GBL_FieldOption WHERE FieldID=@FieldID AND (ExtraFieldType=@ExtraFieldType OR (ExtraFieldType IN ('a','d') AND @ExtraFieldType IN ('a','d')))) BEGIN
		SET @Error = 3 -- No Such Record
		SET @ErrMsg = cioc_shared.dbo.fn_SHR_STP_FormatError(@Error, CAST(@FieldID AS varchar), @FieldObjectName)
	END ELSE IF EXISTS(SELECT * FROM GBL_FieldOption WHERE FieldName=@FieldName AND (@FieldID IS NULL OR FieldID<>@FieldID)) BEGIN
	...
	END ELSE BEGIN
		IF @FieldID IS NULL BEGIN
			...
			IF @Error=0 AND @FieldID IS NOT NULL AND (SELECT COUNT(*) FROM STP_Member WHERE Active=1) > 1 BEGIN
				INSERT INTO GBL_FieldOption_InactiveByMember (
					FieldID,
					MemberID
				)
				SELECT	@FieldID,
			...
			EXEC dbo.sp_STP_RegenerateUserFields 1, @FieldName
		END ELSE BEGIN
			DECLARE @FieldNameChanged bit
			SELECT @FieldNameChanged = CASE WHEN FieldName<>@FieldName THEN CAST(1 AS bit) ELSE CAST(0 AS bit) END FROM GBL_FieldOption WHERE FieldID=@FieldID
...
GRANT EXECUTE ON  [dbo].[sp_GBL_FieldOption_u_Extra] TO [cioc_login_role]
GO
