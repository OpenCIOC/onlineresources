...
SELECT	fo.FieldID,
		...
		CASE WHEN EXISTS(SELECT * FROM VOL_View_DisplayField WHERE ViewType=@ViewType AND FieldID=fo.FieldID)
...
WHERE CanUseDisplay=1
	AND (fi.FieldID IS NULL OR EXISTS(SELECT * FROM VOL_View_DisplayField WHERE ViewType=@ViewType AND FieldID=fo.FieldID))
