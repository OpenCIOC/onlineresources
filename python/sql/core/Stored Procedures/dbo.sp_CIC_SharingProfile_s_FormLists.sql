...
CREATE PROCEDURE [dbo].[sp_CIC_SharingProfile_s_FormLists]
	@MemberID [int],
	@ProfileID [int]
...
SELECT	m.MemberID,
		ISNULL(memd.MemberNameCIC, memd.MemberName) AS MemberNameCIC
FROM STP_Member m
INNER JOIN STP_Member_Description memd
	ON m.MemberID=memd.MemberID AND LangID=(SELECT TOP 1 LangID FROM STP_Member_Description WHERE MemberID=memd.MemberID ORDER BY CASE WHEN MemberNameCIC IS NOT NULL THEN 0 ELSE 1 END, CASE WHEN LangID=@@LANGID THEN 0 ELSE 1 END, LangID)
WHERE m.MemberID<>@MemberID AND m.UseCIC=1
...
GRANT EXECUTE ON  [dbo].[sp_CIC_SharingProfile_s_FormLists] TO [cioc_login_role]
GO
