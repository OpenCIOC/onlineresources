...
CREATE PROCEDURE [dbo].[sp_VOL_FieldOption_u_Help]
	@FieldID int,
	@MemberID int,
	@GlobalSuperUser bit,
...
END ELSE IF NOT EXISTS(SELECT * FROM VOL_FieldOption WHERE FieldID=@FieldID) BEGIN
...
IF @Error = 0 BEGIN
	...
	IF @GlobalSuperUser = 1 BEGIN
		MERGE INTO VOL_FieldOption_Description dst
		USING (
	...
	WHERE HelpTextMember IS NOT NULL AND 
		NOT EXISTS(SELECT * FROM VOL_FieldOption_Description WHERE FieldID=@FieldID AND LangID=nf.LangID)
	EXEC @Error = cioc_shared.dbo.sp_STP_UnknownErrorCheck @@ERROR, @FieldObjectName, @ErrMsg
	
	MERGE INTO VOL_FieldOption_HelpByMember dst
	USING (
...
GRANT EXECUTE ON  [dbo].[sp_VOL_FieldOption_u_Help] TO [cioc_login_role]
GO
