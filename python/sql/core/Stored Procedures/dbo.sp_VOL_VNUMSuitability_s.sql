...
SELECT sb.SB_ID, sbn.LangID, CASE WHEN sbn.LangID=@@LANGID THEN sbn.Name ELSE '[' + sbn.Name + ']' END AS SuitableFor, 
	...
	INNER JOIN dbo.VOL_Suitability_Name sbn
		ON sb.SB_ID=sbn.SB_ID AND sbn.LangID=(SELECT TOP 1 LangID FROM dbo.VOL_Suitability_Name WHERE SB_ID=sbn.SB_ID ORDER BY CASE WHEN LangID=@@LANGID THEN 0 ELSE 1 END, LangID)
	LEFT JOIN dbo.VOL_OP_SB pr
...
WHERE pr.OP_SB_ID IS NOT NULL
	...
	OR (sb.MemberID IS NULL AND (
		NOT EXISTS(SELECT * FROM dbo.VOL_Suitability_InactiveByMember WHERE SB_ID=sb.SB_ID AND MemberID=ISNULL(vo.MemberID, @MemberID))
	))
