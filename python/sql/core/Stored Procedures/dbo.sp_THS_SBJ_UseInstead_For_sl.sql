...
CREATE PROCEDURE [dbo].[sp_THS_SBJ_UseInstead_For_sl]
	@Subj_ID int,
	@ViewType int
...
WHERE (@UseLocalSubjects=1 OR Authorized=1)
	...
	AND NOT EXISTS(SELECT * FROM THS_SBJ_UseInstead ui2 WHERE ui2.Subj_ID=sj.Subj_ID AND ui2.UsedSubj_ID<>@Subj_ID)
GROUP BY sj.Subj_ID, sjn.Name, sj.UseAll
HAVING COUNT(sj.Subj_ID)=1 OR sj.UseAll = 0
ORDER BY sjn.Name
...
GO
GRANT EXECUTE ON  [dbo].[sp_THS_SBJ_UseInstead_For_sl] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[sp_THS_SBJ_UseInstead_For_sl] TO [cioc_login_role]
GO
