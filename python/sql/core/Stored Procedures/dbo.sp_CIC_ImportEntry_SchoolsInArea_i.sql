...
CREATE PROCEDURE [dbo].[sp_CIC_ImportEntry_SchoolsInArea_i]
	@NUM varchar(8),
	...
	IF @BT_SCH_ID IS NOT NULL BEGIN
		IF @HAS_ENGLISH=1 BEGIN
			...
			IF @NotesEn IS NULL BEGIN
				DELETE FROM CCR_BT_SCH_Notes WHERE BT_SCH_ID=@BT_SCH_ID AND EscortNotes IS NULL AND LangID=0
			END ELSE BEGIN
				IF EXISTS(SELECT * FROM CCR_BT_SCH_Notes WHERE BT_SCH_ID=@BT_SCH_ID AND LangID=0) BEGIN
					UPDATE CCR_BT_SCH_Notes
						SET InAreaNotes = @NotesEn
		...
		IF @HAS_FRENCH=1 BEGIN
			...
			IF @NotesFr IS NULL BEGIN
				DELETE FROM CCR_BT_SCH_Notes WHERE BT_SCH_ID=@BT_SCH_ID AND EscortNotes IS NULL AND LangID=2
			END ELSE BEGIN
				IF EXISTS(SELECT * FROM CCR_BT_SCH_Notes WHERE BT_SCH_ID=@BT_SCH_ID AND LangID=2) BEGIN
					UPDATE CCR_BT_SCH_Notes
						SET InAreaNotes = @NotesFr
...
GO
GRANT EXECUTE ON  [dbo].[sp_CIC_ImportEntry_SchoolsInArea_i] TO [cioc_login_role]
GO
