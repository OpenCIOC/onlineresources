...
CREATE PROCEDURE [dbo].[sp_VOL_PrintProfile_Fld_l]
	@MemberID int,
	@ProfileID int
...
UNION SELECT fo.FieldName, ISNULL(FieldDisplay, FieldName) AS FieldDisplay,
		ftn.FieldType, pf.*,
		(SELECT COUNT(*) FROM GBL_PrintProfile_Fld_FindReplace pfr WHERE pfr.PFLD_ID=pf.PFLD_ID) AS FindReplaceCount,
		(SELECT pfd.*, l.Culture
			FROM GBL_PrintProfile_Fld_Description pfd
	...
	LEFT JOIN VOL_FieldOption_Description fod
		ON fo.FieldID=fod.FieldID AND fod.LangID=@@LANGID
	INNER JOIN GBL_PrintProfile_Fld pf
		ON fo.FieldID=pf.VOLFieldID AND ProfileID=@ProfileID
	INNER JOIN GBL_PrintProfile_Fld_Type_Name ftn
		ON pf.FieldTypeID=ftn.FieldTypeID AND ftn.LangID=(SELECT TOP 1 LangID FROM GBL_PrintProfile_Fld_Type_Name WHERE ftn.FieldTypeID=FieldTypeID ORDER BY CASE WHEN LangID=@@LANGID THEN 0 ELSE 1 END, LangID)
	WHERE NOT EXISTS(SELECT * FROM VOL_FieldOption_InactiveByMember fi WHERE fo.FieldID=fi.FieldID AND fi.MemberID=@MemberID)
		AND pf.ProfileID=@ProfileID
...
GRANT EXECUTE ON  [dbo].[sp_VOL_PrintProfile_Fld_l] TO [cioc_login_role]
GO
