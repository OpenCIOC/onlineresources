...
FROM @DescTable nt
WHERE EXISTS(SELECT * FROM CIC_View_DisplayFieldGroup fg INNER JOIN CIC_View_DisplayFieldGroup_Name fgn ON fg.DisplayFieldGroupID=fgn.DisplayFieldGroupID WHERE fgn.Name=nt.Name AND LangID=nt.LangID AND fg.DisplayFieldGroupID<>nt.DisplayFieldGroupID AND fg.ViewType=@ViewType)
...
IF @Error = 0 BEGIN
	MERGE INTO CIC_View_DisplayFieldGroup AS fg
	...
	IF @Error=0  BEGIN
		...
		MERGE INTO CIC_View_DisplayFieldGroup_Name AS fgn
		USING ( 
