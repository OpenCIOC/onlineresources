...
SELECT @CUR_RT_ID = RECORD_TYPE FROM CIC_BaseTable WHERE NUM=@NUM
IF @RT_ID = -1 BEGIN
	SET @RT_ID = @CUR_RT_ID
END
...
SET @makeCCR = CASE
		WHEN EXISTS(SELECT * FROM CIC_View WHERE ViewType=@ViewType AND CCRFields=1) THEN 0
		WHEN NOT EXISTS(SELECT * FROM CIC_View_UpdateField uf INNER JOIN CIC_View_DisplayFieldGroup fg ON uf.DisplayFieldGroupID=fg.DisplayFieldGroupID INNER JOIN GBL_FieldOption fo ON uf.FieldID=fo.FieldID WHERE fg.ViewType=@ViewType AND fo.FieldType='CCR') THEN 0
		WHEN @RSN IS NULL AND @NUM IS NULL THEN 0
		ELSE CASE WHEN EXISTS(SELECT * FROM CCR_BaseTable ccbt INNER JOIN GBL_BaseTable bt ON bt.NUM=ccbt.NUM WHERE bt.RSN=@RSN OR ccbt.NUM=@NUM) THEN 0 ELSE 1 END
	END

SELECT @makeCCR AS makeCCR,
	deleteCCR = CASE
		WHEN NOT EXISTS(SELECT * FROM CCR_BaseTable WHERE NUM=@NUM) THEN 0
		WHEN EXISTS(SELECT * FROM CCR_BT_SCH WHERE NUM=@NUM) THEN 0
		WHEN EXISTS(SELECT * FROM CCR_BT_TOC WHERE NUM=@NUM) THEN 0
		WHEN EXISTS(SELECT * FROM CCR_BaseTable ccbt LEFT JOIN CCR_BaseTable_Description ccbtd ON ccbt.NUM=ccbtd.NUM
			WHERE ccbt.NUM=@NUM
				AND (TYPE_OF_PROGRAM IS NOT NULL
				OR BEST_TIME_TO_CALL IS NOT NULL
				OR TYPE_OF_CARE_NOTES IS NOT NULL
				OR SCHOOL_ESCORT_NOTES IS NOT NULL
				OR SUBSIDY IS NOT NULL
				OR SPACE_AVAILABLE_NOTES IS NOT NULL
				OR SPACE_AVAILABLE_DATE IS NOT NULL
				OR LICENSE_NUMBER IS NOT NULL
				OR LICENSE_RENEWAL IS NOT NULL
				OR LC_TOTAL IS NOT NULL
				OR LC_INFANT IS NOT NULL
				OR LC_TODDLER IS NOT NULL
				OR LC_PRESCHOOL IS NOT NULL
				OR LC_KINDERGARTEN IS NOT NULL
				OR LC_SCHOOLAGE IS NOT NULL
				OR LC_NOTES IS NOT NULL))
			THEN 0
		ELSE 1
		END,
	(SELECT EnforceReqFields FROM GBL_Agency a INNER JOIN GBL_Users u ON a.AgencyCode=u.Agency AND u.User_ID=@User_ID) AS EnforceReqFields
...
WHERE	(CanUseUpdate = 1) 
		...
		AND (fo.PB_ID IS NULL
				OR EXISTS(SELECT * FROM CIC_BT_PB pr WHERE pr.NUM=@NUM AND pr.PB_ID=fo.PB_ID)
				OR @NUM IS NULL AND (
					EXISTS(SELECT * FROM CIC_View vw WHERE vw.ViewType=@ViewType AND vw.PB_ID=fo.PB_ID)
					OR EXISTS(SELECT * FROM CIC_View_AutoAddPub vwa WHERE vwa.ViewType=@ViewType AND vwa.PB_ID=fo.PB_ID)
				)
			)
		AND (@makeCCR = 0 OR fo.FieldType <> 'CCR')
		AND ((@RT_ID IS NULL AND @CUR_RT_ID IS NULL) OR (@RT_ID=@CUR_RT_ID)
			OR EXISTS(SELECT * FROM CIC_View_UpdateField uf2
				INNER JOIN CIC_View_DisplayFieldGroup fg2 ON uf2.DisplayFieldGroupID=fg2.DisplayFieldGroupID
