...
SELECT vo.RECORD_OWNER, vod.POSITION_TITLE, vod.DUTIES, CASE WHEN vo.DISPLAY_UNTIL IS NULL OR vo.DISPLAY_UNTIL > GETDATE() THEN 0 ELSE 1 END AS EXPIRED,
		...
		c.CMP_Name AS CONTACT_NAME, c.TITLE AS CONTACT_TITLE, c.ORG AS CONTACT_ORG, c.CMP_PhoneFull AS CONTACT_PHONE, c.CMP_Fax AS CONTACT_FAX, c.EMAIL AS CONTACT_EMAIL,
	...
	LEFT JOIN GBL_BaseTable_Description btd
		ON bt.NUM=btd.NUM AND btd.LangID=(SELECT TOP 1 LangID FROM GBL_BaseTable_Description WHERE NUM=btd.NUM ORDER BY CASE WHEN LangID=vod.LangID THEN 0 ELSE 1 END, LangID)
	LEFT JOIN GBL_Contact c
