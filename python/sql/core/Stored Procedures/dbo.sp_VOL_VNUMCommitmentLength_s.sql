...
SELECT cl.CL_ID, cln.LangID, CASE WHEN cln.LangID=@@LANGID THEN cln.Name ELSE '[' + cln.Name + ']' END AS CommitmentLength, prn.Notes,
	...
	INNER JOIN dbo.VOL_CommitmentLength_Name cln
		ON cl.CL_ID=cln.CL_ID AND cln.LangID=(SELECT TOP 1 LangID FROM dbo.VOL_CommitmentLength_Name WHERE CL_ID=cln.CL_ID ORDER BY CASE WHEN LangID=@@LANGID THEN 0 ELSE 1 END, LangID)
	LEFT JOIN dbo.VOL_OP_CL pr 
		ON cl.CL_ID = pr.CL_ID AND pr.VNUM=@VNUM
	LEFT JOIN dbo.VOL_OP_CL_Notes prn
...
WHERE pr.OP_CL_ID IS NOT NULL
	...
	OR (cl.MemberID IS NULL AND (
		NOT EXISTS(SELECT * FROM dbo.VOL_CommitmentLength_InactiveByMember WHERE CL_ID=cl.CL_ID AND MemberID=ISNULL(vo.MemberID,@MemberID))
	))
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_VNUMCommitmentLength_s] TO [cioc_vol_search_role]
GO
