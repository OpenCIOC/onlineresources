...
CREATE PROCEDURE [dbo].[sp_CIC_SRCH_TAX_u]
	@TOP int OUTPUT
...
END ELSE BEGIN
	UPDATE cbtd
		...
		FROM CIC_BaseTable_Description cbtd
		WHERE cbtd.SRCH_Taxonomy_U = 1
			AND cbtd.NUM IN (SELECT TOP (@TOP) NUM FROM CIC_BaseTable_Description WHERE SRCH_Taxonomy_U=1)
END
...
SELECT @TOP=COUNT(*) FROM CIC_BaseTable_Description WHERE SRCH_Taxonomy_U=1
...
GRANT EXECUTE ON  [dbo].[sp_CIC_SRCH_TAX_u] TO [cioc_login_role]
GO
