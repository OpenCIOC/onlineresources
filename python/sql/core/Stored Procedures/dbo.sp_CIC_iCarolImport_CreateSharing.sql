...
WHERE ib.SOURCE_DB_CODE = 'ICAROL' AND ib.EXTERNAL_ID IS NOT NULL AND ib.MemberID=@MemberID AND NOT EXISTS(SELECT * FROM dbo.CIC_iCarolImportRollup i WHERE ib.EXTERNAL_ID=i.ResourceAgencyNum) AND EXISTS(SELECT * FROM dbo.GBL_BaseTable_Description btd WHERE btd.NUM=ib.NUM AND btd.DELETION_DATE IS NULL)
