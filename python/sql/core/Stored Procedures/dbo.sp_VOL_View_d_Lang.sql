...
CREATE PROCEDURE [dbo].[sp_VOL_View_d_Lang]
	@ViewType [int],
	@MemberID [int],
	@AgencyCode [char](3),
	@LangID [smallint],
...
-- Language exists ?
END ELSE IF NOT EXISTS(SELECT * FROM VOL_View_Description WHERE ViewType=@ViewType AND LangID=@LangID) BEGIN
	SET @Error = 3 -- No Such Record
	SET @ErrMsg = cioc_shared.dbo.fn_SHR_STP_FormatError(@Error, CAST(@LangID AS varchar), @LanguageObjectName)
...
END ELSE BEGIN
	...
	DELETE FROM VOL_View_Description 
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_View_d_Lang] TO [cioc_login_role]
GO
