...
CREATE PROCEDURE [dbo].[sp_VOL_ExcelProfile_l]
	@MemberID int,
	@ViewType int
...
WHERE MemberID=@MemberID
	AND Domain=2
	AND (@ViewType IS NULL OR EXISTS(SELECT * FROM VOL_View_ExcelProfile vw WHERE vw.ViewType=@ViewType AND vw.ProfileID=ep.ProfileID))
ORDER BY epn.Name
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_ExcelProfile_l] TO [cioc_login_role]
GO
