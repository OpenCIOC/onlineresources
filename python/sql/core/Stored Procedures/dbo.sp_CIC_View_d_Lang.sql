...
CREATE PROCEDURE [dbo].[sp_CIC_View_d_Lang]
	@ViewType [int],
	@MemberID [int],
	@AgencyCode [char](3),
	@LangID [smallint],
...
-- Language exists ?
END ELSE IF NOT EXISTS(SELECT * FROM CIC_View_Description WHERE ViewType=@ViewType AND LangID=@LangID) BEGIN
	SET @Error = 3 -- No Such Record
	SET @ErrMsg = cioc_shared.dbo.fn_SHR_STP_FormatError(@Error, CAST(@LangID AS varchar), @LanguageObjectName)
...
END ELSE BEGIN
	...
	DELETE FROM CIC_View_Description 
	WHERE ViewType=@ViewType AND LangID=@LangID
	
	EXEC @Error = cioc_shared.dbo.sp_STP_UnknownErrorCheck @@ERROR, @LanguageObjectName, @ErrMsg
...
GO
GRANT EXECUTE ON  [dbo].[sp_CIC_View_d_Lang] TO [cioc_login_role]
GO
