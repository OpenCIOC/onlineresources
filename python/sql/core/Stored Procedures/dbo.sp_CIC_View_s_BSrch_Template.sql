...
CREATE PROCEDURE [dbo].[sp_CIC_View_s_BSrch_Template]
	@ViewType [int],
...
IF @PreviewTemplate_ID IS NOT NULL BEGIN
	...
	WHERE Template_ID=@PreviewTemplate_ID AND PreviewTemplate=1 AND (SystemTemplate=1 OR MemberID IS NULL OR MemberID=(SELECT MemberID FROM CIC_View WHERE ViewType=@ViewType))
END

IF @Template_ID IS NULL BEGIN
	SELECT @Template_ID=Template FROM CIC_View WHERE ViewType=@ViewType
END
...
SELECT tp.*,
	...
	LEFT JOIN GBL_Template_Layout tl
		ON tl.LayoutID=tp.SearchLayoutCIC OR (tp.SearchLayoutCIC IS NULL AND tl.LayoutID=(SELECT TOP 1 LayoutID FROM GBL_Template_Layout WHERE DefaultSearchLayout=1 AND LayoutType='cicsearch'))
...
WHERE Template_ID = @Template_ID
	AND LangID=@@LANGID AND MenuType='cicsearch' 
...
GRANT EXECUTE ON  [dbo].[sp_CIC_View_s_BSrch_Template] TO [cioc_cic_search_role]
GRANT EXECUTE ON  [dbo].[sp_CIC_View_s_BSrch_Template] TO [cioc_login_role]
GRANT EXECUTE ON  [dbo].[sp_CIC_View_s_BSrch_Template] TO [cioc_vol_search_role]
GO
