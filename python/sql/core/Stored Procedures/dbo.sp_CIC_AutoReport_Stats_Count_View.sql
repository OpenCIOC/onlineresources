...
CREATE PROCEDURE [dbo].[sp_CIC_AutoReport_Stats_Count_View] (
	@MemberID int,
	@PB_ID int
)
...
SELECT DATENAME(mm, st.AccessDate) + ' ' + CAST(YEAR(st.AccessDate) AS varchar) AS TheMonth,
	...
	ORDER BY YEAR(st.AccessDate), MONTH(AccessDate), ViewName
...
GRANT EXECUTE ON  [dbo].[sp_CIC_AutoReport_Stats_Count_View] TO [cioc_login_role]
GO
