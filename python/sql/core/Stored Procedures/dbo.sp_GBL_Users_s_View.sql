...
SELECT	vw.ViewType,
		...
		OrgLevel2Name,
		OrgLevel3Name,
		(SELECT FullSSLCompatible_Cache FROM GBL_Template WHERE Template_ID=Template) AS TemplateFullSSLCompatible,
		(SELECT FullSSLCompatible_Cache FROM GBL_Template WHERE Template_ID=PrintTemplate) AS PrintFullSSLCompatible,
		PDFBottomMessage,
		PDFBottomMargin,
		AllowPDF,
