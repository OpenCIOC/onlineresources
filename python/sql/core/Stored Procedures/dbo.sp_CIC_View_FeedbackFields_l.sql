...
CREATE PROCEDURE [dbo].[sp_CIC_View_FeedbackFields_l]
	@MemberID int,
	@AgencyCode char(3),
	@ViewType int,
	@RT_ID [int]
...
IF @RT_ID IS NOT NULL BEGIN
	SET @RT_HAS_FORM = CAST(CASE WHEN EXISTS(SELECT ff.FieldID, fg.DisplayOrder, fg.DisplayFieldGroupID
			FROM dbo.CIC_View_FeedbackField ff
			INNER JOIN dbo.CIC_View_DisplayFieldGroup fg
				ON ff.DisplayFieldGroupID=fg.DisplayFieldGroupID AND fg.ViewType=@ViewType
			WHERE ff.RT_ID=@RT_ID) THEN 1 ELSE 0 END AS bit)
...
SELECT	fo.FieldID,
	...
	LEFT JOIN (SELECT ff.FieldID, fg.DisplayOrder, fg.DisplayFieldGroupID, fgn.Name AS DisplayFieldGroupName
			FROM dbo.CIC_View_FeedbackField ff
			INNER JOIN dbo.CIC_View_DisplayFieldGroup fg
				ON ff.DisplayFieldGroupID=fg.DisplayFieldGroupID AND fg.ViewType=@ViewType
			LEFT JOIN dbo.CIC_View_DisplayFieldGroup_Name fgn
				ON fgn.LangID=@@LANGID AND fgn.DisplayFieldGroupID=fg.DisplayFieldGroupID
			WHERE (((@RT_ID IS NULL OR @RT_HAS_FORM=0) AND ff.RT_ID IS NULL) OR ff.RT_ID=@RT_ID)) fg
...
GO
GRANT EXECUTE ON  [dbo].[sp_CIC_View_FeedbackFields_l] TO [cioc_login_role]
GO
