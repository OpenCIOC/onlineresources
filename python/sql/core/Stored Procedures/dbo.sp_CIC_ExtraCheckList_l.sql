...
CREATE PROCEDURE [dbo].[sp_CIC_ExtraCheckList_l]
	@MemberID [int],
	...
	LEFT JOIN CIC_ExtraCheckList_Name excn
		ON exc.EXC_ID=excn.EXC_ID
			AND excn.LangID=CASE
				...
				ELSE (SELECT TOP 1 LangID FROM CIC_ExtraCheckList_Name WHERE EXC_ID=exc.EXC_ID ORDER BY CASE WHEN LangID=@@LANGID THEN 0 ELSE 1 END, LangID)
			END
WHERE exc.FieldName=@FieldName
	...
	AND (
		@ShowHidden=1
		OR NOT EXISTS(SELECT * FROM CIC_ExtraCheckList_InactiveByMember WHERE EXC_ID=exc.EXC_ID AND MemberID=@MemberID)
	)
...
GRANT EXECUTE ON  [dbo].[sp_CIC_ExtraCheckList_l] TO [cioc_login_role]
GO
