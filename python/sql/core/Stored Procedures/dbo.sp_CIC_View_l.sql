...
CREATE PROCEDURE [dbo].[sp_CIC_View_l]
	@MemberID int,
	@AgencyCode char(3),
	@AllAgencies bit,
...
SELECT vw.ViewType, CASE WHEN vwd.LangID=@@LANGID THEN vwd.ViewName ELSE '[' + vwd.ViewName + ']' END AS ViewName,
		CAST(CASE WHEN vw.ViewType=@DefaultViewCIC OR EXISTS(SELECT * FROM CIC_View_Recurse vwr WHERE vwr.ViewType=@DefaultViewCIC AND vwr.CanSee=vw.ViewType) THEN 1 ELSE 0 END AS bit) AS IsPublic,
		Owner
	FROM CIC_View vw
...
ORDER BY CASE WHEN vw.ViewType=@DefaultViewCIC OR EXISTS(SELECT * FROM CIC_View_Recurse vwr WHERE vwr.ViewType=@DefaultViewCIC AND vwr.CanSee=vw.ViewType) THEN 0 ELSE 1 END,
	vwd.ViewName
...
GO
GRANT EXECUTE ON  [dbo].[sp_CIC_View_l] TO [cioc_login_role]
GO
