...
CREATE PROCEDURE [dbo].[sp_VOL_PrintProfile_u]
	@ProfileID [int],
...
WHERE EXISTS(
	...
	WHERE MemberID=@MemberID
		AND ProfileName=nt.ProfileName
		AND LangID=nt.LangID
		AND pp.ProfileID<>@ProfileID AND pp.Domain=2
	)
...
IF @Error = 0 BEGIN
	...
	IF @Error = 0 BEGIN
		...
		MERGE INTO VOL_View_PrintProfile ppv
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_PrintProfile_u] TO [cioc_login_role]
GO
