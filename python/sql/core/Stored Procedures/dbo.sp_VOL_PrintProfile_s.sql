...
CREATE PROCEDURE [dbo].[sp_VOL_PrintProfile_s]
	@MemberID int,
	@AgencyCode char(3),
	@ProfileID [int]
...
-- Ownership OK ?
END ELSE IF NOT EXISTS(SELECT * FROM GBL_PrintProfile WHERE ProfileID=@ProfileID AND MemberID=@MemberID AND Domain=2) BEGIN
...
SELECT	vw.ViewType,
	...
	LEFT JOIN VOL_View_PrintProfile pp 
...
GO
GRANT EXECUTE ON  [dbo].[sp_VOL_PrintProfile_s] TO [cioc_login_role]
GO
