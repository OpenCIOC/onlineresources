...
CREATE PROCEDURE [dbo].[sp_CIC_ExportProfile_Pub_u]
	@ExportPubID int,
...
END ELSE IF NOT EXISTS (SELECT * FROM CIC_ExportProfile WHERE ProfileID=@ProfileID AND MemberID=@MemberID) BEGIN
	...
	SET @ErrMsg = cioc_shared.dbo.fn_SHR_STP_FormatError(@Error, @MemberObjectName, NULL)
END ELSE BEGIN
	UPDATE CIC_ExportProfile_Pub
		SET	IncludeDescription	= @IncludeDescription,
...
GO
GRANT EXECUTE ON  [dbo].[sp_CIC_ExportProfile_Pub_u] TO [cioc_login_role]
GO
