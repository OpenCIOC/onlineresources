...
SELECT 	[FieldID],
		...
		[MemberSpecific],
		[CanShare]
	FROM cioc_setup_source.dbo.VOL_FieldOption
WHERE (ExtraFieldType IS NULL)

INSERT INTO @FieldOption_Description
SELECT fod.[FieldID],[LangID],[FieldDisplay],[CheckboxOnText],[CheckboxOffText]
	FROM cioc_setup_source.dbo.VOL_FieldOption_Description fod
	INNER JOIN cioc_setup_source.dbo.VOL_FieldOption fo
		ON fod.FieldID=fo.FieldID
WHERE fo.ExtraFieldType IS NULL

SELECT COUNT(*) AS TOTAL_AUTH_FIELDS FROM @FieldOption

INSERT INTO @UnmatchedFields
SELECT FieldName FROM VOL_FieldOption fo
	WHERE (fo.ExtraFieldType IS NULL) AND NOT EXISTS(SELECT * FROM @FieldOption WHERE FieldName=fo.FieldName COLLATE Latin1_General_100_CI_AI)
...
SELECT FieldName FROM @FieldOption fou WHERE NOT EXISTS(SELECT * FROM VOL_FieldOption WHERE FieldName=fou.FieldName COLLATE Latin1_General_100_CI_AI)

SELECT FieldName AS NEW_FIELD FROM @NewFields

INSERT INTO VOL_FieldOption (FieldName,CREATED_DATE,CREATED_BY)
...
SELECT DISTINCT fo.FieldID,fou.FieldID,fo.FieldName
	...
	WHERE (fo.ExtraFieldType IS NULL) AND (
		...
		OR NOT ((fo.ExtraFieldType=fou.ExtraFieldType COLLATE Latin1_General_100_CI_AI) OR (fo.ExtraFieldType IS NULL AND fou.ExtraFieldType IS NULL))
		...
		--OR NOT ((fo.AllowNulls=fou.AllowNulls) OR (fo.AllowNulls IS NULL AND fou.AllowNulls IS NULL))
...
INSERT INTO VOL_FieldOption_Description (FieldID,LangID,CREATED_DATE,CREATED_BY)
SELECT cf.FieldID, foud.LangID, GETDATE(), '(Software Update)'
	FROM @FieldOption_Description foud
	INNER JOIN @ChangedFields cf
		ON foud.FieldID=cf.AuthFieldID
WHERE NOT EXISTS(SELECT * FROM VOL_FieldOption_Description WHERE FieldID=cf.FieldID AND LangID=foud.LangID)
