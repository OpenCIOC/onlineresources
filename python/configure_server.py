...
def parse_args():
	...
	parser.add_argument(
		'--db-server', dest='db_server',
		default='(local),1433',
		help='database server'
	)
	parser.add_argument(
		'--db-name', dest='db_name',
		default='cioc', help='database name'
	)
	parser.add_argument(
		'--user-name',
		dest='user_name',
	...
	parser.add_argument(
		'--password',
		default='cioc_login',
		help='db password for cioc databse'
	)
...
def populate_config_file(args):
	...
	config_file = os.path.join(config_dir, os.path.dirname(site_root) + '.ini')
