...
def main():
    ...
    for env_var, value in [
        ("CIOC_ENV_ROOT", env_root),
        ("CIOC_MAIL_HOST", "localhost"),
        ("CIOC_MAIL_PORT", "1025"),
    ]:
    ...
    gencache = [
        ...
        ["00000300-0000-0010-8000-00AA006D2EA4", "0", "2", "8"],
        ["00000600-0000-0010-8000-00AA006D2EA4", "0", "2", "8"],
        ["B691E011-1797-432E-907A-4D8C69339129", "0", "6", "1"],
        ["D97A6DA0-A85C-11CF-83AE-00A0C90C2BD8", "0", "3", "0"],
        ["D97A6DA0-9C1C-11D0-9C3C-00A0C922E764", "0", "3", "0"],
    ]
    for uuid, rev, maj, min in gencache:
        subprocess.call(
            [
                sys.executable,
                "-c",
                "from win32com.client import gencache; gencache.EnsureModule('{%(uuid)s}', %(rev)s, %(maj)s, %(min)s)"
                % {"uuid": uuid, "rev": rev, "maj": maj, "min": min},
            ]
        )
    ...
    webpi_exe = os.path.join(
        os.environ["ProgramW6432"], "Microsoft/Web Platform Installer/WebpiCmd.exe"
    )
